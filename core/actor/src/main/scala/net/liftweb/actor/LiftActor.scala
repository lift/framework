...
object LAScheduler extends LAScheduler with Loggable {
  ...
  /**
   * If it's Full, then create an ArrayBlockingQueue, otherwise create a LinkedBlockingQueue.
   * Default to Full(200000).
   */
...
trait SpecializedLiftActor[T] extends SimpleActor[T] {
  ...
  /**
   * Send a message to the Actor. This call will always succeed and return almost immediately. The
   * message will be processed asynchronously. This is a Java-callable alias for !.
   */
  ...
  /**
   * This method inserts the message at the head of the mailbox. It's protected because this
   * functionality may or may not want to be exposed.
   */
  ...
  /**
   * You can wrap calls around the evaluation of the mailbox. This allows you to set up the
   * environment.
   */
 ...
 *
 * Messages sent to an actor extending this interface are not processed, but are instead recorded in
 * a List. The intent is that when you are testing some other component (say, a snippet) that should
 * send a message to an actor, the test for that snippet should simply test that the actor received
 * the message, not what the actor does with that message. If an actor implementing this trait is
 * injected into the component you're testing (in place of the real actor) you gain the ability to
...
class MockSpecializedLiftActor[T] extends SpecializedLiftActor[T] {
  ...
  /**
   * Send a message to the mock actor, which will be recorded and not processed by the message
   * handler.
   */
...
trait LiftActor extends SpecializedLiftActor[Any]
  ...
  /**
   * Send a message to the Actor and get an LAFuture that will contain the reply (if any) from the
   * message. This method calls !&lt; and is here for Java compatibility.
   */
  ...
  /**
   * Send a message to the Actor and wait for the actor to process the message and reply. This
   * method is the Java callable version of !?.
   */
  ...
  /**
   * Send a message to the Actor and wait for up to timeout milliseconds for the actor to process
   * the message and reply. This method is the Java callable version of !?.
   */
 ...
 *
 * Much like MockSpecializedLiftActor, this class is intended to be injected into other components,
 * such as snippets, during testing. Whereas these components would normally talk to a real actor
 * that would process their message, this mock actor simply records them and exposes methods the
 * unit test can use to investigate what messages have been received by the actor.
 */
...
/**
 * Java versions of Actors should subclass this method. Methods decorated with the @Receive
 * annotation will receive messages of that type.
 */
