...
sealed trait BoxTrait {
   ...
   *         otherwise.
   */
  implicit def option2Box[T](in: Option[T]): PresenceBox[T] = Box(in)
   ...
   */
  def legacyNullTest[T](in: T): PresenceBox[T] = in match {
    case null => Empty
    case _ => Full(in)
  }
   ...
   */
  def !![T](in: T): PresenceBox[T] = legacyNullTest(in)
...
sealed abstract class Box[+A] extends Product with Serializable {
   ...
   */
  def ~>[T](errorCode: => T): ParamTryBox[A, T]

  /**
   * Alias for `[[?~]]`.
   */
  def failMsg(msg: => String): TryBox[A] = ?~(msg)
   ...
   *         box if it is a `Full`.
   */
  def ?~!(msg: => String): TryBox[A] = ?~(msg)

  /**
   * Alias for `?~!`.
   */
  def compoundFailMsg(msg: => String): TryBox[A] = ?~!(msg)
   ...
   */
  def filterMsg(msg: String)(p: A => Boolean): TryBox[A] = filter(p) ?~ msg
...
sealed trait PresenceBox[+A] extends Box[A] {
  ...
  def or[B >: A](alternative: => Full[B])(implicit a: DummyImplicit): Full[B]
  def or[B >: A](alternative: => PresenceBox[B])(implicit a: DummyImplicit, b: DummyImplicit): PresenceBox[B]
   ...
   * `PresenceBox`. `flatten` is not straightforward to specialize due to `Full`
   * participating in both `TryBox` and `PresenceBox`, so we introduce this and
   * `[[flattenTry TryBox.flattenTry]]` as a workaround.
   */
...
sealed trait TryBox[+A] extends Box[A] {
  ...
  /**
   * Variant of `flatten` that will only work on a `TryBox`, and returns a
   * `PresenceBox`. `flatten` is not straightforward to specialize due to `Full`
   * participating in both `TryBox` and `PresenceBox`, so we introduce this and
   * `[[flattenPresence PresenceBox.flattenPresence]]` as a workaround.
   */
...
final case class Full[+A](value: A) extends Box[A]
  ...
  override def or[B >: A](alternative: => Box[B]): Full[A] = this
  override def or[B >: A](alternative: => Full[B])(implicit a: DummyImplicit): Full[A] = this
  override def or[B >: A](alternative: => PresenceBox[B])(implicit a: DummyImplicit, b: DummyImplicit): Full[A] = this
  override def or[B >: A](alternative: => TryBox[B])(implicit a: DummyImplicit, b: DummyImplicit, c: DummyImplicit): Full[A] = this
  override def or[B >: A, T](alternative: => ParamTryBox[B, T])(implicit a: DummyImplicit, b: DummyImplicit, c: DummyImplicit, d: DummyImplicit): Full[A] = this

  override def exists(func: A => Boolean): Boolean = func(value)

  override def forall(func: A => Boolean): Boolean = func(value)

  override def filter(p: A => Boolean): PresenceBox[A] = if (p(value)) this else Empty

  override def foreach[U](f: A => U): Unit = f(value)

  override def map[B](f: A => B): Full[B] = Full(f(value))
  ...
  override def ?~(msg: =>String): TryBox[A] = this
  override def ?~!(msg: =>String): TryBox[A] = this
  override def ~>[T](errorCode: =>T): ParamTryBox[A, T] = this
  ...
  override def asA[B](implicit m: Manifest[B]): PresenceBox[B] = {
    this
      .isA(m.runtimeClass) // convert
      .asInstanceOf[PresenceBox[B]] // restore type
  }
...
case object Empty extends EmptyBox with PresenceBox[Nothing] {
  ...
  override def or[B >: Nothing, E](alternative: => ParamTryBox[B, E])(implicit a: DummyImplicit, b: DummyImplicit, c: DummyImplicit, d: DummyImplicit): ParamTryBox[B, E] = alternative
 ...
 * one.
 */
sealed case class Failure(msg: String, exception: Box[Throwable], chain: Box[Failure]) extends EmptyBox with TryBox[Nothing] {
  type A = Nothing
...
final class ParamFailure[T](override val msg: String,
		            ...
		            override val chain: Box[Failure], val param: T)
      extends Failure(msg, exception, chain)
      with ParamTryBox[Nothing, T]
      with Serializable {
