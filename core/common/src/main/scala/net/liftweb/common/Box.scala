 ...
 *
 * In particular, provides access to the `Box` companion object so that functions like
 * `[[Box$.legacyNullTest legacyNullTest]]` can be used easily from Java, as well as access to the
 * `[[Empty]]` singleton so that empty values can be created easily from Java.
 */
class BoxJBridge {
 ...
 *   - any `[[scala.AnyRef AnyRef]]` object, converting `null` to `[[Empty]]` and anything else to a
 *     `[[Full]]` with the given object
 *
...
object Box extends BoxTrait with Tryo {
  ...
  implicit class ListOfBoxes[T](val theListOfBoxes: List[Box[T]]) extends AnyVal {
    ...
    /**
     * Convert a `List` of `Box`es into a single `Box` containting a `List[T]`, where `T` is the
     * parameterized type of the `Box`es.
     *
     * This method is useful for those cases where you have a lot of operations being executed that
     * all return some `Box[T]`. You want just a `List[T]` if all of those operations succeeded, but
     * you don't want to have Failures disappear if any were present in the list.
     *
     * If all of the `Box`es in the `List` are `Full` or `Empty`, we return a `Full` box containing
     * a `List` of all of the `Full` `Box` values that were present. If any of the `Box`es contain a
     * `Failure`, a `ParamFailure` is returned, containing the original `List[Box[T]]` as the param.
     * The `ParamFailure` itself is chained to a `Failure` chain containing all of the `Failure`
     * boxes in the list.
     *
     * It is worth noting that the size of the list in the resulting `Box[List[T]]` may not be equal
     * to the size of the `List[Box[T]]` that is fed as `Empty` values will disappear altogether in
     * the conversion.
     *
...
private[common] sealed trait OptionImplicits {
   ...
   *
   * Note that `Box` implements `get` specifically to avoid usage of `.get` on `Box` instances.
   * Boxes should be opened using `openOrThrowException` and their contents compared using
   * `== Full(expectedValue)`.
   *
   * @return
...
sealed trait BoxTrait extends OptionImplicits {
  ...
  /**
   * Transform a `List` with zero or more elements to a `Box`, losing all but the first element if
   * there are more than one.
   *
   * @return
   *   `Full(x)` with the head of the list if it contains at least one element and `Empty`
   *   otherwise.
   */
  ...
  /**
   * Apply the specified `PartialFunction` to the specified `value` and return the result in a
   * `Full`; if the `pf`` is not defined at that point return `Empty`.
   *
   * @param pf
  ...
  /**
   * This implicit transformation allows one to use a `Box` as an `Iterable` of zero or one
   * elements.
   *
   * @return
  ...
  /**
   * Create a `Full` box containing the specified value if `in` is an instance of the specified
   * class `clz` and `Empty` otherwise.
   *
   * This is basically a Java-friendly version of `[[asA]]`, which you should prefer when using
   * Scala.
   *
   * For example:
   * {{{
  ...
  /**
   * Create a `Full` box containing the specified value if `in` is of type `B` and `Empty`
   * otherwise.
   *
   * For example:
   * {{{
...
/**
 * Used as a return type for certain methods that should not be called. One example is the `get`
 * method on a Lift `Box`. It exists to prevent client code from using `.get` as an easy way to open
 * a `Box`, so it needs a return type that will match no valid client return types.
 */
final class DoNotCallThisMethod

/**
 * The `Box` class is a container which is able to declare if it is `Full` (containing a single
 * non-null value) or `[[EmptyBox]]`. An `EmptyBox`, or empty, can be the `[[Empty]]` singleton,
 * `[[Failure]]` or `[[ParamFailure]]`. `Failure` and `ParamFailure` contain information about why
 * the `Box` is empty including exception information, possibly chained `Failure`s and a `String`
 * message.
 *
 * This serves a similar purpose to the `[[scala.Option Option]]` class from Scala standard library
 * but adds several features:
 *   - You can transform it to a `Failure` object if it is `Empty` (with the `[[?~]]` or
 *     `[[failMsg]]` method).
 *   - You can chain failure messages on `Failure`s (with the `?~!` or `[[compoundFailMsg]]`
 *     method).
 ...
 *
 * If you grew up on Java, you're used to `Exception`s as part of your program logic. The Scala
 * philosophy and the Lift philosophy is that exceptions are for exceptional conditions such as
 * failure of an external resource (e.g., your database goes offline) rather than simply indicating
 * that a parameter wasn't supplied or couldn't be parsed.
 *
 * Lift's `Box` and Scala's `Option` provide mechanisms for being explicit about a value existing or
 * not existing rather than relying on a reference being not-null. However, extracting a value from
 * a `Box` should be done correctly. Available options are:
 ...
 *      }
 *     }}}
 *   - Using `map`, `flatMap`, `filter`, and `foreach` (`for` comprehensions use these under the
 *     covers):
 *     {{{
...
sealed abstract class Box[+A] extends Product with Serializable {
  ...
  /**
   * Returns `true` if this `Box` contains no value (i.e., it is `Empty` or `Failure` or
   * `ParamFailure`).
   */
  def isEmpty: Boolean
  ...
  /**
   * The only time when you should be using this method is if the value is guaranteed to be
   * available based on a guard outside of the method. In these cases, please provide that
   * information in the justification `String`. For example,
   * `User.currentUser.openOrThrowException("This snippet is only used on pages where the user is
   * logged in")`. For tests, use `[[==]]` or `[[===]]` instead. See the class documentation for
   * more information.
   *
   * A valid justification for using this method should not be "I want my code to fail fast when I
   * call it." Using exceptions in the core logic of your application should be strongly
   * discouraged.
   *
   * @param justification
   *   Justify why calling this method is okay and why it will not result in an exception being
   *   thrown. This serves both as mandatory documentation and as a very clear indication of what
   *   unexpected thing happened in the event you were wrong about the guard.
   *
   * @return
   *   The contents of the `Box` if it is `Full`.
   * @throws NullPointerException
   *   If you attempt to call it on an `EmptyBox`, with a message that includes the provided
   *   `justification`.
   */
  def openOrThrowException(justification: => String): A

  /**
   * Exists to avoid the implicit conversion from `Box` to `Option`. Opening a `Box` unsafely should
   * be done using `openOrThrowException`.
   *
   ...
   * Equivalent to `Option`'s `[[scala.Option.getOrElse getOrElse]]`.
   */
  def openOr[B >: A](default: => B): B = default

  /**
   * Apply a function to the value contained in this `Box` if it exists and return a `Full`
   * containing the result. If this `Box` is not already `Full`, return the unchanged box.
   *
   * @note
  ...
  /**
   * Apply a function returning a `Box` to the value contained in this `Box` if it exists and return
   * the resulting `Box`. If this `Box` is not already `Full`, return this box unchanged.
   *
   * @note
  ...
  /**
   * If this `Box` contains a value and it satisfies the specified `predicate`, return the `Box`
  ...
  /**
   * If this `Box` contains a value and it satisfies the specified `func`, return `true`. Otherwise,
   * return `false`.
   *
   * @return
  ...
  /**
   * If this `Box` contains a value and it does '''not''' satisfy the specified `f`, return the
   * `Box` unchanged. Otherwise, return an `Empty`.
   */
   ...
   * The function does not run if this `Box` is empty.
   */
  ...
  /**
   * Create a `Full` box containing the specified value if this box's value is of type `B` and
   * `Empty` otherwise.
   *
   * For example:
   * {{{
  ...
  /**
   * Returns an `[[scala.collection.Iterator Iterator]]` over the value contained in this `Box`, if
   * any.
   */
  def elements: Iterator[A] = Iterator.empty

  /**
   * Return this Box if `Full`, or the specified alternative if it is empty. Equivalent to
   * `Option`'s `[[scala.Option.orElse orElse]]`.
   */
  ...
  /**
   * Returns a `[[scala.collection.immutable.List List]]` of one element if this is Full, or an
   * empty list if empty.
   */
  def toList: List[A] = Nil

  /**
   * Returns the contents of this box wrapped in `Some` if this is `Full`, or `None` if this is
   * empty (meaning an `Empty`, `Failure` or ParamFailure`).
   */
   ...
   * @return
   *   A `ParamFailure` with the specified value, unless this is already a `ParamFailure` or a
   *   `Full`. If this is a `Failure`, the `ParamFailure` will preserve the message of the
   *   `Failure`.
   */
   ...
   *
   * If this is an `Empty`, this method behaves like `[[?~]]`. If it is a `Failure`, however, this
   * method returns a new `Failure` with the given `msg` and with its `[[Failure.chain chain]]` set
   * to this `Failure`.
   *
   * As with `[[?~]]`, if this is a `Full`, we return it unchanged.
   *
   * @return
   *   A `Failure` with the message if this `Box` is an `Empty` box. Chain this box to the new
   *   `Failure` if this is a `Failure`. The unchanged box if it is a `Full`.
   */
  ...
  /**
   * If this `Box` contains a value and it satisfies the specified `predicate`, return the `Box`
   * unchanged. Otherwise, return a `Failure` with the given `msg`.
   *
   * @see
   *   [[filter]]
   *
   * @return
   *   A `Failure` with the message if the box is empty or the predicate is not satisfied by the
   *   value contained in this Box.
   */
  def filterMsg(msg: String)(p: A => Boolean): Box[A] = filter(p) ?~ msg

  /**
   * This method calls the specified function with the specified `in` value and the value contained
   * in this `Box`. If this box is empty, returns the `in` value directly.
   *
   * @return
  ...
  /**
   * Perform a side effect by passing this `Box` to the specified function and return this `Box`
   * unmodified. Similar to `foreach`, except that `foreach` returns `Unit`, while this method
   * allows chained use of the `Box`.
   *
   * @return
   *   This box.
   */
  ...
  /**
   * For `Full` and `Empty`, this has the expected behavior. Equality in terms of Failure checks for
   * equivalence of failure causes:
   * {{{
   ...
   * }}}
   *
   * For other values, determines equality based upon the contents of this `Box` instead of the box
   * itself. As a result, it is not symmetric. As an example:
   * {{{
   * val foo = "foo"
   ...
   * }}}
   *
   * It is safest to use `===` explicitly when you're looking for this behavior, and use `==` only
   * for box-to-box comparisons:
   * {{{
  ...
  /**
   * Returns true if the value contained in this box is equal to the specified value. This is the
   * same thing that `==` does when it's handed a value that isn't a `Box`, but using this is
   * recommended because it's clearer that the behavior will be different than the usual
   * expectation.
   */
  ...
  /**
   * If the `Box` is `Full`, apply the transform function `f` on the value `v`; otherwise, just
   * return the value untransformed.
   *
   * The transform function is expected to be a function that will take the value `v` and produce a
   * function from the value in the box to a new value of the same type as `v`.
   *
   * For example:
   * {{{
   * val myBox = Full(10)
   ...
   * @return
   *   If the `Box` is `Full`, the value once transformed by the function returned by `f`.
   *   Otherwise, the initial value `v`.
   */
  def fullXform[T](v: T)(f: T => A => T): T = v

  /**
   * An `[[scala.util.Either Either]]` that is a `Left` with the given argument `left` if this is
   * empty, or a `Right` with the boxed value if this is `Full`.
   */
  def toRight[B](left: => B): Either[B, A] = Left(left)

  /**
   * An `[[scala.util.Either Either]]` that is a `Right` with the given argument `right` if this is
   ...
   *
   * Although this function is different for true collections, because `Box` is really a collection
   * of 1, the two functions are identical.
   */
  ...
  /**
   * Transforms this box using the `transformFn`. If `transformFn` is defined for this box, returns
   * the result of applying `transformFn` to it. Otherwise, returns this box unchanged.
   *
   * If you want to change the content of a `Full` box, using `[[map]]` or `[[collect]]` might be
   * better suited to that purpose. If you want to convert an `Empty`, `Failure` or a `ParamFailure`
   * into a `Full` box, you should use `[[flip]]`.
   *
   * @example
   *   {{{
   *
   * // Returns Full("alternative") because the partial function covers the case. Full("error")
   * transform { case Full("error") => Full("alternative") }
   *
   * // Returns Full(1), this Full box unchanged, because the partial function doesn't cover the
   * case. Full(1) transform { case Full(2) => Failure("error") }
   *
   * // Returns this Failure("another-error") unchanged because the partial function doesn't cover
   * the case. Failure("another-error") transform { case Failure("error", Empty, Empty) =>
   * Full("alternative") }
   *
   * // Returns Full("alternative") for an Empty box since `partialFn` is defined for Empty Empty
   * transform { case Empty => Full("alternative") }
   *
   * // Returns Empty because the partial function is not defined for Empty Empty transform { case
  ...
  /**
   * Returns a `Full` box containing the results of applying `flipFn` to this box if it is a
   * `Failure`, `ParamFailure` or `Empty`. Returns `Empty` if this box is `Full`. In other words, it
   * "flips" the full/empty status of this Box.
   */
...
/**
 * An `EmptyBox` is a `Box` containing no value. It can sometimes carry additional failure
 * information, as in `[[Failure]]` and `[[ParamFailure]]`.
 */
...
/**
 * A `Failure` is an `[[EmptyBox]]` with an additional failure message explaining the reason for its
 * being empty. It can also optionally provide an exception and/or a chain of previous `Failure`s
 * that may have caused this one.
 */
sealed case class Failure(msg: String, exception: Box[Throwable], chain: Box[Failure])
    extends EmptyBox {
  type A = Nothing
  ...
  /**
   * Return a list of the exceptions that led to this `Failure`. First, unflattens the list of
   * causes of this `Failure`'s `exception`. Then, if this `Failure` has a `chain`, walks down it
   * and concatenates their `exceptionChain` to the end of this one's.
   *
   * @return
   *   A single list of `Throwable`s from the most direct cause to the least direct cause of this
   *   `Failure`.
   */
  ...
  /**
   * Reduce this `Failure`'s message and the messages of all chained failures a to a single
   * `String`. The resulting string links each step in the failure chain with <-, and this
   * `Failure`'s message is last.
   *
   * For example:
   * {{{
...
/**
 * Companion object used to simplify the creation of simple `ParamFailure`s, as well as allow
 * pattern-matching on the `ParamFailure`.
 */
object ParamFailure {
...
/**
 * A `ParamFailure` is a `[[Failure]]` with an additional type-safe parameter that can allow an
 * application to store other information related to the failure.
 *
 * For example:
 * {{{
 * val loggedInUser =
...
final class ParamFailure[T](
    ...
    val param: T) extends Failure(msg, exception, chain) with Serializable {
...
/**
 * Sometimes it's convenient to access either a `[[Box]][T]` or a `T`. If you specify `BoxOrRaw[T]`,
 * either a `T` or a `Box[T]` can be passed and the "right thing" will happen, including `null`s
...
/**
 * Companion object with implicit conversions to allow `BoxOrRaw[T]` to masquerade as the
 * appropriate types.
 */
object BoxOrRaw {
