...
/**
 * Via an `[[HLists.HList HList]]` containing a collection of `[[Box]]`, either generates an `HList`
...
object CombinableBox {
  ...
  implicit def boxCombinationToCombinableBox[A, B <: HList](in: Result[A :+: B])
      : CombinableBox[A, B] =
    CombinableBox(in)
  ...
  /**
   * If the `[[Failure]]` is going to be condensed, a `FailureList` is generated to allow type-safe
   * pattern matches without worrying about erasure.
   */
