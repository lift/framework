...
/**
 * Implements an LRU Hashmap. Given a size, this map will evict the least recently used item(s) when
 * new items are added.
 *
 ...
 * @param expiredFunc
 *   When a key-value pair is removed, the last thing that happens is that these functions are
 *   invoked. Note that this happens after `[[expired]]` is invoked.
 */
class LRUMap[K, V](initMaxSize: Int, loadFactor: Box[Float], expiredFunc: ((K, V) => Unit)*)
  ...
  /**
   * Updates the `LRUMap`'s current max size to `newMaxSize`, evicting the oldest entries if the
   * size has shrunk.
   */
  ...
  /**
   * Fetches the given key, returning `[[Empty]]` if the key does not exist in the map. A key may
   * not be in the map either if it was never added or if it has been expired.
   *
   ...
   *   If the key does not exist in the map. Use `get` instead to get a safe `[[Box]]` result that
   *   can be checked for existence, or use `[[contains]]` before calling this.
   */
  def apply(key: K) =
  ...
  /**
   * Check if the given `key` exists in the map. A key may not be in the map either if it was never
   * added or if it has been expired.
   */
   ...
   *
   * Marks the given `value` as the most recently used, and, if this `key` is new in the map and the
   * map has grown beyond the specifiex `[[maxSize]]`, evicts the least-recently-used entries.
   */
  ...
  /**
   * Override this method to implement a test to see if a particular element can be expired from the
   * cache.
   */
  ...
  /**
   * A mechanism for expiring elements from cache. This method can devolve into O(n ^ 2) if lots of
   * elements can't be expired.
   */
