...
trait TypedActor[-T, +R] extends SimpleActor[T] {
  ...
  /**
   * Compatible with Scala actors' `!?` method, sends the given `message` to this actor and waits
   * infinitely for a reply.
   */
  def !?(message: T): R

  /**
   * Compatible with Scala actors' `!?` method, sends the given `message` to this actor and waits up
   * to `timeout` for a reply, returning `[[Empty]]` or `[[Failure]]` if no reply is received by
   * then.
   */
  def !?(timeout: Long, message: Any): Box[R]

  /**
   * Asynchronous message send. Send-and-receive eventually. Waits on a Future for the reply
   * message. If recevied within the Actor default timeout interval then it returns `Full(result)`
   * and if a timeout has occured `[[Empty]]` or `[[Failure]]`.
   */
  def !!(message: T): Box[R]

  /**
   * Asynchronous message send. Send-and-receive eventually. Waits on a Future for the reply
   * message. If recevied within timout interval that is specified then it returns `Full(result)`
   * and if a timeout has occured `[[Empty]]` or `[[Failure]]`.
   */
...
/**
 * Generic Actor interface. Can receive any type of message. Can return (via `!!` and `!?`) messages
 * of type `R`.
 */
