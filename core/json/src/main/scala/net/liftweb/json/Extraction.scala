...
object Extraction {
  ...
  def extract[A](json: JValue)(implicit formats: Formats, mf: Manifest[A]): A = {
    ...
    try {
      ...
      extract0(json, types.head, types.tail,false).asInstanceOf[A]
    } catch {
  ...
  def extractOpt[A](json: JValue)(implicit formats: Formats, mf: Manifest[A]): Option[A] = {
    def allTypes(mf: Manifest[_]): List[Class[_]] = mf.erasure :: (mf.typeArguments flatMap allTypes)
    val types = allTypes(mf)
    val r = extract0(json, types.head, types.tail,true)
    if (r == None) None else Some(r.asInstanceOf[A])
  }
  ...
  private def extract0(json: JValue, clazz: Class[_], typeArgs: Seq[Class[_]],optional: Boolean)
    ...
    extract0(json, mkMapping(clazz, typeArgs), optional)
  }

  def extract(json: JValue, target: TypeInfo)(implicit formats: Formats): Any =
    extract0(json, mappingOf(target.clazz),false)

  private def extract0(json: JValue, mapping: Mapping, optional: Boolean)(implicit formats: Formats): Any = {
    def newInstance(constructor: Constructor, json: JValue) = {
          ...
          formats.fieldSerializer(a.getClass).map { serializer =>
            ...
            fieldsToSet.foreach { case (name, typeInfo) =>
              jsonFields.get(name).foreach { case (n, v) =>
                ...
                val value = extract0(v, typeInfo.clazz, typeArgs.getOrElse(Nil),optional)
      ...
      def instantiate = {
        ...
        if (!(args contains None) || !optional) {
          try {
    ...
    def newPrimitive(elementType: Class[_], elem: JValue) = convert(elem, elementType, formats, optional)
    ...
    def build(root: JValue, mapping: Mapping): Any = mapping match {
      case Value(targetType) => convert(root, targetType, formats,optional)
  ...
  private def convert(json: JValue, targetType: Class[_], formats: Formats, optional: Boolean): Any = json match {
    ...
    case JNull => null
    case JNothing => if (optional) None else fail("Did not find value which can be converted into " + targetType.getName)
    case JField(_, x) => convert(x, targetType, formats,optional)
    case _ =>
      val custom = formats.customDeserializer(formats)
