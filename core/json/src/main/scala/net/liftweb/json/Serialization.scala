...
/**
 * Functions to serialize and deserialize a case class. Custom serializer can be inserted if a class
 * is not a case class. <p> Example:<pre> val hints = new ShortTypeHints( ... ) implicit val formats =
...
object Serialization {
  ...
  /**
   * Create Serialization formats with given type hints. <p> Example:<pre> val hints = new
   * ShortTypeHints( ... ) implicit val formats = Serialization.formats(hints) </pre>
   */
