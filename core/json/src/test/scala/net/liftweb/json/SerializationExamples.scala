...
object SerializationExamples extends Specification {
  ...
  "Primitive-wrapping case class serialization example" in {
    val primitives =
      Primitives(124, 123L, 126.5, 127.5.floatValue, "128", Symbol("s"), 125, 129.byteValue, true)
...
object FullTypeHintExamples extends TypeHintExamples {
  ...
  implicit val formats: Formats = Serialization.formats(FullTypeHints(List[Class[_]](
    classOf[Animal],
    classOf[True],
    classOf[False],
    classOf[Falcon],
        ...
        (
          {
            case JObject(List(JField("$dt", JString(s)))) =>
              format.dateFormat.parse(s).getOrElse(throw new MappingException(
                "Can't parse " + s + " to Date"))
          },
          {
        ...
        json match {
          case JArray(xs) =>
            ...
            xs.map(x =>
              Extraction.extract(
                x,
                TypeInfo(t.getActualTypeArguments()(0).asInstanceOf[Class[_]], None))).toIndexedSeq
