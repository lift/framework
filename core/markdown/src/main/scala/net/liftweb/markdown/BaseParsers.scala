...
/**
 * Basic parsers for Markdown Source. Provides general, small parsers that are used by other
 * parsers. Also contains parsers for XML elements.
 */
...
trait BaseParsers extends RegexParsers {
  ...
  /**
   * Whitespace is sometimes important in markdown parsing, we handle it manually. So this returns
   * false.
   */
  ...
  /**
   * Matches everything in the parsed string up to the end. Also matches the empty String. Returns
   * the matched String.
   */
  ...
  /**
   * Matches exactly one char, no matter which. This differs from "elem" as it returns a string
   * consisting of that char.
   */
  ...
  /**
   * Matches one of the given char keys in the map. Returns the string value for the matched char in
   * the given map.
   */
  ...
  def ranges(rs: SortedMap[Char, Char]): Parser[Char] = Parser { in =>
    if (in.atEnd) Failure("End of input.", in)
      ...
      val (begin: Char, end: Char) = if (lower.isEmpty)
  ...
  /////////////////////////////////////////////////////////////////////////
  /**
   * a mapping of all chars that have to be escaped in xml and the resulting escape sequence The
   * array solution is very ugly, but cuts down block parsing time by 25%
   */
  ...
  /**
   * Escapes the given char for XML. Returns Either the necessary XML escape Sequence or the same
   * char in a String.
   */
  ...
  /**
   * Either returns the XML escape sequence for the given char or null. This does not return
   * Option[String] on purpose. While Option[String] would be a much cleaner solution, this is
   * actually called so often that it is a noticeable difference if we use Option here.
   */
  ...
  /**
   * Parser for one char that starts an XML name. According to W3C specs except that range #x10000
   * to #xEFFFF is excluded (cannot be expressed by char literals)
   */
   ...
   */
  def xmlName: Parser[String] = xmlNameStartChar ~ (xmlNameChar *) ^^ { case c ~ cs =>
    c.toString + cs.mkString
  }

  /**
   * Parses a Simplified xml attribute: everything between quotes ("foo") everything between the
   * quotes is run through the escape handling That way you can omit xml escaping when writing
   * inline XML in markdown.
   */
  ...
  /**
   * Runs the given parser on the given input. Expects the parser to succeed and consume all input.
