...
/**
 * A parser for the markdown language. Works on pre-parsed lines that can be created by a
 * LineParser.
 */
...
trait BlockParsers extends Parsers {
  ...
  object Ruler extends MarkdownBlock {
    def addResult(level: Int, out: StringBuilder): Unit = {
      out.append(indent(level)).append(BlockParsers.this.deco().decorateRuler())
    }
  }
   ...
   * Represents a header
   */
  case class Header(content: String, headerLevel: Int, lookup: Map[String, LinkDefinition])
      extends MarkdownBlock {
    def addResult(level: Int, out: StringBuilder): Unit = {
      out.append(indent(level)).append(BlockParsers.this.deco().decorateHeaderOpen(headerLevel))
        .append(applyInline(content, lookup))
        .append(indent(level)).append(BlockParsers.this.deco().decorateHeaderClose(headerLevel))
    }
  }
  ...
  class CodeBlock(lines: List[MarkdownLine]) extends MarkdownBlock {
    def addResult(level: Int, out: StringBuilder): Unit = {
      out.append(indent(level)).append(BlockParsers.this.deco().decorateCodeBlockOpen())
      for (line <- lines) {
        val escaped = escapeXml(line.payload)
        out.append(escaped).append('\n')
        // out.append(line.content)
      }
      out.append(indent(level)).append(BlockParsers.this.deco().decorateCodeBlockClose())
    }
  }
  ...
  class Paragraph(lines: List[MarkdownLine], lookup: Map[String, LinkDefinition])
    ...
    def addResult(level: Int, out: StringBuilder): Unit = {
      out.append(indent(level)).append(BlockParsers.this.deco().decorateParagraphOpen())
      addResultPlain(level, out)
      out.append(indent(level)).append(BlockParsers.this.deco().decorateParagraphClose())
    }

    /**
     * Adds the result without any decoration, (no wrapping tags) Used for building list items that
     * don't have their content wrappend in paragraphs
     */
  ...
  class Blockquote(lines: List[MarkdownLine], lookup: Map[String, LinkDefinition])
    ...
    def addResult(level: Int, out: StringBuilder): Unit = {
      ...
      out.append(indent(level)).append(BlockParsers.this.deco().decorateBlockQuoteOpen())
      innerBlocks.foreach(block => block.addResult(level + 1, out))
      out.append(indent(level)).append(BlockParsers.this.deco().decorateBlockQuoteClose())
    }
  }
   ...
   */
  class ListItem(val lines: List[MarkdownLine], lookup: Map[String, LinkDefinition])
      extends LineParsers {
  ...
  abstract class ListBlock(items: List[ListItem]) extends MarkdownBlock {
    ...
    /**
     * This method recursively goes through the given list and adds the items contents. It checks
     * the previous item if it ends with empty lines. If it does, it signals the current item to
     * create paragraphs. In order for this method to work it has to be called with the first item
     * prepended twice in front of the list. So if the list is a::b::c, call this method with
     * a::a::b::c
     */
    protected def addResult(level: Int, out: StringBuilder, list: List[ListItem]): Unit =
      list match {
  ...
  class OList(items: List[ListItem]) extends ListBlock(items) {
    override def addResult(level: Int, out: StringBuilder): Unit = {
      out.append(indent(level)).append(BlockParsers.this.deco().decorateOListOpen())
      super.addResult(level, out)
      out.append(indent(level)).append(BlockParsers.this.deco().decorateOListClose())
    }
  }
  ...
  class UList(items: List[ListItem]) extends ListBlock(items) {
    override def addResult(level: Int, out: StringBuilder): Unit = {
      out.append(indent(level)).append(BlockParsers.this.deco().decorateUListOpen())
      super.addResult(level, out)
      out.append(indent(level)).append(BlockParsers.this.deco().decorateUListClose())
    }
  }
  ...
  /**
   * Parses a fenced code block: a line starting a fenced code block with "```", followed by any
   * lines that do not stop it, optionally followed by the ending line. Optionally parsing the
   * stopping line causes the code block to extend to the end of the document. (This is the github
   * behavior, where omitting the line closing the code block causes the block to extend to the end
  ...
  /**
   * a consecutive block of paragraph lines returns the content of the matched block wrapped in <p>
   * tags
   */
  def paragraph: Parser[Paragraph] = lookup ~ (line(classOf[OtherLine]) +) ^^ { case lu ~ ls =>
    new Paragraph(ls, lu)
  }

  /**
   * Parses a blockquote fragment: a block starting with a blockquote line followed by more
   * blockquote or paragraph lines, ends optionally with empty lines
   */
  ...
  /**
   * Parses a quoted block. A quoted block starts with a line starting with "> " followed by more
   * blockquote lines, paragraph lines following blockqoute lines and may be interspersed with empty
   * lines
   */
  ...
  /**
   * The continuation of a list item: A line indented by four spaces or a tab (a continuation line),
   * followed by more continuation or paragraph lines followed by empty lines
   */
