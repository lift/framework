...
/**
 * This trait influences the behavior of the Markdown output of inline and block parsers and the
 * complete transformer. Mix in this trait and override methods to change the behavior and override
 * the "deco()" method in the respective parser/transformer to return your modified instances to
 * change the output they create.
 *
 * Inline element decoration methods always get passed the spanned text, so you have to prepend and
 * append the opening/closing tags. For block elements there is always a method for the opening and
 * closing tags. This is to make block processing more efficient to prevent unnecessary String
 * building of whole blocks just to add tags. (The block building uses a StringBuilder internally
 * and just appends the returned tags)
 *
 * If you want line breaks after opening/closing block level tags, you have to add the newline
 * yourself.
 */
...
trait Decorator {
  ...
  /**
   * If true, inline xml tags and verbatim xml blocks are allowed, otherwise they are escaped and
