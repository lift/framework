...
/**
 * A parser for inline markdown, markdown escapes and XML escapes. This is used by the result
 * classes of the block parsers to handle Markdown within a block.
 */
...
trait InlineParsers extends BaseParsers {
  ...
  /**
   * A set of tags we have already created. Used to prevent nesting a link in a link or an emphasis
   * in an emphasis.
   */
  ...
  /**
   * This array is used as a lookup for mapping markdown escapes to the resulting char (if necessary
   * already escaped for XML) Disgusting, I know, but this is such an often called operation that
   * this is the fastest way to do it, even in the year 2010.
   */
  ...
  /**
   * Parses markdown text up to any of the chars defined in the given map. used to quickly escape
   * any text between special inline markdown like emphasis.
   */
  ...
  /**
   * Chars that may indicate the start of a special markdown inline sequence or the end of a link
   * text.
   */
  ...
  /**
   * Inline markdown in a link. Like normal inline stuff but stops when it reaches a closing square
   * bracket.
   */
  def linkInline(ctx: InlineContext)
      : Parser[String] = // ( (not(']') ~> oneInline(ctx.addTag("a")))* ) ^^ {_.mkString}
    ((markdownText(
      specialLinkInlineChars,
      true) | elementParsers(ctx) | ((not(']') ~> aChar))) *) ^^ { _.mkString }
  ...
  /**
   * A title is everything in quotation marks. We allow even quotation marks in quotation marks. We
   * look ahead if we hit the closing brace after the quotation marks to detect if the title ends or
   * not.
   */
  val title: Parser[Option[String]] =
    opt('"' ~> ((markdownText(Set('"'), true) ~ opt(not('"' ~ ows ~ ')') ~> aChar)) *) <~ '"') ^^ {
      case None => None
      case Some(chunks) => {
  ...
  /**
   * Parses an id reference. (Any text that is not a square bracket) Succeeds only if the parsed id
   * is found in the given lookup. Returns the found link definition and the matched text.
   */
  def idReference(ctx: InlineContext): Parser[(String, LinkDefinition)] =
    guard(acceptMatch(ctx.map)(refText ^^ (_.trim.toLowerCase))) ~ refText ^^ { case ld ~ t =>
      (t, ld)
    }

  /**
   * A markdown reference of the form [text][id], [idText][] or [idText] Parser returns a tuple with
   * the link definition first and the text to display second.
   */
