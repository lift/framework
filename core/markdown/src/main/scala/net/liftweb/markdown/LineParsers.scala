...
/**
 * Represents a line of markdown. The prefix is the beginning of the line that indicates the line
 * type, the payload is the actual content after the prefix.
 */
sealed abstract class MarkdownLine(val prefix: String, val payload: String) {

  /**
   * Constructs a MarkdownLine where the prefix is the empty String and the payload is the whole
   * line.
   */
...
/**
 * Represents lines of verbatim xml. Actually this class is a little cheat, as it represents
 * multiple lines. But it is a token that is created when "parsing with a line scope", so it is not
 * too bad.
 */
...
case class AtxHeaderLine(pre: String, pay: String) extends MarkdownLine(pre, pay) {
  ...
  /**
   * removes all whitespace, nl and trailing hashes from the payload " foo ## \n" => "foo"
   */
  def trimHashes() = {
    val s = payload.trim
    var idx = s.length - 1
...
/**
 * This class allows us to reference a map with link definitions resulting from the line parsing
 * during block parsing. It extends a Reader for MarkdownLines and allows us to add the said map to
 * the parsing context. This is basically a modification of the parser monad's state.
 */
case class MarkdownLineReader private (
  ...
  /**
   * Not existing line that signals EOF. This object cannot be referenced by any other code so it
   * will fail all line parsers.
   */
...
/**
 * Parses single lines into tokens. Markdown lines are differentiated by their beginning. These
 * lines are then organized in blocks by the BlockParsers.
 */
trait LineParsers extends InlineParsers {
  ...
  /**
   * A link definition that later gets stripped from the output. Either a link definition on one
   * line or the first line of a two line link definition.
   */
  def linkDefinitionStart: Parser[(LinkDefinitionStart, Option[String])] =
    linkDefinitionId ~ linkDefinitionUrl ~ opt(linkDefinitionTitle) ^^ { case i ~ u ~ t =>
   ...
   */
  val ruler: Parser[MarkdownLine] = """ {0,3}(((-[ \t]*){3,})|((\*[ \t]*){3,}))$""".r ^^ {
    new RulerLine(_)
  }

  /**
   * Matches a line starting with up to three spaces, a '>' and an optional whitespace. (i.e.: the
   * start or continuation of a block quote.)
   */
  ...
  /**
   * A line that starts an unordered list item. Matches a line starting with up to three spaces
   * followed by an asterisk, a space, and any whitespace.
   */
  ...
  /**
   * A line that starts an ordered list item. Matches a line starting with up to three spaces
   * followed by a number, a dot and a space, and any whitespace
   */
  ...
  /**
   * Matches the start of a fenced code block with additional language token: up to three spaces,
   * three or more backticks, whitespace, an optional language token, optional whitespace
   */
  ...
  /**
   * Matches any line. Only called when all other line parsers have failed. Makes sure line
   * tokenizing does not fail and we do not loose any lines on the way.
   */
