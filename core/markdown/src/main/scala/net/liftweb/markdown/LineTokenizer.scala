...
/**
 * A Reader for reading whole Strings as tokens. Used by the Tokenizer to parse whole lines as one
 * Element.
 */
case class LineReader private (
...
/**
 * Chops the input into lines and turns those lines into line tokens. Also takes care of
 * preprocessing link definitions and xml blocks.
 */
class LineTokenizer() extends Parsers {
  ...
  /**
   * Determines if xml blocks may be included verbatim. If true, they are passed through, else they
   * are escaped and turned into paragraphs
   */
  def allowXmlBlocks = true

  /**
   * Returns a parser based on the given line parser. The resulting parser succeeds if the given
   * line parser consumes the whole String.
   */
  ...
  /**
   * Returns the first char in the given string or a newline if the string is empty. This is done to
   * speed up header parsing. Used to speed up line tokenizing substantially by using the first char
   * in a line as lookahead for which parsers to even try.
   */
  ...
  /**
   * Finds the char in the given line that is the best indication of what kind of markdown line this
   * is. The “special” Markdown lines all start with up to three spaces. Those are skipped if
   * present. The first char after those (up to)three spaces or a newline is returned.
   */
  ...
  /**
   * Tries to parse an URL from the next line if necessary. The passed tuple is the result from a
   * previous parser and used to decide how to continue parsing.
   */
  ...
  /**
   * Parses tokens that may occur inside a block. Works like the normal token parser except that it
   * does not check for link definitions and verbatim XML.
   */
  ...
  /**
   * Simple preprocessing: split the input at each newline. These whole lines are then fed to the
   * actual Tokenizer.
   */
   ...
   * Does not check for XML chunks or link definitions.
   */
