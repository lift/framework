...
/**
 * This is the Transformer that uses the other parsers to transform markdown into xhtml. Mix this
 * trait in if you want more control over the output (like switching verbatim xml on/off or using
...
/**
 * Simple Standalone Markdown transformer. Use this if you simply want to transform a block of
 * markdown without any special options. val input:String = ... val xhtml:String = new
 * ActuariusTransformer()(input)
 *
 * Note that this markdown parser isn't inherently thread-safe, as Scala Parser Combinators aren't,
 * so this class instantiates a SingleThreadedTransformer for each thread. You'll need to write your
 * own pooled implementation if this isn't efficient for your usage.
 */
