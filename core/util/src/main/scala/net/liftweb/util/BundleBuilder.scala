...
/**
 * Converts a NodeSeq of a particular format into a ResourceBundle. Basically, each of the
 * second-level nodes that contain the attribute "name" the name becomes an entry in the resulting
 * resource bundle. It is possible to localize each of the entries with the lang and country
 * attributes which will be compared against the incoming Locale. If the default attribute is true,
 * then that entry is used if no others match. Note that language is weighted more heavily than
 * country.<br/><br/> If the node is a Text or PCData node, it will be returned as a String.
