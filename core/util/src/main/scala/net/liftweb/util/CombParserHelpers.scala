...
trait CombParserHelpers {
   ...
   *
   * @note
   *   Currently, it supports the BMP (Basic Multilingual Plane) set from U+0000 to U+FFFF and not
   *   the supplementary characters having code points greater than U+FFFF.
   *
   * @see
   ...
   *   list of characters
   * @return
   *   a unit parser which will succeed if the input matches the list of characters regardless of
  ...
  private def acceptCIChar(c: Elem) =
    acceptIf(a => xform(a) == xform(c))("`" + c + "' expected but " + _ + " found")
  ...
  /**
   * @param func
   *   list permutation function. Returns all permutations on the list or all permutations on the
   *   list plus all permutations on sublists for example
   * @return
...
trait SafeSeqParser extends Parsers {
   ...
   *
   * <p> rep1(f, p) first uses `f' (which must succeed) and then repeatedly uses `p' to parse the
   * input until `p' fails (the result is a `List' of the consecutive results of `f' and `p')</p>
   *
   * @param first
   ...
   * @return
   *   A parser that returns a list of results produced by first applying `f' and then repeatedly
   ...
   * A parser generator for non-empty repetitions.
   *
   * <p>rep1sep(first, p, q) starts by using `first', followed by repeatedly uses of `p' interleaved
   * with `q' to parse the input, until `p' fails. `first' must succeed (the result is a `List' of
   * the consecutive results of `first' and `p')</p>
   *
   * @param first
   ...
   * @param q
   *   a `Parser' that parses the elements that separate the elements parsed by `p' (interleaved
   *   with `q')
   * @return
   *   A parser that returns a list of results produced by repeatedly applying `p' to the input (and
   *   that only succeeds if `p' matches at least once). The results of
   *   `p' are collected in a list. The results of `q' are discarded.
   */
