...
final case class ElemSelector(elem: String, subNodes: Box[SubNode]) extends CssSelector {
...
final case class IdSelector(id: String, subNodes: Box[SubNode]) extends CssSelector {
  def withSubnode(sn: SubNode): CssSelector = this.copy(subNodes = Full(sn))
}

final case class ClassSelector(clss: String, subNodes: Box[SubNode]) extends CssSelector {
  def withSubnode(sn: SubNode): CssSelector = this.copy(subNodes = Full(sn))
}

final case class NameSelector(name: String, subNodes: Box[SubNode]) extends CssSelector {
  def withSubnode(sn: SubNode): CssSelector = this.copy(subNodes = Full(sn))
}
...
object CssSelectorParser extends PackratParsers with ImplicitConversions {
  ...
  private def fixAll(all: List[CssSelector], sn: Option[SubNode]): CssSelector = {
    (all, sn) match {
      ...
      case (lst, Some(sn)) =>
        (lst.dropRight(1) ::: lst.takeRight(1).map(_.withSubnode(sn))).reduceRight((b, a) =>
          EnclosedSelector(b, a))
    }
  }
