...
class Currency(val amount: Long, val symbol: String, val decimals: Int) {
  ...
  /**
   * Return a string formatted as the URL-encoded symbol followed by the amount and decimals
  ...
  /**
   * Addition on Currency objects. This compares currency symbols to prevent addition of different
   * types of currency to one another.
   * @throws CurrencyMismatchException
   *   for mismatched currency types.
   */
  ...
  /**
   * Subtraction on Currency objects. This compares currency symbols to prevent subtraction of
   * different types of currency from one another.
   * @throws CurrencyMismatchException
   *   for mismatched currency types.
   */
  def -(other: Currency): Currency =
...
/**
 * This exception is thrown if an operation is attempted on two currency values where currency
...
object Currency {
  ...
  def apply(s: String): Box[Currency] = s.roboSplit("&") match {
    case List(cur, a, d) =>
      for (av <- asLong(a); dv <- asInt(d)) yield new Currency(av, urlDecode(cur), dv)
    case _ => Empty
  }
}
