      ...
      case e: java.lang.NumberFormatException => {
        try {
          ...
          case e: java.text.ParseException => {
            make(BigDecimal("" + NumberFormat.getCurrencyInstance(locale).parse(x)))
  ...
  abstract class AbstractCurrency(val designation: String) extends Ordered[Currency] {
    ...
    override def equals(that: Any) = that match {
      case that: AbstractCurrency =>
        this.designation + this.format("", scale) == that.designation + that.format("", scale)
      case _ => false
    }
    ...
    def format(currencySymbol: String, numberOfFractionDigits: Int): String = {
      val moneyValue = amount match {
        case null => 0
        case _ =>
          amount.setScale(numberOfFractionDigits, BigDecimal.RoundingMode.HALF_UP).doubleValue;
      }
