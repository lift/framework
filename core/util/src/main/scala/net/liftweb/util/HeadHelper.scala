...
object HeadHelper {
   ...
   * same src attribute and <link> containing the same href attribute value are considered
   * duplicates.
   */
  def removeHtmlDuplicates(in: NodeSeq): NodeSeq = {
    ...
    Text("\n\t") ++ (in flatMap { e =>
      ...
      e match {
        case e: Elem if (e.label == "script") && (src != null) && (jsSources contains src) =>
          NodeSeq.Empty
        case e: Elem if (e.label == "script") && (src != null) && (!(jsSources contains src)) =>
          jsSources += src; e

        case e: Elem if (e.label == "link") && (href != null) && (hrefs contains href) =>
          NodeSeq.Empty
        case e: Elem if (e.label == "link") && (href != null) && !(hrefs contains href) =>
          hrefs += href; e
  ...
  /**
   * This method finds all &lt;head&gt; tags that are descendants of &lt;body&gt; tags in the
   * specified NodeSequence and merges the contents of those tags into the &lt;head&gt; tag closest
