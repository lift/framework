...
trait HttpHelpers {
  ...
  /**
   * Given a map of HTTP properties, return true if the "Content-type" value in the map is either
   * "text/html" or "application/xhtml+xml"
   * @param in
   *   Map which may contain a key named Content-Type
   * @return
   *   true if there is a pair ("Content-Type", "text/html") or ("Content-Type",
  ...
  /**
   * If the specified Elem has an attribute named 'id', return it, otherwise construct a new Elem
   * with a randomly generated id attribute and return the pair
   *
   * @param in
  ...
  /**
   * Within a NodeSeq, find the first elem and run it through the function. Return the resulting
   * NodeSeq
   */
  ...
  /**
   * Given a URL and a Lift function String, append the function even if the URL has query params
   * and a #
   */
  ...
  /**
   * Split a String at the Hash sign, run the function on the non-# side and then append the hash
   * side
   */
  ...
  /**
   * Given a list of query parameters, append them to the URL taking into account # and if there are
  ...
  /**
   * Map the specified function over the elements of the specified NodeSeq and return the
   * concatenated result. This is essentially a container-type-transforming flatMap operation.
   */
