...
trait ListHelpers {
  ...
  /**
   * Compute the deltas between two sequences of a given type. Apply the function based on the
   * differences between the two lists. The resulting List of commands will be returned.
   */
  def delta[T, Res](old: Box[Seq[T]], newList: Seq[T])(f: DeltaInfo[T] => Res): List[Res] =
    delta(old openOr Nil, newList)(f)

  /**
   * Compute the deltas between two sequences of a given type. Apply the function based on the
   * differences between the two lists. The resulting List of commands will be returned. The
   * algorithm used to calculate the diffs is not very efficient and can degrade to O(n^2), so it's
   * not great for large collections. Internally the Seq[T] are converted to a List[T]. Finally,
   * it's highly recommended that T be immutable and does proper equals testing (e.g., a case
   * class).
   */
  ...
  /**
   * Returns a Full Box with the first element x of the list in for which f(x) evaluates to true. If
   * f(x) evaluates to false for every x, then an Empty Box is returned.
   *
   * @param in
  ...
  /**
   * Returns the first application of f to an element of in that results in a Full Box. If f applied
   * to an element of in results in an Empty Box, then f will be applied to the rest of the elements
   * of in until a Full Box results. If the list runs out then an Empty Box is returned.
   *
   * @param in
   *   a list of elements to which f can be applied
   * @param f
  ...
  /**
   * Given an incoming list, return a set of lists that is the original list rotated through all its
   * positions
   *
   * @param in
   *   the list to rotate
  ...
  def permuteList[T](in: Seq[T]): List[List[T]] = (in.toList: @unchecked) match {
    ...
    case xs => rotateList(xs).flatMap(x =>
        (x: @unchecked) match {
  ...
  /**
   * Given a list, return all the permutations including the removal of items (does not return a Nil
   * list unless in is Nil).
   *
   * @param in
   *   the list to permute
   *
  ...
  def permuteWithSublists[T](in: Seq[T]): List[List[T]] = {
    def internal(in: List[T]): List[List[T]] = in match {
      ...
      case xs =>
        ...
        val ret =
          rot.flatMap(z => (z: @unchecked) match { case x :: xs => permuteList(xs).map(x :: _) })
