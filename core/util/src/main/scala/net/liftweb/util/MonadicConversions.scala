...
object MonadicConversions {
  ...
  implicit def bool2Monadic(cond: Boolean)
      : Product with MonadicCondition with java.io.Serializable = cond match {
    case true => True
    case _ => False(Nil)
  }
...
/**
 * A MonadicCondition allows building boolean expressions of the form (a(0) && a(1) && .. && a(n)),
 * where a(k) is a boolean expression, and collecting the computation failures to a list of
 * messages.
 *
 * <pre> Example:
 *
 ...
 *
 * val exp = (!isTooYoung ~ "too young") && (!isTooBad ~ "too bad") && (!isTooStupid ~ "too stupid")
 *
 * println(exp match { case False(msgs) => msgs mkString("Test failed because it is '", "' and '",
 * "'.") case _ => "success" }) </pre>
 */
