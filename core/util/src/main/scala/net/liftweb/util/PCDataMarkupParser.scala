...
object HtmlEntities {
  ...
  val entMap: Map[String, Char] = Map.empty ++ entList.map { case (name, value) =>
    (name, value.toChar)
  }
...
class PCDataXmlParser(val input: Source) extends ConstructingHandler
    with PCDataMarkupParser[PCDataXmlParser] with ExternalSources {
  val preserveWS = true
  ent ++= HtmlEntities()
  import scala.xml._
  ...
  override def xAttributes(pscope: NamespaceBinding): (MetaData, NamespaceBinding) = {
    ...
    def findIt(base: MetaData, what: MetaData): MetaData = (base, what) match {
      ...
      case (pb: PrefixedAttribute, pn: PrefixedAttribute) if pb.key == pn.key && pb.pre == pn.key =>
        pn
