...
trait Settable extends ValueHolder {
  ...
  /**
   * Perform an atomic update of this Settable. The current value is passed to the function and the
   * ValueHolder is set to the result of the function. This is enclosed in the
   * performAtomicOperation method which will, by default, synchronize this instance
   */
  def atomicUpdate(f: ValueType => ValueType): ValueType =
    performAtomicOperation(set(f(get)))

  /**
   * Perform an atomic operation on the Settable. By default synchronizes the instance, but it could
