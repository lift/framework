...
/**
 * A wiring Cell. A Cell can be a ValueCell which holds a value which can be set (and thus update
 * the dependencies), a FuncCell (a cell that is a function that depends on other cells), or a
 * DynamicCell which has a value that updates each time the cell is accessed.
 */
trait Cell[T] extends Dependent {
  ...
  /**
   * Add a Dependent to this cell. The Dependent will be notified when the Cell's value changes.
   * Dependents are kept around as WeakReferences so they do not have to be explicitly removed from
   * the List
   */
  ...
  /**
   * Notify dependents of a state change. Note this will be performed on a separate thread
   * asynchronously
   */
...
/**
 * A cell that changes value on each access. This kind of cell could be used to access an external
 * resource. <b>Warning</b> the function may be accessed many times during a single wiring
 * recalculation, so it's best to use this as a front to a value that's memoized for some duration.
 */
final case class DynamicCell[T](f: () => T) extends Cell[T] {
  ...
  /**
   * If the predicate cell changes, the Dependent will be notified. This Cell has no predicates, so
   * this is just Unit
   */
  def predicateChanged(which: Cell[_]): Unit = {}
...
final class ValueCell[A](initialValue: A) extends Cell[A] with LiftValue[A] {
  ...
  /**
   * If the predicate cell changes, the Dependent will be notified. A ValueCell cannot have
   * predicates
   */
  def predicateChanged(which: Cell[_]): Unit = {}
...
object FuncCell {
   ...
   */
  def apply[A, B, C, Z](a: Cell[A], b: Cell[B], c: Cell[C])(f: (A, B, C) => Z): Cell[Z] =
    FuncCell3(a, b, c, f)
  ...
  def apply[A, B, C, D, Z](
      ...
      b: Cell[B],
      c: Cell[C],
      d: Cell[D])(f: (A, B, C, D) => Z): Cell[Z] = FuncCell4(a, b, c, d, f)
  ...
  def apply[A, B, C, D, E, Z](
      ...
      c: Cell[C],
      d: Cell[D],
      e: Cell[E])(f: (A, B, C, D, E) => Z): Cell[Z] = FuncCell5(a, b, c, d, e, f)
}
...
final case class FuncCell3[A, B, C, Z](a: Cell[A], b: Cell[B], c: Cell[C], f: (A, B, C) => Z)
    extends Cell[Z] {
  private var value: Z = _
  private var ct: Long = 0
