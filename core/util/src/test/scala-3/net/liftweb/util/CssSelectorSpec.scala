/*
 * Copyright 2010-2011 Lift Committers and Contributors
 *
...
class CssBindHelpersSpec extends Specification with XmlMatchers {
  ...
  "css bind helpers" should {
    ...
    "not duplicate classes" in {
      ...
      ((res \\ "a").head \ "@class").head.text === "selected L"
    }
    ...
    "Another nested select" in {
      ...
      ret(0).asInstanceOf[Elem].label === "div"
      ret.length === 1
      (ret \ "@id").text === "get"

      (ret \\ "input").length === 2

      ((ret \\ "input").toList(0) \ "@type").map(_.text) === List("moose")
    ...
    "Select a node and transform stuff" in {
      ...
      ret(0).asInstanceOf[Elem].label === "span"
      ret.length === 1
      (ret \ "@id").text === "bar"
    }
    ...
    "substitute multiple Strings by id" in {
      ("#foo" #> "hello" &
        "#baz" #> "bye"
        )(<b><div id="baz">Hello</div><span id="foo"/></b>) === (NodeSeq fromSeq <b>{Text("bye")}{Text("hello")}</b>)
    }
    ...
    "remove an attribute from a class" in {
      ...
      (func(<span class="foo andOther" />) \ "@class").text === "foo"
    }
    ...
    "transform child content on *+" in {
      ...
      res.text === "I like moose"
    }

    "transform child content on -*" in {
      val res = ("* -*" #> "moose").apply(<a> I like</a>)
      res.text === "moose I like"
    }
    ...
    "substitute multiple Strings with a List by id" in {
      ("#foo" #> "hello" &
        "#baz" #> List("bye", "bye"))(<b><div id="baz">Hello</div><span id="foo"/></b>) === (NodeSeq fromSeq <b>{Text("bye")}{Text("bye")}{Text("hello")}</b>)
    }

    "substitute multiple Strings with a List by id" in {
      (("#foo" replaceWith "hello") &
        ("#baz" replaceWith List("bye", "bye")))(<b><div id="baz">Hello</div><span id="foo"/></b>) === (NodeSeq fromSeq <b>{Text("bye")}{Text("bye")}{Text("hello")}</b>)
    }
