...
that the template when rendered alone (say, for a user test, or when a frontend
...
Notice that we combine the two CSS selector transforms here by using the `&`.
You can chain together as many CSS selector transforms as you want this way, as
long as they don't modify the same parts of the same element. We'll deal with
that limitation [a little later](#section13) footnote:[This is because CSS
