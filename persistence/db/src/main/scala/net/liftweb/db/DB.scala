...
trait DB extends Loggable {
  ...
  /**
   * queryCollector can be used to collect all statements executed in a single request when passed
   * to addLogFunc
   *
   * Use S.queryLog to get the list of (statement, duration) entries or set an analyzer function
   * using S.addAnalyzer
   */
  ...
  private def jndiConnection(name: ConnectionIdentifier): Box[Connection] = {
    val toTry: List[() => Connection] = List(
      () => {
        logger.trace(
          "Trying JNDI lookup on java:/comp/env followed by lookup on %s".format(name.jndiName))
        (new InitialContext).lookup("java:/comp/env").asInstanceOf[Context].lookup(
          name.jndiName).asInstanceOf[DataSource].getConnection
      },
      () => {
        logger.trace("Trying JNDI lookup on java:/comp/env/%s".format(name.jndiName))
        (new InitialContext).lookup(
          "java:/comp/env/" + name.jndiName).asInstanceOf[DataSource].getConnection
  ...
  /**
   * Allows you to override the connection manager associated with particular connection identifiers
  ...
  private def newConnection(name: ConnectionIdentifier): SuperConnection = {
    ...
    val cmConn = for {
      connectionManager <-
        threadLocalConnectionManagers.box.flatMap(_.get(name)) or Box(connectionManagers.get(name))
  ...
  /**
   * Build a LoanWrapper to pass into S.addAround() to make requests for the
   * DefaultConnectionIdentifier transactional for the complete HTTP request
   */
  def buildLoanWrapper(): LoanWrapper =
    buildLoanWrapper(List(DefaultConnectionIdentifier))

  /**
   * Build a LoanWrapper to pass into S.addAround() to make requests for the List of
   * ConnectionIdentifiers transactional for the complete HTTP request
   */
    ...
    (info.get(name): @unchecked) match {
      case Some(ConnectionHolder(c, 1, post, manualRollback)) => {
        ...
        try {
          ...
          logger.trace("Invoking %d postTransaction functions. rollback=%s".format(
            post.size,
            rolledback))
  ...
  /**
   * Append a function to be invoked after the transaction on the specified connection identifier
   * has ended. The value passed to the function indicates true for success/commit or false for
   * failure/rollback.
   *
   * Note: the function will only be called when automatic transaction management is in effect,
   * either by executing within the context of a buildLoanWrapper or a DB.use {}
   */
  def appendPostTransaction(name: ConnectionIdentifier, func: Boolean => Unit): Unit = {
    info.get(name) match {
      case Some(ConnectionHolder(c, n, post, rb)) =>
        info(name) = ConnectionHolder(c, n, func :: post, rb)
        logger.trace("Appended postTransaction function on %s, new count=%d".format(
          name,
          post.size + 1))
      case _ =>
  ...
  /**
   * Append function to be invoked after the current transaction on DefaultConnectionIdentifier has
   * ended
   */
  ...
  private def asString(pos: Int, rs: ResultSet, md: ResultSetMetaData): String = {
    ...
    md.getColumnType(pos) match {
      case ARRAY | BINARY | BLOB | DATALINK | DISTINCT | JAVA_OBJECT | LONGVARBINARY | NULL | OTHER | REF | STRUCT | VARBINARY =>
  ...
  /**
   * Executes the given parameterized query string with the given parameters. Parameters are
   * substituted in order. For Date/Time types, passing a java.util.Date will result in a Timestamp
   * parameter. If you want a specific SQL Date/Time type, use the corresponding java.sql.Date,
   * java.sql.Time, or java.sql.Timestamp classes.
   */
  ...
  def runQuery(
      query: String,
      params: List[Any],
      connectionIdentifier: ConnectionIdentifier): (List[String], List[List[String]]) = {
  ...
  /**
   * Executes the given parameterized update string with the given parameters. Parameters are
   * substituted in order. For Date/Time types, passing a java.util.Date will result in a Timestamp
   * parameter. If you want a specific SQL Date/Time type, use the corresponding java.sql.Date,
   * java.sql.Time, or java.sql.Timestamp classes.
   */
  ...
  /**
   * Executes {@code statement} and converts the {@code ResultSet} to model instance {@code T} using
   * {@code f}
   */
...
class StandardDBVendor(
  ...
  protected def createOne: Box[Connection] = {
    tryo { t: Throwable => logger.error("Cannot load database driver: %s".format(driverName), t) } {
    ...
    (dbUser, dbPassword) match {
      case (Full(user), Full(pwd)) =>
        tryo { t: Throwable =>
          logger.error("Unable to get database connection. url=%s, user=%s".format(dbUrl, user), t)
...
trait ProtoDBVendor extends ConnectionManager {
  ...
  /**
   * Override and set to false if the maximum pool size can temporarilly be expanded to avoid pool
   * starvation
   */
  ...
  /**
   * The absolute maximum that this pool can extend to The default is 20. Override this method to
   * change.
   */
  protected def doNotExpandBeyond = 20

  /**
   * The logic for whether we can expand the pool beyond the current size. By default, the logic
   * tests allowTemporaryPoolExpansion &amp;&amp; poolSize &lt;= doNotExpandBeyond
   */
  ...
  /**
   * Test the connection. By default, setAutoCommit(false), but you can do a real query on your
   * RDBMS to see if the connection is alive
   */
      ...
      pool match {
        ...
        case Nil =>
          ...
          if (pool.isEmpty && poolSize == curSize && canExpand_?) {
            tempMaxSize += 1
            logger.debug("Temporarily expanding pool. name=%s, tempMaxSize=%d".format(
              name,
              tempMaxSize))
          }
          newConnection(name)
        ...
        case x :: xs =>
          ...
          try {
            ...
            case e: Exception =>
              try {
                logger.debug(
                  "Test connection failed, removing connection from pool, name=%s".format(name))
                poolSize = poolSize - 1
                tryo(x.close)
                newConnection(name)
              } catch {
