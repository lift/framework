...
/**
 * JDBC Driver Abstraction base class. New driver types should extend this base class. New drivers
 * should "register" in the companion object DriverType.calcDriver method.
 */
abstract class DriverType(val name: String) {
  ...
  /**
   * This specifies that the driver supports FKs in tables. Note that to enable FK generation in
   * Schemifier, you also need to set MapperRules.createForeignKeys_? to true before running it.
   */
  def supportsForeignKeys_? : Boolean = false

  /**
   * This indicates that Schemifier needs to run with a non-transacted connection. Certain databases
   * require that gathering information on tables (which Schemifier uses for updates) run outside of
   * a transaction.
   */
  ...
  /**
   * Performs an insert and optionally returns the ResultSet of the generated keys that were
   ...
   * @param pkName
   *   Zero or more generated column names that need to be returned
   */
  def performInsert[T](
  ...
  /**
   * Name of the default db schema. If not set, then the schema is assumed to equal the db user
   * name.
   */
  ...
  /**
   * Allow the driver to do specific remapping of column types for cases where not all types are
   * supported. Classes that want to do custom type mapping for columns should override the
   * customColumnTypeMap method.
   */
  ...
  /**
   * This method can be overriden by DriverType impls to allow for custom setup of Primary Key
   * Columns (creating sequeneces or special indices, for example). The List of commands will be
   * executed in order.
   */
  ...
  /**
   * This defines the syntax for adding a column in an alter. This is used because some DBs (Oracle,
   * for one) use slightly different syntax.
   */
...
object DriverType {
  var calcDriver: Connection => DriverType = conn => {
    ...
    (
      ...
      case (PostgreSqlDriver.name, major, minor) if ((major == 8 && minor >= 2) || major > 8) =>
        PostgreSqlDriver
...
object H2Driver extends DriverType("H2") {
  ...
  /**
   * Whether the primary key has been defined by the index column. H2 creates primary key for a
   * table, when AUTO_INCREMENT type is used. <--- NOT TRUE I went into the H2 console, created a
   * table with auto_increment and was able to insert duplicate ids. Then I created it with
...
abstract class BasePostgreSQLDriver extends DriverType("PostgreSQL") {
  ...
  /**
   * "$user" schema is searched before "public", but it does not exist by default, so "public" is
   * our default choice.
   */
  override def defaultSchemaName: Box[String] = Full("public")
}
...
object PostgreSqlOldDriver extends BasePostgreSQLDriver {
  ...
  override def performInsertWithGenKeys[T](
    ...
    val pkValueQuery =
      genKeyNames.map(String.format("currval('%s_%s_seq')", tableName, _)).mkString(", ")
    DB.statement(conn) {
      stmt =>
...
/**
 * Sybase ASE Driver. Tested with ASE version 15, but should work with lower versions as well.
 */
 ...
 *
 * <ul> <li>Oracle XE 10.2.0.1</li> <li>Oracle Database 11g Enterprise Edition Release 11.2.0.1.0 -
 * 64bit Production</li> </ul>
 *
