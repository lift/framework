...
trait DBLog {
   ...
   */
  protected def logStatement[T](description: String)(f: => T): T = logStatement({ ignore: T =>
    description
  })(f)
  ...
  def statementEntries: List[DBStatementEntry] = executedStatements.filter(
    _.isInstanceOf[DBStatementEntry]).reverse.asInstanceOf[List[DBStatementEntry]]

  /** Return a list of all of the DBMetaEntry instances in the log buffer */
  def metaEntries: List[DBMetaEntry] =
    executedStatements.filter(_.isInstanceOf[DBMetaEntry]).reverse.asInstanceOf[List[DBMetaEntry]]
...
object DBLog {
  ...
  private def proxyPreparedStatement(stmt: => PreparedStatement, query: String) = {
    try {
      ...
      case sqle: SQLException =>
        throw new SQLException("Error preparing statement: \"%s\"".format(query), sqle)
    }
  }

  /**
   * This class corresponds to a logged version of java.sql.Statement. All operations are supported
   * via dynamic dispatch. This is done so that we can support both JDBC3 and JDBC4 without having
   * two code trees.
   *
   * To enable logging of DB operations, use DB.addLogFunc
   */
  sealed private[DBLog] class LoggedStatementHandler(underlying: Statement)
    ...
    def invoke(proxy: Object, method: Method, args: Array[Object]): Object = method.getName match {
      ...
      case "execute" if args(1).getClass == classOf[Int] => {
        logStatement({ ret: Object =>
          "Exec \"%s\", Auto-gen keys = %s : result = %s".format(
            args(0),
            StatementConstantDescriptions.genKeyDescriptions(args(1).asInstanceOf[Int]),
            ret)
        }) {
          chain(method, args)
        }
      ...
      case "getMoreResults" => {
        logMeta({ ret: Object =>
          "Get more results (%s) : %s".format(
            StatementConstantDescriptions.getMoreResultsDescriptions(args(0).asInstanceOf[Int]),
            ret)
        }) {
          chain(method, args)
        }
      ...
      case "setFetchDirection" => {
        logMeta("Set fetch direction = " + StatementConstantDescriptions.fetchDirDescriptions(
  ...
  /**
   * This class corresponds to a logged version of java.sql.PreparedStatement. All operations should
   * be supported.
   *
   * To enable logging of DB operations, use DB.addLogFunc
   */
  sealed private[DBLog] class LoggedPreparedStatementHandler(
      stmt: String,
      underlying: PreparedStatement) extends LoggedStatementHandler(underlying) {
    ...
    override def invoke(proxy: Object, method: Method, args: Array[Object]): Object = {
      method.getName match {
        ...
        case "setAsciiStream" => {
          paramMap += args(0).asInstanceOf[Int] -> "(Ascii Stream: %s (%d bytes))".format(
            args(1),
            args(2))
          chain(method, args)
        }
        ...
        case "setBinaryStream" => {
          paramMap += args(0).asInstanceOf[Int] -> "(Binary Stream: %s (%d bytes))".format(
            args(1),
            args(2))
          chain(method, args)
        }
        ...
        case "setCharacterStream" => {
          paramMap += args(0).asInstanceOf[Int] -> "(Char stream : %s (%d bytes))".format(
            args(1),
            args(2))
          chain(method, args)
        }
        ...
        case "setDate" => {
          paramMap += args(
            0).asInstanceOf[Int] -> (String.valueOf(args(1)) + ":" + String.valueOf(args(2)))
          chain(method, args)
        }
        ...
        case "setNCharacterStream" => {
          paramMap += args(0).asInstanceOf[Int] -> "(NChar Stream : %s (%d bytes))".format(
            args(1),
            args(2))
          chain(method, args)
        }
        ...
        case "setNClob" => {
          paramMap += args(0).asInstanceOf[Int] -> "(NClob : %s (%d bytes))".format(
            args(1),
            args(2))
          chain(method, args)
        }
        ...
        case "setUnicodeStream" => {
          paramMap += args(0).asInstanceOf[Int] -> "(Unicode Stream : %s (%d bytes))".format(
            args(1),
            args(2))
          chain(method, args)
        }
