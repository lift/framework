...
trait ManyToMany extends BaseKeyedMapper {
  ...
  /**
   * An override for delete_! to propogate the deletion to all children of this parent. Returns
   * false as soon as the parent or a one-to-many field returns false. If they are all successful
   * returns true.
   */
   ...
   * @param qp
   *   Any QueryParams to limit entries in the join table (other than matching thisField to
   *   primaryKey) To limit children based on fields in the other table (not the join table), it is
   *   currently necessary to point the join mapper to a view which pulls the join table's fields as
   *   well as fields of the other table.
   */
  class MappedManyToMany[O <: Mapper[O], K2, T2 <: KeyedMapper[K2, T2]](
    ...
    protected def isJoinForChild(e: T2)(join: O): Boolean =
    ...
    override def patchInPlace(
        from: Int,
        patch: IterableOnce[T2],
        replaced: Int): MappedManyToMany.this.type = {
    ...
    /**
     * Discard the cached state of this MappedManyToMany's children and reinitialize it from the
     * database
     */
    def refresh: List[T2] = {
    ...
    /**
     * Save the state of this MappedManyToMany to the database. This will do the following: 1) Prune
     * join table instances whose "child" foreign key's value is its defaultValue, i.e., -1 2) Set
     * all join table instances' "parent" foreign key 3) Delete all join table instances whose child
     * instance was removed 4) Save all child instances 5) If step 3 succeeds save all join
     * instances 6) Return true if steps 2-4 all returned true; otherwise false
     */
    def save: Boolean = {
      ...
      _joins foreach {
        thisField.actualField(_).asInstanceOf[MappedForeignKey[K, O, X] forSome {
    ...
    /**
     * Deletes all join rows, including those marked for removal. Returns true if both succeed,
     * otherwise false
     */
    def delete_! : Boolean = {
