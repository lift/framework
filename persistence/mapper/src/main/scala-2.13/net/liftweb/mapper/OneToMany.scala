...
/**
 * Add this trait to a Mapper for managed one-to-many support For example: class Contact extends
 * LongKeyedMapper[Contact] with OneToMany[Long, Contact] { ... }
 * @tparam K
...
trait OneToMany[K, T <: KeyedMapper[K, T]] extends KeyedMapper[K, T] { this: T =>
  ...
  /**
   * An override for save to propagate the save to all children of this parent. Returns false as
   * soon as the parent or a one-to-many field returns false. If they are all successful returns
   * true.
   */
  ...
  /**
   * An override for delete_! to propagate the deletion to all children of one-to-many fields
   * implementing Cascade. Returns false as soon as the parent or a one-to-many field returns false.
  ...
  /**
   * This implicit allows a MappedForeignKey to be used as foreignKey function. Returns a function
   * that takes a Mapper and looks up the actualField of field on the Mapper.
   */
  implicit def foreignKey[
      K,
      O <: Mapper[O],
      T <: KeyedMapper[K, T]](field: MappedForeignKey[K, O, T]): O => MappedForeignKey[K, O, T] =
  ...
  class MappedOneToMany[O <: Mapper[O]](
      ...
      extends MappedOneToManyBase[O](
        () => {
          ...
          for (child <- ret) {
            foreign.actualField(child).asInstanceOf[MappedForeignKey[
              K,
              O,
              T]].primeObj(net.liftweb.common.Full(OneToMany.this: T))
          }
          ret
        },
        foreign
  ...
  /**
   * This is the base class to use for fields that represent one-to-many or parent-child
   * relationships. Maintains a list of children, tracking pending additions and deletions, and
   * keeping their foreign key pointed to this mapper. Implements Buffer, so the children can be
   * managed as one. Most users will use MappedOneToMany, however to support children from multiple
   * tables it is necessary to use MappedOneToManyBase.
   * @param reloadFunc
  ...
  class MappedOneToManyBase[O <: Mapper[_]](
    ...
    def patchInPlace(
        from: Int,
        patch: IterableOnce[O],
        replaced: Int): MappedOneToManyBase.this.type = {
    ...
    /**
     * Reloads the children from storage. NOTE: This may leave children in an inconsistent state. It
     * is recommended to call save or clear() before calling refresh.
     */
    def refresh(): Unit = {
    ...
    /**
     * Saves this "field," i.e., all the children it represents. Returns false as soon as save on a
     * child returns false. Returns true if all children were saved successfully.
     */
    def save: Boolean = {
      unlinked foreach { u =>
        val f = foreign(u)
  ...
  /**
   * Trait that indicates that the children represented by this field should be deleted when the
   * parent is deleted.
   */
