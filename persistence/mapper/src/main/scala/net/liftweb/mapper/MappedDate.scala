...
/**
 * Represents a date without hour, minute or second fields. The underlying type is java.util.Date to
 * keep things simple, but be aware that the hour, minute and second values will most likely be
...
abstract class MappedDate[T <: Mapper[T]](val fieldOwner: T) extends MappedField[Date, T] {
  ...
  /**
   * This defines the string parsing semantics of this field. Used in setFromAny. By default uses
   * LiftRules.dateTimeConverter's parseDate; override for field-specific behavior
   */
   ...
   * default uses LiftRules.dateTimeConverter's formatDate; override for field-specific behavior
   */
