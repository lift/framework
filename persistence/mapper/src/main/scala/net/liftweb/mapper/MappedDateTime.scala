...
abstract class MappedDateTime[T <: Mapper[T]](val fieldOwner: T) extends MappedField[Date, T] {
   ...
   * uses LiftRules.dateTimeConverter's parseDateTime; override for field-specific behavior
   */
   ...
   * default uses LiftRules.dateTimeConverter's formatDateTime; override for field-specific behavior
   */
  ...
  def buildSetLongValue(accessor: Method, columnName: String): (T, Long, Boolean) => Unit =
    (inst, v, isNull) =>
      doField(
        inst,
        accessor,
        { case f: MappedDateTime[T] => f.st(if (isNull) Empty else Full(new Date(v))) })
