...
/**
 * <p> A field that maps to a decimal value. Decimal precision and rounding are controlled via the
 * context parameter. The default value is zero. </p>
 *
 * <p><b><i>Note:</i></b><br/> Using MathContext.UNLIMITED, whether explicitly or implicitly, means
 * that no precision or scaling will be used for the SQL field definition; the default scale for
 * DECIMAL is zero per the SQL standard, but the precision for DECIMAL is vendor-specific. For
 * example, PostgreSQL uses maximum precision if it's not specified, but SQL Server uses a default
 * precision of 18. </p>
 *
 * @author
 *   Derek Chen-Becker
...
abstract class MappedDecimal[T <: Mapper[T]](
  ...
  /**
   * Constructs a MappedDecimal with the specified initial value and context. The scale is taken
  ...
  /**
   * Constructs a MappedDecimal with the specified initial value. The context is set to
   * MathContext.UNLIMITED (see note above about default precision). The scale is taken from the
   * initial value.
   *
   * @param fieldOwner
   *   The Mapper that owns this field
    ...
    (inst, v) =>
      doField(
        ...
        { case f: MappedDecimal[T] =>
          f.wholeSet(if (v == null) defaultValue else coerce(BigDecimal(v.getTime)))
        })

  def buildSetStringValue(accessor: Method, columnName: String): (T, String) => Unit = (inst, v) =>
    doField(
      inst,
      accessor,
      { case f: MappedDecimal[T] =>
  ...
  def buildSetActualValue(accessor: Method, data: AnyRef, columnName: String): (T, AnyRef) => Unit =
    (inst, v) =>
      doField(
        ...
        accessor,
        { case f: MappedDecimal[T] =>
          f.wholeSet(if (v == null) defaultValue else coerce(BigDecimal(v.toString)))
        })

  /**
   * Returns the SQL creation string for this field. See the note at the top of the page concerning
   * default precision.
   */
