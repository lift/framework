...
trait BaseForeignKey extends BaseMappedField {
  ...
  /**
   * Called when Schemifier adds a foreign key. Return a function that will be called when
   * Schemifier is done with the schemification.
   */
...
object MappedForeignKey {
  implicit def getObj[
      ...
      Other <: KeyedMapper[
        KeyType,
        Other]](in: MappedForeignKey[
    KeyType,
    MyOwner,
    Other]): Box[Other] = in.obj
}
...
trait MappedForeignKey[KeyType, MyOwner <: Mapper[MyOwner], Other <: KeyedMapper[KeyType, Other]]
  ...
  /**
   * Make sure the MetaMapper for the KeyedMapper we're checking is in fact the same one as we are
   * associated with. Issue #532.
   */
  ...
  /**
   * Some people prefer the name foreign to materialize the foreign reference. This is a proxy to
   * the obj method.
   */
  ...
  /**
   * Set the value from a possible instance of the foreign mapper class. v will be cached in obj. If
   * v is Empty, set the value to defaultValue (-1)
   * @return
  ...
  /**
   * This method, which gets called when the mapper class is going to be saved, sets the field's
   * value from obj if it's set to the default (!defined_?). Overrides LifecycleCallbacks.beforeSave
   */
...
abstract class MappedStringForeignKey[T <: Mapper[T], O <: KeyedMapper[String, O]](
    ...
    extends MappedString[T](fieldOwner, maxLen) with MappedForeignKey[String, T, O]
    with BaseForeignKey {
