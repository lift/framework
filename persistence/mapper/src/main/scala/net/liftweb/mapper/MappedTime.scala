...
/**
 * Represents a time with hour, minute and second fields. The underlying type is java.util.Date to
 * keep things simple, but be aware that the date portion of the values will most likely be
 * discarded when this is saved to the database.
 *
 * @see
 *   MappedDateTime
 * @see
...
abstract class MappedTime[T <: Mapper[T]](val fieldOwner: T) extends MappedField[Date, T] {
  ...
  /**
   * This method defines the string parsing semantics of this field. Used in setFromAny. By default
   * uses LiftRules.dateTimeConverter's parseTime; override for field-specific behavior
   */
  def parse(s: String): Box[Date] = LiftRules.dateTimeConverter().parseTime(s)

  /**
   * This method defines the string parsing semantics of this field. Used in toString, _toForm. By
   * default uses LiftRules.dateTimeConverter's formatTime; override for field-specific behavior
   */
