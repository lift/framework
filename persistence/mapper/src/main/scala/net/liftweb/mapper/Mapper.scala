...
trait Mapper[A <: Mapper[A]] extends BaseMapper with Serializable with SourceInfo {
  ...
  /**
   * If the instance calculates any additional fields for JSON object, put the calculated fields
   * here
   */
  ...
  /**
   * Returns the instance in a Full Box if the instance is valid, otherwise returns a Failure with
   ...
   *   called with displayHtml, fieldId, form
   */
  def flatMapFieldTitleForm[T](func: (NodeSeq, Box[NodeSeq], NodeSeq) => Seq[T]): List[T] =
   ...
   *   called with displayHtml, fieldId, form
   */
  def flatMapFieldTitleForm2[T](func: (NodeSeq, MappedField[_, A], NodeSeq) => Seq[T]): List[T] =
  ...
  /**
   * Given a function that takes a mapper field and returns a NodeSeq for the field, return, for
   * this mapper instance, a set of CSS selector transforms that will transform a form for those
   * fields into a fully-bound form that will interact with this instance.
   */
  def fieldMapperTransforms(fieldTransform: (BaseOwnedMappedField[A] => NodeSeq))
...
trait LongKeyedMapper[OwnerType <: LongKeyedMapper[OwnerType]] extends KeyedMapper[Long, OwnerType]
...
trait CreatedTrait {
  ...
  /**
   * The createdAt field. You can change the behavior of this field: <pre name="code" class="scala">
   * override lazy val createdAt = new MyCreatedAt(this) { override def dbColumnName = "i_eat_time"
   * } </pre>
   */
...
trait UpdatedTrait {
  ...
  /**
   * The updatedAt field. You can change the behavior of this field: <pre name="code" class="scala">
   * override lazy val updatedAt = new MyUpdatedAt(this) { override def dbColumnName =
   * "i_eat_time_for_breakfast" } </pre>
   */
  lazy val updatedAt: MyUpdatedAt = new MyUpdatedAt(this)

  protected class MyUpdatedAt(obj: self.type) extends MappedDateTime(obj.asInstanceOf[MapperType])
...
trait KeyedMapper[KeyType, OwnerType <: KeyedMapper[KeyType, OwnerType]] extends Mapper[OwnerType]
  ...
  override def comparePrimaryKeys(other: OwnerType): Boolean =
...
/**
 * If this trait is mixed into a validation function, the validation for a field will stop if this
 * validation function returns an error
 */
