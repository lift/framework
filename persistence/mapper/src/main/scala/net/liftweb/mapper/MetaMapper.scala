...
object MapperRules extends Factory {
  ...
  /**
   * This function converts a header name into the appropriate XHTML format for displaying across
   * the headers of a formatted block. The default is &lt;th&gt; for use in XHTML tables. If you
   * change this function, the change will be used for all MetaMappers, unless they've been
   * explicitly changed.
   */
  var displayNameToHeaderElement: String => NodeSeq = in => <th>{in}</th>

  /**
   * This function converts an element into the appropriate XHTML format for displaying across a
   * line formatted block. The default is &lt;td&gt; for use in XHTML tables. If you change this
   * function, the change will be used for all MetaMappers, unless they've been explicitly changed.
   */
  var displayFieldAsLineElement: NodeSeq => NodeSeq = in => <td>{in}</td>

  /**
   * This function is the global (for all MetaMappers that have not changed their formatFormElement
   * function) that converts a name and form for a given field in the model to XHTML for
   * presentation in the browser. By default, a table row ( &lt;tr&gt; ) is presented, but you can
   * change the function to display something else.
   */
  ...
  /**
   * Function that determines if foreign key constraints are created by Schemifier for the specified
   * connection.
   *
  ...
  /**
   * This function is used to calculate the displayName of a field. Can be used to easily localize
   * fields based on the locale in the current request
   */
  ...
  /**
   * Calculate the name of a column based on the name of the MappedField. Must be set in Boot before
   * any code that touches the MetaMapper.
   *
  ...
  /**
   * Calculate the name of a table based on the name of the Mapper. Must be set in Boot before any
   * code that tocuhes the MetaMapper.
   *
   * To get snake_case, use this
   *
...
trait MetaMapper[A <: Mapper[A]] extends BaseMetaMapper with Mapper[A] {
  ...
  /**
   * If there are model-specific validations to perform, override this method and return an
   * additional list of validations to perform
   */
  ...
  /**
   * Execute a PreparedStatement and return a List of Mapper instances. {@code f} is where the user
   * will do the work of creating the PreparedStatement and preparing it for execution.
   *
   * @param f
    ...
    DB.use(dbId) {
      conn =>
        ...
        val (query, start, max) = addEndStuffs(
          addFields(
            "SELECT COUNT(*) FROM " + MapperRules.quoteTableName.vend(_dbTableNameLC) + "  ",
            false,
            bl,
            conn),
          bl,
  ...
  def findAllFieldsDb(
      dbId: ConnectionIdentifier,
      fields: Seq[SelectableField],
      by: QueryParam[A]*): List[A] = findMapFieldDb(dbId, fields, by: _*)(v => Full(v))
  ...
  def findAll(by: QueryParam[A]*): List[A] =
    dealWithPrecache(findMapDb(dbDefaultConnectionIdentifier, by: _*)(v => Full(v)), by)
   ...
   * and Start or MaxRows values (depending on whether the driver supports LIMIT and OFFSET) and the
   * complete List of QueryParams based on any synthetic query parameters calculated during the
   * query creation.
   *
   * @param fields
   ...
   *
   * @return
   *   a Tuple of the Query String, Start (offset), MaxRows (limit), and the list of all query
   *   parameters including and synthetic query parameters
   */
        ...
        x match {
          case Cmp(field, opr, Full(_), _, dbFunc) =>
            (1 to field.dbColumnCount).foreach {
              cn =>
                updatedWhat = updatedWhat + whereOrAnd + dbFunc(MapperRules.quoteColumnName.vend(
          ...
          case ByList(field, orgVals) =>
            ...
            if (vals.isEmpty) updatedWhat = updatedWhat + whereOrAnd + " 0 = 1 "
              ...
              vals.map(v =>
                MapperRules.quoteColumnName.vend(field._dbColumnNameLC) + " = ?").mkString(
                whereOrAnd + " (",
                " OR ",
                ")")
          ...
          case in: InRaw[A, _] =>
            updatedWhat = updatedWhat + whereOrAnd + (in.rawSql match {
              case null | "" => " 0 = 1 "
              case sql => " " + MapperRules.quoteColumnName.vend(
              ...
              "(" + in.innerMeta.addEndStuffs(
                in.innerMeta.addFields(
                  "SELECT " +
                    ...
                    " FROM " +
                    MapperRules.quoteTableName.vend(in.innerMeta._dbTableNameLC) + " ",
                  false,
                  in.queryParams,
                  conn
                ),
  ...
  private def _addOrdering(in: String, params: List[QueryParam[A]]): String = {
    params.flatMap {
      case OrderBy(field, order, nullOrder) => List(MapperRules.quoteColumnName.vend(
          field._dbColumnNameLC) + " " + order.sql + " " + (nullOrder.map(_.getSql).openOr("")))
  ...
  def dirty_?(toTest: A): Boolean = mappedFieldList.exists(mft =>
  ...
  /**
   * This method will update the instance from JSON. It allows for attacks from untrusted JSON as it
   * bypasses normal security. By default, the method is protected. You can write a proxy method to
  ...
  /**
   * This method will encode the instance as JSON. It may reveal data in fields that might otherwise
   * be proprietary. It should be used with caution and only exposed as a public method after a
   * security review.
   */
  ...
  /**
   * Run the list of field validations, etc. This is the raw validation, without the notifications.
  ...
  def clean_?(toCheck: A): Boolean =
    mappedColumns.foldLeft(true)((bool, ptr) => bool && !(??(ptr._2, toCheck).dirty_?))

  /**
   * Sets a prepared statement value based on the given MappedField's value and column name. This
   * delegates to the BaseMappedField overload of setPreparedStatementValue by retrieving the
   * necessary values.
   *
   * @param conn
   *   The connection for this prepared statement
   * @param st
   *   The prepared statement
   * @param index
   *   The index for this prepared statement value
   * @param field
   *   The field corresponding to this prepared statement value
   * @param columnName
   *   The column name to use to retrieve the type and value
   * @param setObj
   *   A function that we can delegate to for setObject calls
   */
   ...
   *   The field corresponding to this prepared statement value
   * @param columnType
   *   The JDBC SQL Type for this value
   * @param value
   *   The value itself
   * @param setObj
   *   A function that we can delegate to for setObject calls
  ...
  /**
   * This is a utility method to simplify using setObject. It's intended use is to generate a
   * setObject proxy so that the intermediate code doesn't need to be aware of drivers that ignore
   * column types.
   */
  ...
  def save(toSave: A): Boolean = {
    toSave match {
      ...
      case _ =>
        ...
        /**
         * @return
         *   true if there was exactly one row in the result set, false if not.
         */
          ...
          val ret = DB.use(toSave.connectionIdentifier) {
            conn =>
              ...
              val ret = if (saved_?(toSave)) {
                ...
                val ret: Boolean = if (!dirty_?(toSave)) true
                else {
                ...
                val generatedColumns =
                  (mappedColumnInfo.filter(_._2.dbAutogenerated_?).map(_._1)).toList
  ...
  def columnPrimaryKey_?(name: String) =
    mappedColumnInfo.get(name).map(c => (c.dbPrimaryKey_? && c.dbAutogenerated_?)) getOrElse false

  def createInstances(
      dbId: ConnectionIdentifier,
      rs: ResultSet,
      start: Box[Long],
      omax: Box[Long]): List[A] = createInstances(dbId, rs, start, omax, v => Full(v))
  ...
  private def createApplier(name: String, inst: AnyRef /*, clz : Class*/ ): (A, AnyRef) => Unit = {
    ...
    if ((accessor eq null) || accessor == None) {
      ...
      (accessor.get.invoke(this).asInstanceOf[MappedField[AnyRef, A]]).buildSetActualValue(
        accessor.get,
        inst,
        name)
    }
   ...
   *
   * Elements with a class matching the field name are mapped to the NodeSeq produced by the
   * fieldHtml function that is passed in.
   *
   * So, with a MetaMapper that has three fields, name, date, and description, the resulting CSS
   * selector transforms are:
   *
   * {{{
   * Seq(
   ...
   * }}}
   *
   * Above, -name field-, -date field-, and -description field- refer to the actual MappedField
   * objects for those fields.
   */
  ...
  lazy val fieldMatcher: PartialFunction[(A, String), MappedField[Any, A]] = {
    case (actual, fieldName) if _mappedFields.contains(fieldName) =>
      fieldByName[Any](fieldName, actual).openOrThrowException("we know this is defined")
  }
  ...
  private val mappedAppliers =
    new HashMap[(String, Box[Class[(C forSome { type C })]]), (A, AnyRef) => Unit];
    ...
    for (v <- mapperAccessMethods) {
      v.invoke(this) match {
        case untypedMf: MappedField[_, _] if !untypedMf.ignoreField_? =>
          ...
          for (colName <- mf.dbColumnNames(v.getName).map(MapperRules.quoteColumnName.vend).map(
              _.toLowerCase)) {
  ...
  val columnNamesForInsert = (mappedColumnInfo.filter(c =>
    !(c._2.dbPrimaryKey_? && c._2.dbAutogenerated_?)).map(_._1)).toList.mkString(",")

  val columnQueriesForInsert = {
    (mappedColumnInfo.filter(c => !(c._2.dbPrimaryKey_? && c._2.dbAutogenerated_?)).map(p =>
      "?")).toList.mkString(",")
  }
   ...
   * This function converts a header name into the appropriate XHTML format for displaying across
   * the headers of a formatted block. The default is &lt;th&gt; for use in XHTML tables. If you
   * change this function, the change will be used for this MetaMapper unless you override the
   * htmlHeades method
   */
   ...
   * This function converts an element into the appropriate XHTML format for displaying across a
   * line formatted block. The default is &lt;td&gt; for use in XHTML tables. If you change this
   * function, the change will be used for this MetaMapper unless you override the doHtmlLine
   * method.
   */
  ...
  /**
   * This function converts a name and form for a given field in the model to XHTML for presentation
   * in the browser. By default, a table row ( &lt;tr&gt; ) is presented, but you can change the
  ...
  def toHtml(toMap: A): NodeSeq =
    mappedFieldList.map(e => ??(e.method, toMap)).
    filter(f => f.dbDisplay_?).flatMap(field =>
  ...
  /**
   * The name of the database table. Override this method if you want to change the table to
   * something other than the name of the Mapper class
   */
  ...
  private def _beforeValidation(what: A): Unit = {
    setupInstanceForPostCommit(what);
    eachField(what, beforeValidation) { field => field.beforeValidation }
  }
  private def _beforeValidationOnCreate(what: A): Unit = {
    eachField(what, beforeValidationOnCreate) { field => field.beforeValidationOnCreate }
  }
  private def _beforeValidationOnUpdate(what: A): Unit = {
    eachField(what, beforeValidationOnUpdate) { field => field.beforeValidationOnUpdate }
  }
  private def _afterValidation(what: A): Unit = {
    eachField(what, afterValidation) { field => field.afterValidation }
  }
  private def _afterValidationOnCreate(what: A): Unit = {
    eachField(what, afterValidationOnCreate) { field => field.afterValidationOnCreate }
  }
  private def _afterValidationOnUpdate(what: A): Unit = {
    eachField(what, afterValidationOnUpdate) { field => field.afterValidationOnUpdate }
  }

  private def _beforeSave(what: A): Unit = {
    setupInstanceForPostCommit(what); eachField(what, beforeSave) { field => field.beforeSave }
  }
  private def _beforeCreate(what: A): Unit = {
    eachField(what, beforeCreate) { field => field.beforeCreate }
  }
  private def _beforeUpdate(what: A): Unit = {
    eachField(what, beforeUpdate) { field => field.beforeUpdate }
  }

  private def _afterSave(what: A): Unit = {
    eachField(what, afterSave) { field => field.afterSave }
  }
  private def _afterCreate(what: A): Unit = {
    eachField(what, afterCreate) { field => field.afterCreate }
  }
  private def _afterUpdate(what: A): Unit = {
    eachField(what, afterUpdate) { field => field.afterUpdate }
  }

  private def _beforeDelete(what: A): Unit = {
    setupInstanceForPostCommit(what); eachField(what, beforeDelete) { field => field.beforeDelete }
  }
  private def _afterDelete(what: A): Unit = {
...
/**
 * Represents a generic user-specified index on the given columns. The user provides a function to
 * generate the SQL needed to create the index based on the table and columns. Validation is
 * required since this is raw SQL being run on the database server.
 */
final case class GenericIndex[A <: Mapper[A]](
    createFunc: (String, List[String]) => String,
    validated: IHaveValidatedThisSQL,
    columns: List[IndexItem[A]]) extends BaseIndex[A] // (indexColumns : _*)
...
case class BoundedIndexField[A <: Mapper[A]](field: MappedField[String, A], len: Int)
    extends IndexItem[A] {
...
final case class Cmp[O <: Mapper[O], T](
    ...
    dbFunc: Box[String]) extends QueryParam[O]
...
/**
 * Represents a query criterion using a parameterized SQL string. Parameters are substituted in
 * order. For Date/Time types, passing a java.util.Date will result in a Timestamp parameter. If you
 * want a specific SQL Date/Time type, use the corresponding java.sql.Date, java.sql.Time, or
...
object By {
  ...
  def apply[O <: Mapper[O], T, U](field: MappedField[T, O], value: U)(implicit ev: U => T) =
...
object By_>= {
  ...
  def apply[O <: Mapper[O], T, U](
      field: MappedField[T, O],
      value: U)(implicit ev: U => T) = Cmp[O, T](field, >=, Full(value), Empty, Empty)

  def apply[O <: Mapper[O], T](field: MappedField[T, O], otherField: MappedField[T, O]) = Cmp[O, T](
    field,
    >=,
    Empty,
    Full(otherField),
...
object ByRef {
  ...
  def apply[O <: Mapper[O], T](field: MappedField[T, O], otherField: MappedField[T, O]) =
...
trait KeyedMetaMapper[Type, A <: KeyedMapper[Type, A]] extends MetaMapper[A]
  ...
  def findDb(dbId: ConnectionIdentifier, key: Any): Box[A] =
    key match {
      ...
      case prod: Product if (testProdArity(prod)) =>
        findDb(dbId, convertToQPList(prod).toIndexedSeq: _*)
  ...
  def find(key: String): Box[A] =
    dbStringToKey(key) flatMap (realKey => findDbByKey(selectDbForKey(realKey), realKey))
  ...
  /**
   * Override this definition in your model to enable CRUD snippets for that model. Set to false by
   * default.
   *
  ...
  /**
   * Defines the default CRUD snippets. Override if you want to change the names of the snippets.
   * Defaults are "add", "edit", and "view".
   *
  ...
  /**
   * Provides basic transformation of <code>html</code> to a form for the given <code>obj</code>.
   * When the form is submitted, <code>cleanup</code> is run.
   */
  ...
  /**
   * Base add form snippet. Fetches object from <code>addSnippetSetup</code> and invokes
  ...
  /**
   * Base edit form snippet. Fetches object from <code>editSnippetSetup</code> and invokes
  ...
  /**
   * Basic transformation of <code>html</code> to HTML for displaying the object from
   * <code>viewSnippetSetup</code>.
   */
  ...
  /**
   * Lame attempt at automatically getting an object from the HTTP parameters. BROKEN! DO NOT USE!
   * Only here so that existing sub-classes KeyedMetaMapper don't have to implement new methods when
   * I commit the CRUD snippets code.
   */
  ...
  /**
   * Default callback behavior of the edit snippet. Called when the user presses submit. Saves the
   * passed in object.
   *
   * @param obj
  ...
  /**
   * Default callback behavior of the add snippet. Called when the user presses submit. Saves the
   * passed in object.
   *
   * @param obj
   *   mapped object of this metamapper's type
...
class KeyObfuscator {
  ...
  def obscure[
      KeyType,
      MetaType <: KeyedMapper[KeyType, MetaType]](what: KeyedMapper[KeyType, MetaType]): String = {
    obscure(what.getSingleton, what.primaryKeyField.get)
  }

  def apply[KeyType, MetaType <: KeyedMapper[KeyType, MetaType], Q](
      theType: KeyedMetaMapper[KeyType, MetaType],
      key: Q)(implicit ev: Q => KeyType): String = {
    val k: KeyType = key
    obscure(theType, k)
  }
