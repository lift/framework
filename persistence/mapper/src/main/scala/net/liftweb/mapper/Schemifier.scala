...
/**
 * Given a list of MetaMappers, make sure the database has the right schema <ul> <li>Make sure all
 * the tables exists</li> <li>Make sure the columns in the tables are correct</li> <li>Create the
 * indexes</li> <li>Create the foreign keys</li> </ul>
 */
object Schemifier extends Loggable {
  ...
  private def using[RetType <: Any, VarType <: ResultSet](f: => VarType)(f2: VarType => RetType)
      : RetType = {
    val theVar = f
    try {
      f2(theVar)
   ...
   * @param structureOnly
   *   if true, will only check tables and columns, not indexes and constraints. Useful if schema is
   *   maintained outside Lift, but still needs structure to be in sync
   * @param logFunc
   ...
   *
   * @return
   *   The list of statements needed to bring the database in a consistent state. This list is
   *   created even if performWrite=false
   */
  def schemify(
      performWrite: Boolean,
      structureOnly: Boolean,
        ...
        tables.foldLeft(EmptyCollector)((b, t) =>
          ...
          tables.foldLeft(EmptyCollector)((b, t) =>
            b + tableCheck(
              t,
              "ensureColumns",
              ensureColumns(performWrite, logFunc, t, connection, actualTableNames))) +
          (if (structureOnly)
             EmptyCollector
           else
             (tables.foldLeft(EmptyCollector)((b, t) =>
               b + tableCheck(
                 t,
                 "ensureIndexes",
                 ensureIndexes(performWrite, logFunc, t, connection, actualTableNames))) +
               tables.foldLeft(EmptyCollector)((b, t) =>
                 b + tableCheck(
                   t,
                   "ensureConstraints",
  ...
  def destroyTables_!!(
      ...
      cnt: Int,
      logFunc: (=> AnyRef) => Unit,
      stables: List[BaseMetaMapper]): Unit = {
   ...
   *
   * @param performWrite
   *   Whether the SQL command should be executed.
   * @param logFunc
   *   Logger.
   * @param connection
   *   Database connection.
   * @param makeSql
   *   Factory for SQL command.
   *
   * @return
   *   SQL command.
   */
    ...
    table.dbIndexes.foreach {
      index =>
        ...
        val standardCreationStatement =
          (table._dbTableNameLC + "_" + columns.map(_.field._dbColumnNameLC).mkString(
            "_")) + " ON " + table._dbTableNameLC + " ( " + columns.map(_.indexDesc).comma + " )"
  ...
  private def ensureConstraints(
    ...
    val ret = if (connection.supportsForeignKeys_? && MapperRules.createForeignKeys_?(dbId)) {
      table.mappedFields.flatMap { f =>
        f match { case f: BaseMappedField with BaseForeignKey => List(f); case _ => Nil }
      }.toList.flatMap {
        field =>
