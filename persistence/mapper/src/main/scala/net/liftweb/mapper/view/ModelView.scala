...
/**
 * A snippet that can list and edit items of a particular Mapper class This trait can help reduce
 * boilerplate in the common scenario where you want a snippet class to provide list and edit
 * snippets for a specific Mapper class.
 * @author
 *   nafg
 */
...
class ModelView[T <: Mapper[T]](var entity: T, val snippet: ModelSnippet[T]) {
  ...
  /**
   * If Some(string), will redirect to string on a successful save. If None, will load the same
   * page. Defaults to Some("list"). This var is used by ModelSnippet.onSave, which is a
   * ModelView=>Unit
   */
  ...
  /**
   * This function is used as a snippet in the edit view to provide alternate text depending on
   * whether an existing entity is being edited or a new one is being created.
   */
  ...
  /**
   * This method checks whether the entity validates; if so it saves it, and if successful redirects
   * to the location specified by redirectOnSave, if any. If save or validation fails, the
   * appropriate message(s) is/are displayed and no redirect is performed.
   */
  def save(): Unit = {
  ...
  /**
   * returns a string that represents the id, or &lt;new&gt; if the entity is a new entity. If the
   * entity has been saved then the id is determined as follows: If it is a KeyedMapper then it
   * calls toString on the entity's primaryKeyField. Otherwise it calls toString on a field named
   * "id."
   */
   ...
   * `name`. If the field has a Full toForm implementation then that is used; otherwise its asHtml
   * is called.
   */
