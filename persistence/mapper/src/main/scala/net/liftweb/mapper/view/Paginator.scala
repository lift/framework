...
/**
 * Helper for when using paginators with a ModelSnippet. Adds a dispatch that delegates the
 * "paginate" snippet to the paginator member.
 * @author
...
/**
 * Paginate mapper instances by supplying the model you wish to paginate and Paginator will run your
 * query for you etc.
 *
 * @param meta
...
class MapperPaginator[T <: Mapper[T]](val meta: MetaMapper[T]) extends Paginator[T] {
  ...
  def page: Seq[T] =
    meta.findAll(constantParams ++ Seq[QueryParam[T]](MaxRows(itemsPerPage), StartAt(first)): _*)
}
...
class SortedMapperPaginator[T <: Mapper[T]](
  ...
  override def page: Seq[T] = meta.findAll(constantParams ++ Seq[QueryParam[T]](
    mapperSort,
