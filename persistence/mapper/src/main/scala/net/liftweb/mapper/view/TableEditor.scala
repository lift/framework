...
/**
 * Keeps track of pending adds to and removes from a list of mappers. Supports in-memory sorting by
 * a field. Usage: override metaMapper with a MetaMapper instance, call sortBy to specify the field
 * to sort by. If it is already sorted by that field it will sort descending, otherwise ascending.
...
trait ItemsList[T <: Mapper[T]] {
  ...
  /**
   * The MetaMapper that provides create and findAll functionality etc. Must itself be a T (the
   * mapper type it represents)
   */
  ...
  /**
   * Returns the items (current + added - removed), sorted. Sorting sorts strings case-insensitive,
   * as well as Ordered and java.lang.Comparable. Anything else where both values are nonnull are
   * sorted via their toString method (case sensitive)
   */
  def items: Seq[T] = {
          ...
          (a, b) =>
            ((field.actualField(a).get: Any, field.actualField(b).get: Any) match {
              ...
              case (aval: java.lang.Comparable[_], bval: java.lang.Comparable[_]) =>
                (aval.asInstanceOf[java.lang.Comparable[Any]] compareTo bval.asInstanceOf[
...
/**
 * Holds a registry of TableEditor delegates Call TableEditor.registerTable(name_to_use_in_view,
 * meta_mapper_for_the_table, display_title) in Boot after DB.defineConnectionManager. Referencing
 * TableEditor triggers registering its snippet package and enabling the provided template,
 * /tableeditor/default.
 * @author
 *   nafg
 */
...
package snippet {
  ...
  /**
   * This is the snippet that the view references. It requires the following contents: table:title -
   * the title registered in Boot header:fields - repeated for every field of the MetaMapper, for
   * the header. field:name - the displayName of the field, capified. Links to sort by the field.
   * table:items - repeated for each record item:fields - repeated for each field of the current
   * record field:form - the result of toForm on the field item:removeBtn - a button to remove the
   * current item table:insertBtn - a button to insert another item For a default layout, use
   * lift:embed what="/tableeditor/default", with
   * @author
   *   nafg
   */
...
/**
 * This class does the actual view binding against a ItemsList. The implementation is in the base
 * trait ItemsListEditor
 * @author
 *   nafg
 */
protected class TableEditorImpl[T <: Mapper[T]](val title: String, meta: T with MetaMapper[T])
