...
/**
 * Systems under specification for Mapper. The model classes here are defined in
 * MapperSpecsModel.scala
 */
  ...
  providers.foreach(provider => {
    try {
      ...
      ("Mapper for " + provider.name) should {
        ...
        "schemify" in {
          val elwood =
            SampleModel.find(By(SampleModel.firstName, "Elwood")).openOrThrowException("Test")
          val madeline =
            SampleModel.find(By(SampleModel.firstName, "Madeline")).openOrThrowException("Test")
          val archer =
            SampleModel.find(By(SampleModel.firstName, "Archer")).openOrThrowException("Test")
          val notNull =
            SampleModel.find(By(SampleModel.firstName, "NotNull")).openOrThrowException("Test")
        ...
        "enforce NOT NULL" in {
          ...
          SampleModel.create.firstName(
            "Not Null").notNull(nullString).save must throwA[java.sql.SQLException]
        }
        ...
        "Precache works with OrderBy" in {
          if ((provider ne DbProviders.DerbyProvider)
            ...
            val dogs =
              Dog.findAll(By(Dog.name, "fido"), OrderBy(Dog.name, Ascending), PreCache(Dog.owner))
        ...
        "we can read and write String primary keys" in {
          ...
          Thing.find(By(
            Thing.thing_id,
            i1.thing_id.get)).openOrThrowException("Test").name.get must_== "frog"
          Thing.find(By(
            Thing.thing_id,
            i2.thing_id.get)).openOrThrowException("Test").name.get must_== "dog"
        }
        ...
        "Save flag results in update rather than insert" in {
          ...
          val frog =
            SampleModel.find(By(SampleModel.firstName, "Frog")).openOrThrowException("Test")
