...
object SpecCrudType {
  ...
  def DEFAULT_REPO_CONTENT =
    (0 until 100).map(n => n.toString -> new SpecCrudType(n.toString, s"Line number $n"))
 ...
 *
 * @param fieldName
 *   fake date field na,e
 * @param getter
 *   return `fieldName` value as [[String]] from [[T]] instance
 * @param setter
 *   convert given [[String]] and it as set `fieldName` value in [[T]] instance
 * @tparam T
 *   target fake data holder type ([[SpecCrudType]] for now)
 */
case class SpecField[T](fieldName: String, getter: T => String, setter: (T, String) => Unit)
   ...
   *
   * @return
   *   Element with represents field name in HTML
   */
...
/**
 * Helper class for creating fake data repository for using as
 * [[net.liftweb.proto.Crudify.CrudBridge]] and for others methods needed by
 * [[net.liftweb.proto.Crudify]] implementation
 *
 * @param initialContent
 *   initial content for repor
 */
class SpecCrudRepo(initialContent: (String, SpecCrudType)*) {
   ...
   * @param count
   *   maximal returned items count
   * @return
   *   Repo items starting form `start` and truncated to `count` size
   */
...
trait SpecCrudify extends Crudify {
  ...
  override protected implicit def buildFieldBridge(from: FieldPointerType): FieldPointerBridge =
    new FieldPointerBridge {
      override def displayHtml: NodeSeq = from.displayHtml
    }

  override protected def computeFieldFromPointer(
...
object RequestContext {
   ...
   *   HTTP request params
   * @return
   *   Test HTTP request filled with params`
   */
  ...
  /**
   * Call `function` inside `Lift` context produced from `request` Make functions like `S ?` or
   * `S.param` works during call of `function`
   *
   * @param request
   *   HTTP request filled with needed params
   * @param function
   *   target function to execute in context of given `Lift` request
   * @tparam T
  ...
  /**
   * Call `function` inside session based `Lift` context produced from `request` Make functions like
   * `S.fmapFunc` or `SHtml.onSubmitUnit` works during call of `function`
   *
   * @param request
   *   HTTP request filled with needed params
   * @param function
