...
class TestRunner(
    ...
    afterTestListeners: List[(String, Boolean, Box[Throwable], List[StackTraceElement]) => Any]) {
  ...
  def setup[T](what: List[Item]): (() => TestResults, (String, () => T) => T) = {
    ...
    def afterTest(
        ...
        success: Boolean,
        excp: Box[Throwable],
        trace: List[StackTraceElement]): Unit = {
        ...
        val (success, trace, excp) =
          try {
            ...
            case e: Throwable =>
              def combineStack(
                  ex: Throwable,
                  base: List[StackTraceElement]): List[StackTraceElement] = ex match {
                case null => base
