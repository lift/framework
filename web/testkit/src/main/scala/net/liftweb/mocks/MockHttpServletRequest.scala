...
/**
 * A Mock ServletRequest. Change its state to create the request you are interested in. At the very
 * least, you will need to change method and path.
 *
 * There are several things that aren't supported:
 *
 * <ul> <li>getRequestDispatcher - returns null always</li> <li>getRequestedSessionId - always
 * returns null. The related isRequestedSessionId... methods similarly all return false</li>
 ...
 */
class MockHttpServletRequest(val url: String = null, var contextPath: String = "")
   ...
   *
   * Defaults to UTF-8. Note that this differs from the default encoding per the HTTP spec
   * (ISO-8859-1), so you will need to change this if you need something other than UTF-8.
   */
  ...
  /**
   * The port that this request was received on. You should probably change serverPort as well if
   * you change this.
   */
  var localPort = 80
  ...
  /**
   * The preferred locales for the client, in decreasing order of preference. If not set, the
  ...
  /**
   * The query parameters for the request. There are two main ways to set this List, either by
   * modifying the parameters var directly, or by assigning to queryString, which will parse the
   * provided string into GET parameters.
   */
      ...
      q.split('&').foreach {
        pair =>
          pair.split('=') match {
            ...
            case Array("") =>
              throw new IllegalArgumentException("Invalid query string: \"" + q + "\"")
            case Array(key) => {
              // Append to the current key's value
              newParams += key -> ""
            }
  ...
  /**
   * Indicates whether the request is being handled by a secure protocol (e.g. HTTPS). If you set
   * the scheme to https you should set this to true.
   */
  var secure = false
  ...
  /**
   * The port that this request was received on. You should probably change localPort as well if you
   * change this.
   */
  var serverPort = 80
  ...
  /**
   * Set fields based on the given url string. If the url begins with "http" it is assumed to be a
   * full URL, and is processed with processUrl(URL). If the url begins with "/" then it's assumed
   * to be only the path and query string.
   *
   * @param url
   *   The URL to extract from
   */
   ...
   *
   * <ol> <li>The host portion is used to set localAddr, localHost and serverName. You will need to
   * manually set these if you want different behavior.</li> <li>The userinfo field isn't processed.
   * If you want to mock BASIC authentication, use the addBasicAuth method</li> </ol>
   *
   * @param url
   *   The URL to extract from
   * @param contextPath
  ...
  def computeRealPath(path: String) = {
    if (!path.startsWith(contextPath)) {
      throw new IllegalArgumentException(
        "Path \"%s\" doesn't begin with context path \"%s\"!".format(path, contextPath))
    }
...
//    asMap(newMap.map{case (k,v) => (k,v.toArray)}.asInstanceOf[Map[Object,Object]])
  ...
  def getDateHeader(h: String): Long = {
    val handler: PartialFunction[Throwable, Box[Long]] = {
      case pe: ParseException => {
        throw new IllegalArgumentException("Could not parse the date for %s : \"%s\"".format(
          h,
          getHeader(h)))
        Empty
      }
  ...
  /**
   * A utility method to set the given header to an RFC1123 date based on the given long value
   * (epoch seconds).
   */
  ...
  def startAsync(
      ...
      response: jakarta.servlet.ServletResponse): AsyncContext = null
