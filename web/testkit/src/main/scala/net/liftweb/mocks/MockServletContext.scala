 ...
 *
 * def testLiftCore = { val output = new ByteArrayOutputStream val outputStream = new
 * MockServletOutputStream(output) val writer = new PrintWriter(outputStream)
 *
 * val req = new MockHttpServletRequest req.method = "GET" req.path = "/" val res = new
 * MockHttpServletResponse(writer, outputStream)
 *
 * val filter = new LiftFilter filter.init(new MockFilterConfig(new
 * MockServletContext("target/test1-1.0-SNAPSHOT"))) filter.doFilter(req, res,new
 * DoNothingFilterChain) assertTrue(output.toString.startsWith("<?xml")) }
 */
...
class MockServletContext(var target: String) extends ServletContext {
  def addJspFile(
      servletName: String,
      jspFile: String): jakarta.servlet.ServletRegistration.Dynamic = null
  ...
  def addFilter(x$1: String, x$2: Class[_ <: jakarta.servlet.Filter]): FilterRegistration.Dynamic =
    null
  ...
  def addServlet(
      servletNAme: String,
      servletClass: Class[_ <: jakarta.servlet.Servlet]): ServletRegistration.Dynamic = null
  def addServlet(
      servletName: String,
      servlet: jakarta.servlet.Servlet): ServletRegistration.Dynamic = null
  ...
  def getFilterRegistrations(): java.util.Map[String, _ <: jakarta.servlet.FilterRegistration] =
    null
  ...
  def getServletRegistrations(): java.util.Map[String, _ <: jakarta.servlet.ServletRegistration] =
    null
  ...
  def setSessionTrackingModes(trackingModes: java.util.Set[jakarta.servlet.SessionTrackingMode])
      : Unit = ()
