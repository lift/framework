...
object Comet extends DispatchSnippet with LazyLoggable {
  ...
  /**
   * Given a comet actor and the HTML contents for that actor, renders it into a container and
   * returns the results. Waits up to the timeout specified by the comet actor's
   * `cometRenderTimeout` property and then fails if the actor has not yet properly rendered.
   */
   ...
   *   - You can use a query parameter using `metaname`; e.g., for a url like `foo?id=122`, your
   *     comet could take the name "122" if you use: `metaname=id`
  ...
  private def buildComet(cometHtml: NodeSeq): NodeSeq = {
    ...
    try {
      theType match {
        case Full(cometType) =>
          S.findOrCreateComet(cometType, cometName, cometHtml, S.attrsFlattenToMap, true).map {
            foundComet =>
...
object NoCometTypeException
    extends CometFailureException("Comets with no type are no longer supported as of Lift 3.") {
