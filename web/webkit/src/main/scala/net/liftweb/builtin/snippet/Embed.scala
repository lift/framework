...
/**
 * This object implements the logic for the &lt;lift:embed&gt; tag. It supports retrieving a
 * template based on the "what" attribute, and any &lt;lift:bind-at&gt; tags contained in the embed
 * tag will be used to replace &lt;lift:bind&gt; tags within the embedded template.
 */
object Embed extends DispatchSnippet {
  ...
  def render(kids: NodeSeq): NodeSeq = {
    for {
      ...
      templateOpt <-
        ctx.findTemplate(what.text) ?~ ("FIX" + "ME trying to embed a template named '" + what + "', but the template was not found. ")
    } yield {
