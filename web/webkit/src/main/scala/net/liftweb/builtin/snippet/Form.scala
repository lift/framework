...
/**
 * This object is the default handler for the &lt;lift:form&gt; tag, which is used to perform AJAX
 * submission of form contents. If the "onsubmit" attribute is set on this tag, then the contents
 * there will be run prior to the actual AJAX call. If a "postsubmit" attribute is present on the
 * tag, then its contents will be executed after successful submission of the form.
 */
object Form extends DispatchSnippet {
  ...
  /**
   * Add the post method and postback (current URL) as action. If the multipart attribute is
   * specified, set the enctype as "multipart/form-data"
   */
