...
/**
 * <p>This built-in snippet can be used to render a menu representing your SiteMap. There are three
 * main snippet methods that you can use:</p>
 *
 * <ul> <li>builder - Renders the entire SiteMap, optionally expanding all child menus</li>
 * <li>group - Renders the MenuItems corresponding to the specified group.</li> <li>item - Renders
 * the specific named MenuItem</li> </ul>
 *
...
object Menu extends DispatchSnippet {
  ...
  /**
   * <p>This snippet method renders a menu representing your SiteMap contents. The menu is rendered
   * as a set of nested unordered lists (&lt;ul /&gt;). By default, it only renders nested child
   * menus for menus that match the current request path. You can add the "expandAll" attribute to
   * the snippet tag to force full expansion of all child menus. Additionally, you can use the
   * following attribute prefixes to further customize the generated list and list item
   * elements:</p>
   *
   * <ul> <li>top - Adds the specified attribute to the top-level &lt;ul&gt; element that makes up
   * the menu</li> <li>ul - Adds the specified attribute to each &lt;ul&gt; element (top-level and
   * nested children) that makes up the menu</li> <li>li - Adds the specified attribute to each
   * &lt;li&gt; element for the menu</li> <li>li_item - Adds the specified attribute to the current
   * page’s menu item</li> <li>outer_tag - the tag for the outer XML element (ul by default)</li>
   * <li>inner_tag - the tag for the inner XML element (li by default)</li> <li>li_path - Adds the
   * specified attribute to the current page’s breadcrumb path. The breadcrumb path is the set of
   * menu items leading to this one.</li> <li>linkToSelf - False by default, but available as 'true'
   * to generate link to the current page</li> <li>level - Controls the level of menus that will be
   * output. "0" is the top-level menu, "1" is children of the current menu item, and so on. Child
   * menus will be expanded unless the "expand" attribute is set to <pre>false</pre>.</li>
   * <li>expand - Controls whether or not to expand child menus. Defaults to <pre>true</pre>.</li>
   * </ul>
   *
   * <p>If you are using designer friendly invocation, you can access the namespaced attributes:
   * <br/> &lt;div class="lift:Menu?li_item:class=foo+bar"&gt;menu&lt;/div&gt; </p>
   *
   ...
   *
   * <p>Customizing the elements is handled through the prefixed attributes described above. For
   * instance, you could make the current page menu item red:</p>
   *
  ...
  private def renderWhat(expandAll: Boolean): Seq[MenuItem] =
    (if (expandAll)
       for { sm <- LiftRules.siteMap; req <- S.request } yield sm.buildMenu(req.location).lines
  ...
  /**
   * <p>Renders the title for the current request path (location). You can use this to automatically
   * set the title for your page based on your SiteMap:</p>
   *
   * <pre> ⋮ &lt;head&gt; &lt;title&gt;&lt;lift:Menu.title /&gt;&lt;/title&gt; &lt;/head&gt; ⋮
   * </pre> <p>HTML5 does not support tags inside the &lt;title&gt; tag, so you must do: </p>
   *
   * <pre> &lt;head&gt; &lt;title class=&quot;lift:Menu.title&quote;&gt;The page named %*% is being
   * displayed&lt;/title&gt; &lt;/head&gt; </pre> <p> And Lift will substitute the title at the %*%
   * marker if the marker exists, otherwise append the title to the contents of the &lt;title&gt;
   * tag. </p>
   */
  ...
  /**
   * Renders a group of menu items. You specify a group using the LocGroup LocItem case class on
   * your Menu Loc:
   *
   * {{{
   * val menus =
   ...
   * }}}
   *
   * By default, menu items bind the href and text of an `a` element in the template, and iterates
   * over `li` elements. You can customize these settings using the `repeatedSelector`,
   * `linkSelector`, and `hrefSelector` parameters; for example: .
   * {{{
   ...
   * }}}
   *
   * These selectors are CSS selector transform selectors. `repeatedSelector` and `linkSelector` are
   * the left-hand-side, while `hrefSelector` is the second part, which indicates what will be
   * replaced by the href text. For example, the above would roughly yield a transform that looks
   * like:
   *
   * {{{
   * "p" #> {
   ...
   * Menu(Loc("b",...,...,LocGroup("test"))) :: Menu(Loc("c",...,...,LocGroup("test"))) :: Nil
   * </pre>
   *
   ...
   *
   * <p>The menu item is rendered as an anchor tag (&lta /&gt;). The text for the link defaults to
   * the named Menu's Loc.linkText, but you can specify your own link text by providing contents to
   * the tag:</p>
   *
   * <pre> &lt;lift:Menu.item name="b"&gt;This is a link&lt;/lift:Menu.item&gt; </pre>
   *
   * <p>Additionally you can customize the tag using attributes prefixed with "a":</p>
   *
   * <pre> &lt;lift:Menu.item name="b" a:style="color: red;" /&gt; </pre>
   *
   * <p>The param attribute may be used with Menu Locs that are CovertableLoc to parameterize the
   * link</p>
   *
   * <p>Normally, the Menu item is not shown on pages that match its Menu's Loc. You can set the
   * "donthide" attribute on the tag to force it to show text only (same text as normal, but not in
   * an anchor tag)</p>
   *
   * <p>Alternatively, you can set the "linkToSelf" attribute to "true" to force a link. You can
   * specify your own link text with the tag's contents. Note that <b>case is significant</b>, so
