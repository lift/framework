...
/**
 * This built in snippet renders messages (Errors, Warnings, Notices) in a <i>div</i>. Typically it
 * is used in templates as a place holder for any messages that are <b>not</b> associated with an
 * ID. Setting the attribute <i>showAll</i> to <i>true</i> will render all messages, with and
 * without an ID. This will lead to duplicate messages if additionally the <i>Msg</i> built in
 * snippet is used to show messages associated with an ID.
 *
 * E.g. (child nodes are optional) <pre name="code" class="xml"> &lt;lift:Msgs showAll="false"&gt;
 ...
 * &lt;lift:notice_class&gt;noticeBox&lt;/lift:notice_class&gt; &lt;/lift:snippet&gt; </pre>
 *
 * JavaScript fadeout and effects for the three types of notices (Errors, Warnings and Notices) can
 * be configured via LiftRules.noticesAutoFadeOut and LiftRules.noticesEffects. Notices for
 * individual elements based on id can be rendered using the &lt;lift:msg/> tag.
 *
 * @see
...
object Msgs extends DispatchSnippet {
  ...
  /**
   * This method renders the current notices to XHtml based on the current user-specific formatting
   * from the &lt;lift:Msgs> tag.
   */
  ...
  /**
   * This method wraps the JavaScript fade and effect scripts into lift's page script that runs
   * onLoad.
   */
  ...
  /**
   * This method produces appropriate JavaScript to fade out the given notice type. The caller must
   * provide a default value for cases where fadeout is not configured, as well as a wrapping
  ...
  /**
   * This method produces and appends a script element to lift's page script to fade out the given
   * notice type.
   *
   * @see
   *   net.liftweb.http.LiftRules.noticesAutoFadeOut
  ...
  /**
   * This method produces appropriate JavaScript to apply effects to the given notice type. The
   * caller must provide a default value for cases where effects are not configured, as well as a
   * wrapping function to transform the output.
   *
   * @see
   *   net.liftweb.http.LiftRules.noticesEffects
   */
  ...
  /**
   * This method produces and appends a script element to lift's page script to apply effects to the
   * given notice type.
   *
   * @see
   *   net.liftweb.http.LiftRules.noticesEffects
...
/**
 * This SessionVar holds formatting data for notice notices so that the AJAX and static notice
 * renderers use the same formatting.
 */
...
/**
 * This SessionVar holds formatting data for warning notices so that the AJAX and static notice
 * renderers use the same formatting.
 */
...
/**
 * This SessionVar holds formatting data for error notices so that the AJAX and static notice
 * renderers use the same formatting.
 */
...
/**
 * This case class is used to hold formatting data for the notice groups so that AJAX and static
 * notices render consistently.
 */
