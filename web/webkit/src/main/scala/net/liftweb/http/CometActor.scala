...
trait StatefulComet extends CometActor {
  ...
  /**
   * If there's some ThreadLocal variable that needs to be set up before processing the state
   * deltas, set it up here.
   */
...
/**
 * This is a message class for use with ListenerManager and CometListener instances. The use of the
 * shouldUpdate function is deprecated, and should instead be handled by the message processing
 * partial functions on the CometListener instances themselves.
 *
 * @see
 *   CometListener
 * @see
...
/**
 * This trait manages a set of Actors in a publish/subscribe pattern. When you extend your Actor
 * with this trait, you automatically get handling for sending messages out to all subscribed
 * Actors. Simply override the high-, medium-, or lowPriority handlers to do your message
 * processing. When you want to update all subscribers, just call the updateListeners method. The
 * createUpdate method is used to generate the message that you want sent to all subscribers.
 *
 ...
 *
 * object Ticker extends ListenerManager with LiftActor { import net.liftweb.util.ActorPing
 *
 ...
 *
 * <pre name="code" class="scala"> class CometClock extends CometListener { val registerWith =
 * Ticker
 *
 * ... handling code ... } </pre>
 *
...
trait ListenerManager {
  ...
  /**
   * Called after RemoveAListener-message is processed and no more listeners exist. Default does
   * nothing.
   */
  ...
  /**
   * Send a message we create to all of the listeners. Note that with this invocation the
   * createUpdate method is not used.
   */
  ...
  /**
   * This method is called when the <pre>updateListeners()</pre> method needs a message to send to
   * subscribed Actors. In particular, createUpdate is used to create the first message that a newly
   * subscribed CometListener will receive.
   */
  protected def createUpdate: Any

  /**
   * Override this method to process high priority messages. Note: <b>you must not process messages
   * with a wildcard (match all)</b>, since this will intercept the messages used for subscription
   * control.
   */
  protected def highPriority: PartialFunction[Any, Unit] = Map.empty

  /**
   * Override this method to process medium priority messages. See the highPriority method for an
   * important note on wildcard processing.
   *
   * @see
   *   #highPriority
   */
...
/**
 * A LiftActorJ with ListenerManager. Subclass this class to get a Java-usable LiftActorJ with
 * ListenerManager
 */
...
/**
 * This trait adds functionality to automatically register with a given Actor using AddAListener and
 * RemoveAListener control messages. The most typical usage would be to register with an instance of
 * ListenerManager. You will need to provide a def/val for the <pre>registerWith</pre> member to
 * control which Actor to connect to.
 *
...
trait CometListener extends BaseCometActor {
  ...
  /**
   * This controls which Actor to register with for updates. Typically this will be an instance of
   * ListenerActor, although you can provide your own subscription handling on top of any
   * SimpleActor.
   */
...
trait LiftCometActor extends TypedActor[Any, Any] with ForwardableActor[Any, Any] with Dependent {
  ...
  /**
   * Override in sub-class to customise timeout for AJAX-requests to the comet-component for the
   * specific comet
   */
   ...
   * {@link LiftRules#cometProcessingTimeout} occurs one may override this to send a message to the
   * user informing of the timeout. <p/><p/> Do NOT manipulate actor-state here. If you want to
   * manipulate state, send the actor a new message. <p/><p/> Typical example would be: <pre>
   * override def cometTimeoutHandler(): JsCmd = { Alert("Timeout processing comet-request, timeout
   * is: " + cometProcessingTimeout + "ms") } </pre>
   */
  def cometProcessingTimeoutHandler(): JsCmd = Noop

  /**
   * This is to react to comet-actors timing out while initial rendering, calls to render(). When
   * the timeout specified in {@link LiftRules#cometRenderTimeout} occurs one may override this to
   * customise the output. <p/><p/> Do NOT manipulate actor-state here. If you want to manipulate
   * state, send the actor a new message. <p/><p/> Typical example would be: <pre> override def
   * renderTimeoutHandler(): Box[NodeSeq] = { Full(&lt;div&gt;Comet {this.getClass} timed out,
   * timeout is {cometRenderTimeout}ms&lt;/div&gt;) } </pre>
   */
  ...
  /**
   * Poke the CometActor and cause it to do a partial update Noop which will have the effect of
   * causing the component to redisplay any Wiring elements on the component. This method is
   * Actor-safe and may be called from any thread, not just the Actor's message handler thread.
   */
  def poke(): Unit = {}

  /**
   * Is this CometActor going to capture the initial Req object? If yes, override this method and
   * return true and override captureInitialReq to capture the Req. Why have to explicitly ask for
   * the Req? In order to send Req instances across threads, the Req objects must be snapshotted
   * which is the process of reading the POST or PUT body from the HTTP request stream. We don't
   * want to do this unless we have to, so by default the Req is not snapshotted/sent. But if you
...
trait BaseCometActor extends LiftActor with LiftCometActor with CssBindImplicits {
  ...
  /**
   * If this method returns true, this CometActor will be treated as a source of continuous partial
   * updates. Usually the basic assumption of a Lift Comet Actor is that render will display the
   * current state of the actor. Partial updates may modify that display, but it is always possible
   * to recreate the complete display by calling render again (in which case pending partial updates
   * can be safely discarded). If this method returns true, render will not be called, but partial
   * updates are guaranteed to always be sent to the client. This is useful as a means to push
   * arbitrary JavaScript to the client without a distinct UI for the comet actor itself.
   */
  ...
  /**
   * The lifespan of this component. By default CometActors will last for the entire session that
   * they were created in, even if the CometActor is not currently visible. You can set the lifespan
   * of the CometActor. If the CometActor isn't visible on any page for some period after its
   * lifespan the CometActor will be shut down.
   */
  ...
  /**
   * Return the list of ListenerIds of all long poll agents that are waiting for this CometActor to
   * change its state. This method is useful for detecting presence.
   */
  protected def cometListeners: List[ListenerId] = listeners.map(_._1)

  /**
   * This method will be called when there's a change in the long poll listeners. The method does
   * nothing, but allows you to get a granular sense of how many browsers care about this
   * CometActor. Note that this method should not block for any material time and if there's any
   * processing to do, use Scheduler.schedule or send a message to this CometActor. Do not change
   * the Actor's state from this method.
   */
  ...
  /**
   * Override this method to deal with JSON sent from the browser via the sendJson function. This is
   * based on the Lift JSON package rather than the handleJson stuff based on the older
   * util.JsonParser. This is the preferred mechanism. If you use the jsonSend call, you will get a
   * JObject(JField("command", cmd), JField("param", params))
   */
  def receiveJson: PartialFunction[JsonAST.JValue, JsCmd] = Map()

  /**
   * The JavaScript call that you use to send the data to the server. For example: &lt;button
   * onclick={jsonSend("Hello", JsRaw("Dude".encJs))}&gt;Click&lt;/button&gt;
   */
  def jsonSend: JsonCall = _sendJson

  /**
   * The call that packages up the JSON and tosses it to the server. If you set autoIncludeJsonCode
   * to true, then this will be included in the stuff sent to the server.
   */
   ...
   * This block of HTML is useful for the editor part of a Comet-based control where the data is
   * JSON and updated with partialUpdates.
   */
  ...
  /**
   * We have to cache fixedRender and only change it if the template changes or we get a
   * reRender(true)
   */
  ...
  /**
   * By default, we do not cache the value of fixedRender. If it's expensive to recompute it each
   * time there's a conversion of something to a RenderOut, override this method if you want to
   * cache fixedRender.
   */
  protected def cacheFixedRender = false

  /**
   * A helpful implicit conversion that takes a NodeSeq => NodeSeq (for example a CssSel) and
   * converts it to a Box[NodeSeq] by applying the function to defaultHtml
   */
  protected implicit def nodeSeqFuncToBoxNodeSeq(f: NodeSeq => NodeSeq): Box[NodeSeq] =
    Full(f(defaultHtml))

  /**
   * By default, `CometActor` handles `RedirectShortcutException`, which is used to handle many
   * types of redirects in Lift. If you override this `PartialFunction` to do your own exception
   * handling and want redirects from e.g. `S.redirectTo` to continue working correctly, make sure
   * you chain back to this implementation.
   */
   ...
   *
   * There are implicit conversions for a bunch of stuff to RenderOut (including NodeSeq). Thus, if
   * you don't declare the return turn to be something other than RenderOut and return something
   * that's coercible into RenderOut, the compiler "does the right thing"(tm) for you. <br/> There
   * are implicit conversions for NodeSeq, so you can return a pile of XML right here. There's an
   * implicit conversion for NodeSeq => NodeSeq, so you can return a function (e.g., a CssBindFunc)
   * that will convert the defaultHtml to the correct output. There's an implicit conversion from
   * JsCmd, so you can return a pile of JavaScript that'll be shipped to the browser.<br/> Note that
   * the render method will be called each time a new browser tab is opened to the comet component
   * or the comet component is otherwise accessed during a full page load (this is true if a
   * partialUpdate has occurred.) You may want to look at the fixedRender method which is only
   * called once and sets up a stable rendering state.
   */
  def render: RenderOut

  /**
   * Cause the entire component to be reRendered and pushed out to any listeners. This method will
   * cause the entire component to be rendered which can result in a huge blob of JavaScript to be
   * sent to the client. It's a much better practice to use partialUpdate for non-trivial CometActor
   * components.
   *
   * @param sendAll
  ...
  /**
   * Clear the common dependencies for Wiring. This method will clearPostPageJavaScriptForThisPage()
   * and unregisterFromAllDependencies(). The combination will result in a clean slate for Wiring
   * during a redraw. You can change the behavior of the wiring dependency management by overriding
   * this method
   */
  ...
  /**
   * By default, Lift deals with managing wiring dependencies. This means on each full render (a
   * full render will happen on reRender() or on a page load if there have been partial updates.)
   * You may want to manually deal with wiring dependencies. If you do, override this method and
   * return true
   */
  ...
  /**
   * Perform a partial update of the comet component based on the JsCmd. This means that the JsCmd
   * will be sent to all of the currently listening browser tabs. This is the preferred method over
   * reRender to update the component
   */
  ...
  /**
   * This method will be called after the Actor has started. Do any setup here. DO NOT do
   * initialization in the constructor or in initCometActor... do it here.
   */
  protected def localSetup(): Unit = {}

  /**
   * Comet Actors live outside the HTTP request/response cycle. However, it may be useful to know
   * what Request led to the creation of the CometActor. You can override this method and capture
   * the initial Req object. Note that keeping a reference to the Req may lead to memory retention
   * issues if the Req contains large message bodies, etc. It's optimal to capture the path or
   * capture any request parameters that you care about rather the keeping the whole Req reference.
   */
  ...
  /**
   * Ask another CometActor a question. That other CometActor will take over the screen real estate
  ...
  /**
   * Convert a NodeSeq => NodeSeq to a RenderOut. The render method returns a RenderOut. This method
   * implicitly (in Scala) or explicitly (in Java) will convert a NodeSeq => NodeSeq to a RenderOut.
   * This is helpful if you use Lift's CSS Selector Transforms to define rendering.
   */
  protected implicit def nsToNsFuncToRenderOut(f: NodeSeq => NodeSeq): RenderOut = {
    val additionalJs =
  ...
  /**
   * Convert a Seq[Node] (the superclass of NodeSeq) to a RenderOut. The render method returns a
   * RenderOut. This method implicitly (in Scala) or explicitly (in Java) will convert a NodeSeq to
   * a RenderOut. This is helpful if you return a NodeSeq from your render method.
   */
...
private[http] class XmlOrJsCmd(
  ...
  val xml = _xml.flatMap(content =>
    S.session.map(s => s.processSurroundAndInclude("JS SetHTML id: " + id, content)))
  ...
  def toJavaScript(session: LiftSession, displayAll: Boolean): JsCmd = {
    val updateJs =
      (if (ignoreHtmlOnJs) Empty else xml, javaScript, displayAll) match {
        ...
        case (Full(xml), Full(js), true) => LiftRules.jsArtifacts.setHtml(
            id + "_outer",
            (
              spanFunc(Helpers.stripHead(xml)) ++ fixedXhtml.openOr(Text("")))) & JsCmds.JsTry(
            js,
            false)
        case (Full(xml), _, true) => LiftRules.jsArtifacts.setHtml(
            id + "_outer",
            (
    ...
    var ret: JsCmd = JsCmds.JsTry(JsCmds.Run("destroy_" + id + "();"), false) &
      ...
      JsCmds.JsTry(
        JsCmds.Run(
          "destroy_" + id + " = function() {" + (destroy.openOr(JsCmds.Noop).toJsCmd) + "};"),
        false)
  ...
  def outSpan: NodeSeq = {
    S.appendGlobalJs(
      Run("var destroy_" + id + " = function() {" + (destroy.openOr(JsCmds.Noop).toJsCmd) + "}"))
...
case class AnswerRender(response: XmlOrJsCmd, who: LiftCometActor, when: Long, displayAll: Boolean)
...
case class AskQuestion(
    what: Any,
    who: LiftCometActor,
    listeners: List[(ListenerId, AnswerRender => Unit)]) extends CometMessage

case class AnswerQuestion(what: Any, listeners: List[(ListenerId, AnswerRender => Unit)])
    extends CometMessage
...
/**
 * The RenderOut case class contains the rendering for the CometActor. Because of the implicit
 * conversions, RenderOut can come from <br/>
 * @param xhtml
 ...
 * @param destroyScript
 *   is executed when the comet widget is redrawn ( e.g., if you register drag or mouse-over or some
 *   events, you unregister them here so the page doesn't leak resources.)
 * @param ignoreHtmlOnJs
 *   -- if the reason for sending the render is a Comet update, ignore the xhtml part and just run
 *   the JS commands. This is useful in IE when you need to redraw the stuff inside
 *   <table><tr><td>... just doing innerHtml on <tr> is broken in IE
 */
