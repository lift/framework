...
trait ContentParser {
  ...
  /**
   * Given an InputStream to a resource with a matching suffix, this function should provide the
   * corresponding NodeSeq ready to serve
   * @return
   */
...
object ContentParser {
  ...
  /**
   * Convenience function to convert a simple `String => Box[NodeSeq]` content parser function into
   * a `InputStream => Box[NodeSeq]` function for satisfying the `ContentParser` contract.
   * @param simpleParser
  ...
  /**
   * Default surround function used by `ContentParser.basic` and the built-it markdown parser which
   * results in the template being surrounded by `default.html` with the content located at
   * `id=content`.
   */
  ...
  /**
   * A basic `ContentParser` which handles one template filename suffix, operates on a string, and
   ...
   *   the function for surrounding the content returned by the `parseFunction`. See
   *   [[defaultSurround]].
   */
  def apply(
  ...
  /**
   * A fully-specified `ContentParser` which handles multiple filename suffixes, operates on an
   * `InputStream`, and surrounds the top-level templates with the default surround
   */
  def apply(
