...
/**
 * Represents an HTML attribute for an event handler. Carries the event name and the JS that should
 * run when that event is triggered as a String.
 */
private case class EventAttribute(eventName: String, jsString: String)
private object EventAttribute {

  /**
   * Some elements allow a URL attribute to take a `javascript:(//)`-style URL instead of setting an
   * `on*` event in order to invoke JS. For example, you can (and Lift does) set a form's `action`
   * attribute to `javascript://(some JS)` instead of setting `onsubmit` to `(someJS); return
   * false`.
   *
   * This is a map from those attribute names to the corresponding JS event that would be used to
   * execute that JS when it isn't run in line.
   */
...
/**
 * Helper class that performs certain Lift-specific normalizations of HTML represented as
 * `NodeSeq`s:
 *   - Fixes URLs for `link`, `a`, `form`, and `script` elements to properly prepend the container's
 *     context path in case these are absolute URLs.
 *   - Extracts event attributes (replacing them with
 *     `[[LiftRules.attributeForRemovedEventAttributes]]` if needed), returning instead JavaScript
 *     that will attach the corresponding event handler to that element.
...
private[http] final object HtmlNormalizer {
      ...
      if (eventAttributes.nonEmpty) {
        ...
        NodeAndEventJs(
          element.copy(attributes =
            new UnprefixedAttribute("id", generatedId, attributesIncludingEventsAsData)),
  ...
  /**
   * Normalizes `nodes` to adjust URLs with the given `contextPath`, stripping comments if
   * `stripComments` is `true`, and extracting event JavaScript into the `js` part of the returned
   * `NodesAndEventJs` if `extractEventJavaScript` is `true`.
   *
