  ...
  def merge(xhtml: NodeSeq, req: Req): Node = {
    ...
    def waitUntilSnippetsDone(): Unit = {
      ...
      snippetHashs.synchronized {
        if (myMillis >= waitUntil || snippetHashs.isEmpty || !snippetHashs.values.iterator.contains(
            Empty)) ()
        else {
        ...
        LiftRules.xhtmlValidator.toList.flatMap(_(tmpRet)) match {
          ...
          case xs =>
            ...
            val errors: NodeSeq = xs.map(e =>
              <div style="border: red solid 2px">XHTML Validation error:{e.msg}at line{
                e.line + 1
              }and column{e.col}</div>)
