...
class LiftRules() extends Factory with FormVendor with LazyLoggable {
   ...
   */
  val noticesAutoFadeOut = new FactoryMaker[(NoticeType.Value) => Box[(FiniteDuration, FiniteDuration)]]((notice : NoticeType.Value) => Empty){}
  ...
  private def _getLiftSession(req: Req): LiftSession = {
    ...
    val cometSessionId = wp match {
      case LiftPath :+ "comet" :+ session :+ _ => Full(session)
      case _ => Empty
    }
  ...
  */
   ...
   */
  val lazySnippetTimeout: FactoryMaker[FiniteDuration] = new FactoryMaker(() => 30.seconds) {}
  ...
  /**
    ...
    */
  def liftPath: String = S.contextPath + "/" + liftContextRelativePath().mkString("/")
