...
trait AbstractScreen extends Factory with Loggable {
  ...
  def additionalAttributes: MetaData =
    if (hasUploadField) new UnprefixedAttribute("enctype", Text("multipart/form-data"), Null)
    else Null

  /**
   * Add a FieldContainer to the Screen. A FieldContainer can contain either a single field (a
   * BaseField) or a collection of BaseFields. The key take-away is that if the LiftScreen or Wizard
   * is a singleton, you can still display variable number of fields by returning a variable number
   * of BaseField instances from the FieldContainer. <br/> WARNING -- this method is public so it
   * can be called from a Wizard. This method should only be called from within the Screen or Wizard
   * that owns the Screen and not from external code.
   */
  ...
  trait Field extends ConfirmField {
    ...
    /**
     * A field my have an "otherValue" which can be used to store a list of options or other
     * information that the fields needs on a Screen/Wizard by Screen/Wizard basis
     */
   ...
   */
  protected def builder[T](name: => String, default: => T, stuff: FilterOrValidate[T]*)(implicit
  ...
  protected object FilterOrValidate {
    ...
    implicit def promoteFieldBinding(binding: FieldBinding): FilterOrValidate[Nothing] =
  ...
  /**
   * Override the screen default for fields appearing on the confirm screen and force this field to
   * appear on the confirm screen
   */
  protected case object OnConfirmScreen extends FilterOrValidate[Nothing]

  /**
   * Override the screen default for fields appearing on the confirm screen and force this field not
   * to appear on the confirm screen
   */
  ...
  protected final case class FieldTransform(func: BaseField => NodeSeq => NodeSeq)
  ...
  protected def field[T](
      ...
      stuff: FilterOrValidate[T]*)(implicit man: Manifest[T], marker: BoxMarker): Field {
    type ValueType = T
  } = {
    val paramFieldId: Box[String] = (stuff.collect {
      case FormFieldId(id) => id
    ...
    new Field {
      ...
      override def set(v: T) =
        underlying.openOrThrowException("Legacy code").set(setFilter.foldLeft(v)((v, f) => f(v)))
   ...
   */
  protected def field[T](name: => String, default: => T, stuff: FilterOrValidate[T]*)(implicit
   ...
   *
   * @param theName
   *   the name of the field. This is call-by-name, so you can do things like S.?("Dog's Name") such
   *   that the string will be localized
   *
   * @param defaultValue
   *   the starting value for the field. This is also call-by-name which is handy for constructs
   *   like: <code class="scala">SomeExternalRequestVarOrSessionVar.get</code>
   *
   * @param theToForm
   *   a function to convert the field into a form
   *
   * @param otherValue
   *   a handy way include other values in the field. The other value is calcualted when the field
   *   is initialized. You can, for example, put a list of valid options in the field.
   *
   * @param stuff
  ...
  protected def makeField[T, OV](
      ...
      stuff: FilterOrValidate[T]*): Field { type ValueType = T; type OtherValueType = OV } = {
    val newBinding: Box[FieldBinding] = (stuff.collect {
      case AFieldBinding(i) => i
    }).headOption
  ...
  protected def select[T](
      ...
      default: => T,
      choices: => Seq[T],
      stuff: FilterOrValidate[T]*)(implicit f: SHtml.PairStringPromoter[T]): Field {
  ...
  protected def multiselect[T](
      ...
      default: => Seq[T],
      choices: => Seq[T],
      stuff: FilterOrValidate[Seq[T]]*)(implicit f: SHtml.PairStringPromoter[T]): Field {
   ...
   */
  protected def grabParams(in: Seq[FilterOrValidate[_]]): List[SHtml.ElemAttr] = {
    val sl = in.toList
    in.collect {
...
trait ScreenWizardRendered extends Loggable {
  ...
  protected def renderAll(
    ...
    def fieldsWithStyle(style: BindingStyle, includeMissing: Boolean) =
      logger.trace(
        "Looking for fields with style %s, includeMissing = %s".format(style, includeMissing),
    ...
    def templateFields: List[CssBindFunc] =
      List(sel(_.fieldContainer, ".%s") #> (fieldsWithStyle(Template, true) map (field =>
        bindField(field))))
    ...
    def customFields: List[CssBindFunc] =
      for {
        ...
        "Binding custom field %s to %s".format(bindingInfo.selector(formName), custom.template),
    ...
    def dynamicFields: List[CssBindFunc] =
      for {
        ...
        traceInline(
          "Binding dynamic field %s to %s".format(bindingInfo.selector(formName), template),
    ...
    def bindField(f: ScreenFieldInfo): NodeSeq => NodeSeq = {
      ...
      def bindLabel(): CssBindFunc = {
        val basicLabel =
          sel(_.label, ".%s [for]") #> curId & nsSetChildren(_.label, f.text ++ labelSuffix)
        myNotices match {
          case Nil => basicLabel
          case _ =>
    ...
    val processed =
      S.session map (_.runTemplate("css-bound-screen", allTemplate)) openOr (allTemplate)
   ...
   *
   * If the ajax=true attribute is present on the original snippet invocation, the forms will be
   * ajax.
   */
...
trait LiftScreen extends AbstractScreen with StatefulSnippet with ScreenWizardRendered {
  ...
  /**
   * What to do when the Screen is done. By default, will do a redirect back to Whence, but you can
   * change this behavior, for example, put up some other Ajax thing or alternatively, remove the
  ...
  abstract class ScreenVar[T](dflt: => T) extends NonCleanAnyVar[T](dflt) {
    ...
    /**
     * Different Vars require different mechanisms for synchronization. This method implements the
     * Var specific synchronization mechanism
     */
  ...
  protected def bindLocalAction(selector: String, func: () => JsCmd): CssSel = {
    mapLocalAction(func)(name =>
      selector #> (
        SHtml.makeAjaxCall(
          LiftRules.jsArtifacts.serialize(NextId.get) + ("&" + LocalActionRef.get + "=" + name)).cmd
      ).toJsCmd)
  }
  ...
  protected def renderHtml(): NodeSeq = {
    ...
    renderAll(
      ...
      theScreen.screenFields.filter(_.shouldDisplay_?).flatMap(f =>
        if (f.show_?) List(ScreenFieldInfo(
          f,
          f.displayHtml,
          f.helpAsHtml,
          f.toForm,
...
object FieldBinding {
  ...
  /**
   * Bind the field using the default template defined in an external template (as in
   * Bind.helpers()-based binding)
   */
  ...
  /**
   * Bind the field using the results of a function. The provided function will be called every time
