...
object LiftSession {
  ...
  /**
   * Holds user's functions that will be called when a function owner is no longer has functions
   * registered for callbacks.
   *
   * Note that owners are '''typically''', though not always, the [[S.renderVersion render version]]
   * of a page, so this is a good way to find out when a page no longer has any client-side
   * functions (e.g., form fields) bound on the server.
   */
  ...
  /**
   * After the session is created, if you need to do anything within the context of the session
   * (like set SessionVars, etc), add the function to this list
   */
  ...
  @volatile private var constructorCache: Map[(Class[_], Box[Class[_]]), Box[ConstructorType]] =
    Map()
...
trait HowStateful {
  ...
  /**
   * There may be cases when you are allowed container state (e.g., migratory session, but you're
   * not allowed to write Lift non-migratory state, return true here.
   */
  def allowContainerState_? = howStateful.box openOr true

  /**
   * Within the scope of the call, this session is forced into statelessness. This allows for
   * certain URLs in on the site to be stateless and not generate a session, but if a valid session
   * is presented, they have the scope of that session/User
   */
...
/**
 * Sessions that include this trait will not be retained past the current request and will give
 * notifications of failure if stateful features of Lift are accessed
 */
...
/**
 * Sessions that include this trait will only have access to the container's state via
 * ContainerVars. This mode is "migratory" so that a session can migrate across app servers. In this
 * mode, functions that access Lift state will give notifications of failure if stateful features of
 * Lift are accessed
 */
...
/**
 * Keeps information around about what kinds of functions are run at the end of page rendering. The
 * results of these functions will be appended to the bottom of the page.
 *
 * @param renderVersion
...
/**
 * The responseFuture will be satisfied by the original request handling thread when the response
 * has been calculated. Retries will wait for the future to be satisfied in order to return the
 * proper response.
 */
...
class LiftSession(
  ...
  /**
   * The unique id of this session. Can be used to securely and uniquely identify the session,
   * unlike underlyingId which is tied to the host session id. This id should be secure to emit into
   * a page as needed (for example, it is used to provide randomness/cache-busting to the comet
   * request path).
   */
  ...
  /**
   * A mapping between pages denoted by RenderVersion and functions to execute at the end of the
   * page rendering
   */
  @volatile private var postPageFunctions: Map[String, PostPageFunctions] = Map()

  /**
   * A list of AJAX requests that may or may not be pending for this session. There is an entry for
   * every AJAX request we don't *know* has completed successfully or been discarded by the client.
   *
   * See LiftServlet.handleAjax for how we determine we no longer need to hold a reference to an
   * AJAX request.
   */
  ...
  /**
   * Find a function in the function lookup table. You probably never need to do this, but well, you
   * can look them up.
   */
  ...
  /**
   * Given a test that takes an [[S.AFuncHolder]] and produces a boolean, remove any bound function
   * in the session that passes the test.
   *
   * If any function owner is completely absent after going through the functions, any function
   * registered via `[[LiftSession$.onFunctionOwnersRemoved]]` will be called with the list of
   * removed owners.
   */
  ...
  /**
   * Set your session-specific progress listener for mime uploads pBytesRead - The total number of
   * bytes, which have been read so far. pContentLength - The total number of bytes, which are being
   * read. May be -1, if this number is unknown. pItems - The number of the field, which is
  ...
  /**
   * Clear the PostPage JavaScript functions for the current page. This is used by CometActor to
   * remove the PostPage JavaScript functions from the given component during redraw.
   */
  ...
  /**
   * Associate a function that renders JavaScript with the current page. This function will be run
   * and the resulting JavaScript will be appended to any rendering associated with this page... the
   * normal page render, Ajax calls, and even Comet calls for this page.
   *
   * @param func
  ...
  /**
   * Get the post-page JavaScript functions for a sequence of page IDs. This is used by the
   * CometActor to get the post-page JavaScript functions for the comet actor and for the page the
   * the comet actor is associated with
   */
  ...
  /**
   * Updates the timestamp of the functions owned by this owner and return the number of updated
   * functions
   */
  ...
  /**
   * Define the context path for this session. This allows different sessions to have different
   * context paths.
   */
   ...
   *
   * @param template
   *   -- the NodeSeq that makes up the page... or the template will be located via
   *   findVisibleTemplate
   * @param request
   ...
   * The continuation function must return Nothing (it must throw an exception... this is typically
   * done by calling S.redirectTo(...)). This method is useful for changing sessions on login. Issue
   * #727.
   */
              ...
              case Left(true) =>
                checkStatelessInSiteMap(request) {
                  ...
                  LiftRules.allowParallelSnippets.doWith(() => !Props.inGAE) {
                    (request.location.flatMap(_.earlyResponse) or LiftRules.earlyResponse.firstFull(
                      request)) or
  ...
  def runSourceContext(value: Any, xform: NodeSeq => NodeSeq, ns: NodeSeq): NodeSeq = {
    ...
    value match {
      ...
      case node: scala.xml.Node =>
        currentSourceContext.doWith(node)(processSurroundAndInclude("Source", xform(ns)))
              ...
              instantiateOrRedirect(clz) flatMap {
                inst =>
                  invokeMethod(
                    clz,
                    inst,
                    method) or invokeMethod(clz, inst, method, params.toList.toArray) match {
      ...
      in match {
        ...
        case mine: PrefixedAttribute if (mine.pre == "lift") => {
          mine.key match {
            ...
            case "snippet" =>
              findAttributeSnippet(mine.value.text, processAttributes(in.next, allow))
  ...
  /**
   * During the HTTP request/response cycle or in a CometActor, Lift populates "S" with information
   * about the current session, the current request, etc. This method allows you to wrap a function
   * in another function that will snapshot current state (request vars, Req, Loc, etc.) such that
   * when the returned function is executed, it will be executed as if it had been executed in the
   * scope of the thread where it was create. This allows you to farm work out to separate threads,
   * but make it look to those threads as if the scope was the same as if it had been executed on
   * the thread that created the function.
   */
   ...
   *
   * The returned function, when invoked with a parameter of type `A`, will invoke the passed
   * `deferredFunction` with that parameter in the original request and session context.
   */
  ...
  private def processSnippet(
                    ...
                    (locateAndCacheSnippet(cls)) match {
                      ...
                      case Full(inst) => {
                        ...
                        def nodeSeqFunc: Box[NodeSeq] =
                          ...
                          } yield meth.invoke(inst).asInstanceOf[Function1[
                            NodeSeq,
                            NodeSeq]].apply(kids)
                          ...
                          ((Helpers.invokeMethod(
                            ...
                            case it =>
                              val intersection = if (Props.devMode) {
                                val methodNames =
                                  inst.getClass.getMethods().map(_.getName).toList.distinct
                                val methodAlts = List(
                                  method,
    ...
    def checkAttr(attr_name: String, in: MetaData, base: MetaData): MetaData =
      in.filter(_.key == attr_name).toList match {
        case Nil => base
        case x => new UnprefixedAttribute(
            attr_name,
            Text(x.head.value.text),
            base)
      }
      ...
      attrs.get("form").map(_.text.trim.toLowerCase) match {
        case Some("post") =>
          S.withAttrs(attrs.filter(_.key == "multipart")) {
            ...
            case e: Elem => e % LiftRules.formAttrs.vend.foldLeft[MetaData](Null)((base, name) =>
        ...
        case Some(ft) =>
          <form action={S.uri} method={ft}>
            ...
            checkMultiPart(attrs) % LiftRules.formAttrs.vend.foldLeft[MetaData](Null)(
   ...
   *
   * In general, you should heavily consider using `[[normalizeHtmlAndEventHandlers]]` or its
   * friendliest sibling, `[[normalizeHtmlAndAppendEventHandlers]]`.
   */
  ...
  /**
   * Applies various HTML corrections to the passed HTML, including adding the context path to links
   * and extracting event handlers into a separate `JsCmd`.
   *
   * Note that most of the time you can just call `[[normalizeHtmlAndAppendEventHandlers]]` and not
   * worry about the extra `JsCmd`, as Lift will automatically append it to the response.
   *
   * @param forceExtractInlineJavaScript
   *   If `None`, uses `LiftRules.extractInlineJavaScript` to decide whether or not to extract
   *   inline JS from the passed nodes. If `Some`, extracts (`Some(true)`) or doesn't
   *   (`Some(false)`).
   */
  ...
  /**
   * Runs `[[normalizeHtmlAndEventHandlers]]` to adjust URLs to context paths and extract JS event
   * handlers from the given `NodeSeq` and pull them into a separate JsCmd, then uses
   * `[[net.liftweb.http.S!.appendJs(js:net\.liftweb\.http\.js\.JsCmd):Unit* S.appendJs]]` to append
   * that JS to the response's JavaScript (the page-specific JS file if we are rendering a page, or
   * the end of the returned JS if this is an AJAX or comet request).
   */
  ...
  /**
   * The partial function that defines how lift tags are processed for this session. Initially
   * composed of LiftRules.liftTagProcessing orElse the default lift tag processing. If you need to
   * change the way a particular session handles lift tags, alter this partial function.
   */
  @volatile
      ...
      case ("snippet", elm, metaData, kids, page) =>
        metaData.get("type") match {
          case Some(tn) =>
            S.doSnippet(tn.text) {
              NamedPF(
                (tn.text, elm, metaData, kids, page),
                liftTagProcessing)
            }
  ...
  /**
   * Pass in a LiftActor and get a JavaScript expression (function(x) {...}) that represents an
   * asynchronous Actor message from the client to the server.
   *
   * The Actor should respond to a message in the form of a JsonAST.JValue.
   *
   * This method requires the session be stateful
   *
   * In general, your Actor should be a subclass of ScopedLiftActor because that way you'll have the
   ...
   *
   * @return
   *   a JsExp that contains a function that can be called with a parameter and when the function is
   *   called, the parameter is JSON serialized and sent to the server
   */
   ...
   *   the Actor to send messages to.
   * @param xlate
   *   a function that will take the JsonAST.JValue and convert it into a representation that can be
   *   sent to the Actor (probably deserialize it into a case class.) If the translation succeeds,
   *   the translated message will be sent to the actor. If the translation fails, an error will be
          ...
          (p: JsonAST.JValue) => {
            in.!(xlate(p) match {
              ...
              case Failure(msg, _, _) =>
                logger.error("Failed to deserialize JSON message " + p + ". Error " + msg); p
            })
            JsCmds.Noop
          }
        ).cmd
  ...
  /**
   * Create a Actor that will take messages on the server and then send them to the client. So, from
   * the server perspective, it's just an Async message send. From the client perspective, they get
   * a function called each time the message is sent from the server.
   *
   ...
   *
   * If the message is a JsonAST.JValue, it's turned into a JSON string, sent to the client and the
   * client calls the function named in the `toCall` parameter with the value.
   *
   * If the message is anything else, we attempt to JSON serialize the message and if it can be JSON
   * serialized, it's sent over the wire and passed to the `toCall` function on the server
   *
   * @param toCall
  ...
  def serverActorForClient(
    ...
    testStatefulFeature {
      val ca = new CometActor {
         ...
         *
         * There are implicit conversions for a bunch of stuff to RenderOut (including NodeSeq).
         * Thus, if you don't declare the return turn to be something other than RenderOut and
         * return something that's coercible into RenderOut, the compiler "does the right thing"(tm)
         * for you. <br/> There are implicit conversions for NodeSeq, so you can return a pile of
         * XML right here. There's an implicit conversion for NodeSeq => NodeSeq, so you can return
         * a function (e.g., a CssBindFunc) that will convert the defaultHtml to the correct output.
         * There's an implicit conversion from JsCmd, so you can return a pile of JavaScript that'll
         * be shipped to the browser.<br/> Note that the render method will be called each time a
         * new browser tab is opened to the comet component or the comet component is otherwise
         * accessed during a full page load (this is true if a partialUpdate has occurred.) You may
         * want to look at the fixedRender method which is only called once and sets up a stable
         * rendering state.
         */
   ...
   * If the CometActor already exists in the session, the message will be sent immediately. If the
   * CometActor is not yet instantiated, the message will be sent to the CometActor as part of setup
   * `[[queueCometMessage]]` if it is created as part of the current HTTP request/response cycle.
   *
   * @param theType
  ...
  /**
   * Similar behavior to
   * [[LiftSession#sendCometMessage(theType:String,msg:Any):Unit the main sendCometMessage]], except
   ...
   * Similar behavior to
   * [[LiftSession#sendCometMessage(theType:String,msg:Any):Unit the main sendCometMessage]], except
   * that this version will limit based on the name of the comet. Providing `name` as `Empty`, will
   * specifically select comets with no name.
   *
   * @param theType
   *   the type of the CometActor
   * @param name
   ...
   * Similar behavior to
   * [[LiftSession#sendCometMessage(theType:String,msg:Any):Unit the main sendCometMessage]], except
   * that this version will limit based on the name of the comet and it takes its type selector as a
   * type parameter.
   *
   * @param name
   *   the optional name of the CometActor
   ...
   * exist, the comet type is first looked up via `LiftRules.cometCreationFactory`, then
   * `LiftRules.cometCreation`, and finally as a class name in the comet packages designated by
  ...
  private def findOrBuildComet[T <: LiftCometActor](
    ...
    testStatefulFeature {
      val existingOrNewComet =
        Box.legacyNullTest(nasyncComponents.get(cometInfo)).asA[T] or newCometFn(creationInfo)
  ...
  /**
   * Need to add your own CometActor to a Session/Page? You might 'cause creating a CometActor
   * programmatically within a snippet may be between that referring to a CometActor by class name.
   *
   ...
   */
  def buildAndStoreComet[T <: LiftCometActor](newCometFn: (CometCreationInfo) => Box[T])(
  ...
  private[liftweb] def findAndMerge(
    ...
    val name: String = templateName.map(s => if (s.startsWith("/")) s else "/" + s).openOr(
  ...
  /**
   * Build a bunch of round-trip calls between the client and the server. The client calls the
   * server with a parameter and the parameter gets marshalled to the server and the code is
   * executed on the server. The result can be an item (JValue) or a Stream of Items.
   *
   * @param info
    ...
    testStatefulFeature {
      ...
      val ca = new CometActor {
        ...
        override def lowPriority: PartialFunction[Any, Unit] = {
          ...
          case DoneMsg(guid) =>
            partialUpdate(
              JsCmds.JsSchedule(JsRaw(s"lift.sendEvent(${guid.encJs}, {'done': true} )").cmd))
      ...
      lazy val theFunc =
        JsRaw(s"""function(v) {${SHtml.jsonCall(JsRaw("v"), localFunc(_)).toJsCmd}}""")

      lazy val build: (String, JsExp) = "_call_server" -> theFunc

      JsObj(build :: info.map(info =>
        info.name -> JsRaw(
          s"""
            |function(param) {
...
/**
 * If a class is to be used as a lift view (rendering from code rather than a static template) and
 * the method names are to be used as "actions", the view must be marked as "InsecureLiftView"
 * because there exists the ability to execute arbitrary methods based on wire content
 */
trait InsecureLiftView

/**
 * The preferred way to do lift views... implement a partial function that dispatches the incoming
 * request to an appropriate method
 */
trait LiftView {
      ...
      next match {
        ...
        case up: UnprefixedAttribute if up.key == "class" =>
          up.value.text.charSplit(' ').filter(s => !isLiftClass(s)) match {
            case Nil =>
            case xs => nonLift = new UnprefixedAttribute(
                "class",
                xs.mkString(" "),
                nonLift)
          }
        ...
        case p: PrefixedAttribute
            if (p.pre == "l" || p.pre == "lift") && p.key == "parallel" => par = true

        case up: UnprefixedAttribute if up.key == "lift" || up.key == "data-lift" => // ignore

        case p: PrefixedAttribute
            if p.pre == "lift" && p.key == "snippet" => nonLift = p.copy(nonLift)
...
/**
 * A function (well, an interface with a bunch of methods on it) to call depending on the state of
 * the round trip function.
 */
trait RoundTripHandlerFunc {

  /**
   * Send data back to the client. This may be called many times and each time, more data gets sent
   * back to the client.
   * @param value
...
final case class StreamRoundTrip[T](name: String, func: T => Stream[Any])(implicit
    val manifest: Manifest[T]) extends RoundTripInfo
