...
/**
 * Mix this trait into a class to provide support for MVC style coding. Each controller line is
 * defined as: <pre name="code" class="scala"> serve { case "user" :: User(user) :: _ => "#name" #>
 * user.firstName } </pre>
 *
 * The above code matches /user/4, loads the user with primary key 4 from the database, then applies
 * the transform to the /user.html template replacing the node with the id "name" with the firstName
 * of the user
 */
trait MVCHelper extends LiftRules.DispatchPF {
  ...
  private object curSession extends RequestVar[LiftSession](S.session openOr
  ...
  /**
   * A trait that holds a response for the MVCHelper. Conversions exist from Unit (just serve the
   * template), CssBindFunc (do the substitution on the template), NodeSeq (run the template),
   * LiftResponse (send the response back), or Box or Option of any of the above.
   */
  ...
  object MVCResponse {
    ...
    implicit def boxThinginy[T](box: Box[T])(implicit f: T => MVCResponse): MVCResponse =
      new MVCResponse {
        val toResponse: Box[LiftResponse] = boxToResp(box)(f)
      }

    implicit def optionThinginy[T](box: Option[T])(implicit f: T => MVCResponse): MVCResponse =
      new MVCResponse {
        val toResponse: Box[LiftResponse] = boxToResp(box)(f)
      }
  }
   ...
   */
  protected implicit def boxToResp[T](in: Box[T])(implicit c: T => MVCResponse): Box[LiftResponse] =
    in match {
  ...
  /**
   * Convert an Empty into an appropriate LiftResponse. In the case of Failure, you may want to
   * display a particular error message to the user.
   */
  ...
  /**
   * Validate what, if it passes validation, then redirect to the new URL, else display the messages
