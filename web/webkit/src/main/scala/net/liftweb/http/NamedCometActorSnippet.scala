...
trait NamedCometActorSnippet {
   ...
   *
   * <pre name="code" class="scala"><code> def name= S.param(p).openOr("A") </code></pre>
   *
   * This causes every comet actor for class @cometClass with the same @name to include the same
   * Comet Actor
   */
  def name: String

  /**
   * The render method that inserts the <lift:comet> tag to add the comet actor to the page
   */
