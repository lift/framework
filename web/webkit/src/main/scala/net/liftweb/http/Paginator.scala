...
/**
 * Base class for things that require pagination. Implements a contract for supplying the correct
 * number of browsable pages etc
 *
 * @tparam T
...
trait Paginator[T] extends Loggable {
  ...
  /**
   * Returns a list of page numbers to be displayed in 'zoomed' mode, i.e., as the page numbers get
   * further from the current page, they are more sparse.
   */
  def zoomedPages =
    (
 ...
 *
 * T: The type of the elements, accessed via def page within the listing snippet C: The type of the
 * columns, used to specify sorting
 *
 * @author
 *   nafg and Timothy Perrett
 */
trait SortedPaginator[T, C] extends Paginator[T] {
  ...
  /**
   * Returns a new SortState based on a column index. If the paginator is already sorted by that
   * column, it toggles the direction; otherwise the direction is ascending. Note that this method
   * does not alter the sort state in the paginator; it only calculates the direction toggle.
   * Example usage: sortedPaginator.sort = sortedPaginator.sortedBy(columns.indexOf(clickedColumn))
   */
 ...
 *
 * The values for the pagination are bound according to the classes specified in the [[paginate]]
 * method, using a CSS selector transform.
 *
 * @author
 *   nafg and Timothy Perrett
 */
trait PaginatorSnippet[T] extends Paginator[T] {
  ...
  def currentXml: NodeSeq =
    if (count == 0)
      ...
      Text(S.?(
        ...
        Array(recordsFrom, recordsTo, count).map(_.asInstanceOf[AnyRef]).toList: _*))
  ...
  /**
   * Overrides the super's implementation so the first record can be overridden by a URL query
   * parameter.
   */
  ...
  /**
   * Returns XML that links to a page starting at the given record offset, if the offset is valid
   * and not the current one.
   * @param ns
   *   The link text, if the offset is valid and not the current offset; or, if that is not the
   *   case, the static unlinked text to display
   */
  ...
  /**
   * This method binds template HTML based according to the specified configuration. You can
   * reference this as a snippet method directly in your template; or you can call it directly as
   * part of your binding code.
   *
...
/**
 * This trait adds snippet functionality for sorted paginators. You can place bind points in your
 * template for column headers, and it turns them into links That you can click to sort by that
 * column. Simply write, e.g.,
...
trait SortedPaginatorSnippet[T, C] extends SortedPaginator[T, C] with PaginatorSnippet[T] {
   ...
   * part of your binding code.
   *
   * In addition to the classes bound in {@link PaginatorSnippet}, for each header in the `headers`
   * list, this will bind elements with that class name and put a link in them with their contents.
   *
   * For example, with a list of headers `List("foo", "bar")`, this would bind the `.foo` element's
   * contents to contain a link to a page that renders that column sorted, as well as the `.bar`
   * element's contents to contain a link to a page that renders that column sorted.
   */
...
/**
 * Sort your paginated views by using lifts functions mapping. The only down side with this style is
 * that your links are session specific and non-bookmarkable. If you mix this trait in to a
 * StatefulSnippet, it should work out the box. Otherwise, implement 'registerThisSnippet.'
 * @author
 *   nafg and Timothy Perrett
 */
trait StatefulSortedPaginatorSnippet[T, C] extends SortedPaginatorSnippet[T, C] {
  ...
  /**
   * This method is called before the new page is served, to set up the state in advance. It is
   * implemented by StatefulSnippet so you can just mix in StatefulSortedPaginatorSnippet to one; or
   * you can implement it yourself, using things like S.mapSnippet.
   */
