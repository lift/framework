...
object UserAgentCalculator extends Factory {
  ...
  /**
   * You can change the mechanism by which the user agent for IE is calculated by doing the Factory
  ...
  def defaultIeCalcFunction(userAgent: Box[String]): Box[Double] =
    for {
      ...
      ieVersionString <-
        Box.legacyNullTest(ieMatch.group(2)) or Box.legacyNullTest(ieMatch.group(3))
  ...
  /**
   * You can change the mechanism by which the user agent for Safari is calculated by doing the
   * Factory thing with this object
   */
  object safariCalcFunction
      extends FactoryMaker[Box[String] => Box[Double]](defaultSafariCalcFunction _)
  ...
  /**
   * You can change the mechanism by which the user agent for Firefox is calculated by doing the
   * Factory thing with this object
   */
  object firefoxCalcFunction
      extends FactoryMaker[Box[String] => Box[Double]](defaultFirefoxCalcFunction _)
  ...
  /**
   * You can change the mechanism by which the user agent for Chrome is calculated by doing the
   * Factory thing with this object
   */
  object chromeCalcFunction
      extends FactoryMaker[Box[String] => Box[Double]](defaultChromeCalcFunction _)

  /**
   * You can change the mechanism by which Lift calculates if the User-Agent represents an iPhone.
   * Put your special calculation function in here
   */
  object iPhoneCalcFunction extends FactoryMaker[Box[Box[String] => Boolean]](Empty)

  /**
   * You can change the mechanism by which Lift calculates if the User-Agent represents an iPad. Put
   * your special calculation function in here
   */
...
class InMemFileParamHolder(
    ...
    val file: Array[Byte]) extends FileParamHolder(name, mimeType, fileName) {
...
class OnDiskFileParamHolder(
    ...
    override val mimeType: String,
    override val fileName: String,
    val localPath: Path) extends FileParamHolder(name, mimeType, fileName) {
...
/**
 * A ThreadGlobal containing the low-level HTTPRequest and HTTPResponse instances Set very, very
 * early.
 */
...
object Req {
  ...
  def apply(
    ...
    val stateless =
      NamedPF.applyBox(StatelessReqTest(wholePath, original.request), otherStatelessTest) or
  ...
  def apply(
    ...
    val turi = if (request.uri.length >= contextPath.length)
    ...
    val localParams: Map[String, List[String]] = Map(rewritten.params.toList.map {
    ...
    val paramCalcInfo: AvoidGAL = new AvoidGAL(() => {
      ...
      if ((reqType.post_? ||
        ...
        val params = localParams ++ (request.params.sortWith { (s1, s2) => s1.name < s2.name }).
  ...
  private def _fixHref(
    ...
    val updated =
      if (hv.startsWith("/") &&
        !hv.startsWith("//") &&
        !LiftRules.excludePathFromContextPathRewriting.vend(hv)) contextPath + hv
      else hv
  ...
  /**
   * Corrects the HTML content,such as applying context path to URI's, session information if
   * cookies are disabled etc.
   */
...
/**
 * Holds information about the content type and subtype including the q parameter and extension
 * information.
 */
final case class ContentType(
  ...
  /**
   * Compares this to another ContentType instance based on the q and if the q matches, compare
   * based on specialization (* vs. explicit and then order.
   */
...
/**
 * The ContentType companion object that has helper methods for parsing Accept headers and other
 * things that contain multiple ContentType information.
 */
object ContentType {

  /**
   * Parse the String into a series of ContentType instances, returning the multiple ContentType
   * instances
   */
...
class Req(
  ...
  /**
   * Build a new Req, the same except with a different path. Useful for creating Reqs with
   * sub-paths.
   */
  def withNewPath(newPath: ParsePath): Req = {
    ...
    new Req(
      ...
      /**
       * What is the content type in order of preference by the requester calculated via the Accept
       * header
       */
   ...
   *
   * Most ajax frameworks, including jQuery and Prototype, set this header when doing any ajax
   * request.
   */
  def ajax_? =
   ...
   * application/x-www-form-urlencoded or a multipart mime. It will also be empty if rawInputStream
   * is accessed
   */
  def body: Box[Array[Byte]] = _body.flatMap(_.body)

  /**
   * The raw input stream of a POST or PUT that is not application/x-www-form-urlencoded or a
   * multipart mime and if this method is called before the body method. Remember to close the
   * stream when done.
   */
  ...
  /**
   * The JValue representation of this Req's body, if the body is JSON-parsable AND the content-type
   * of the request is JSON. Returns a Failure if the request is not considered a JSON request (see
   * json_?), or if there was an error parsing the JSON.
   *
  ...
  /**
   * Forcibly tries to parse the request body as JSON. Does not perform any content type checks,
   * unlike the json method.
   */
  lazy val forcedBodyAsJson: Box[JsonAST.JValue] = {
    try {
      ...
      def charSet: String =
        contentType.flatMap(ct => r.findFirstIn(ct).flatMap(r2.findFirstIn)).getOrElse("UTF-8")

      body.map(b =>
        JsonParser.parse(new InputStreamReader(new ByteArrayInputStream(b), charSet)))
    } catch {
  ...
  /**
   * The Elem representation of this Req's body, if the body is XML-parsable AND the content-type of
   * the request is XML. Returns a Failure if the request is not considered a XML request (see
   * xml_?), or if there was an error parsing the XML.
   *
  ...
  /**
   * Forcibly tries to parse the request body as XML. Does not perform any content type checks,
   * unlike the xml method.
   */
  ...
  /**
   * Test the current SiteMap Loc for access control to insure that this Req is allowed to access
   * the page
   */
  ...
  def createNotFound: LiftResponse =
    NamedPF((this, Empty), LiftRules.uriNotFound.toList) match {
      ...
      case NotFoundAsNode(node) => LiftRules.convertResponse(
          (node, 404),
  ...
  private[http] def createNotFound(f: (ParsePath) => Box[LiftResponse]): Box[LiftResponse] =
    NamedPF((this, Empty), LiftRules.uriNotFound.toList) match {
      ...
      case NotFoundAsNode(node) => Full(LiftRules.convertResponse(
          (node, 404),
          S.getResponseHeaders(LiftRules.defaultHeaders((node, this))),
          S.responseCookies,
          this))
...
/**
 * This case class is used for pattern matching. See LiftRules.statelessRewrite and
 ...
 * `[[LiftRules.urlDecorate]]` or the container's built-in URL decoration which may append the
 * session id to the URL (dependent on `[[LiftRules.encodeJSessionIdInUrl_?]]`).
 */
object URLRewriter {
