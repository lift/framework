...
/**
 * An object representing the current state of the HTTP request and response. It uses the
 * DynamicVariable construct such that each thread has its own local session info without passing a
...
object S extends S {
  ...
  /**
   * RewriteHolder holds a partial function that re-writes an incoming request. It is used for
   * per-session rewrites, as opposed to global rewrites, which are handled by the LiftRules.rewrite
   * RulesSeq. This case class exists so that RewritePFs may be manipulated by name. See
   * S.addSessionRewriter for example usage.
   *
   * @see
   *   # sessionRewriter
   * @see
  ...
  /**
   * DispatchHolder holds a partial function that maps a Req to a LiftResponse. It is used for
   * per-session dispatch, as opposed to global dispatch, which are handled by the
   * LiftRules.dispatch RulesSeq. This case class exists so that DispatchPFs may be manipulated by
   * name. See S.addHighLevelSessionDispatcher for example usage.
   *
   * @see
   *   LiftResponse
   * @see
  ...
  /**
   * The CookieHolder class holds information about cookies to be sent during the session, as well
   * as utility methods for adding and deleting cookies. It is used internally.
   *
   * @see
   *   # _responseCookies
   * @see
  ...
  object PFPromoter {
    ...
    implicit def fromFunc[A, B](pff: () => PartialFunction[A, B]): PFPromoter[A, B] =
   ...
   */
  private final class BinFuncHolder(val func: FileParamHolder => Any, val owner: Box[String])
  ...
  /**
   * Impersonates a function that is executed on HTTP requests from client. The function takes a
   * String as the only parameter and returns an Any.
   */
  ...
  /**
   * Impersonates a function that is executed on HTTP requests from client. The function takes a
   * List[String] as the only parameter and returns an Any.
   */
  private final class LFuncHolder(val func: List[String] => Any, val owner: Box[String])
      extends AFuncHolder with Serializable {
  ...
  /**
   * Impersonates a function that is executed on HTTP requests from client. The function takes zero
   * arguments and returns an Any.
   */
   ...
   */
  private[http] final case class PageStateHolder(owner: Box[String], session: LiftSession)
      extends AFuncHolder {
  ...
  object AFuncHolder {
    ...
    implicit def boolToAF(f: Boolean => Any): AFuncHolder =
      LFuncHolder(lst => f(lst.foldLeft(false)((v, str) => v || Helpers.toBoolean(str))))
  }
...
trait S extends HasParams with Loggable with UserAgentCalculator {
  ...
  /**
   * This is simply a flag so that we know whether or not the state for the S object has been
   * initialized for our current scope.
   *
   * @see
   *   # inStatefulScope_ ?
   * @see
  ...
  /**
   * The _snippetMap holds mappings from snippet names to snippet functions. These mappings are
   * valid only in the current request. This val is typically modified using the mapSnippet method.
   *
   * @see
   *   # mapSnippet
   * @see
  ...
  /**
   * Holds the attributes that are set on the current snippet tag. Attributes are available to
   * snippet functions via the S.attr and S.attrs methods. The attributes are held in a tuple,
   * representing (most recent attributes, full attribute stack).
   *
   * @see
   *   # attrs
   * @see
  ...
  /**
   * During an Ajax call made on a Comet component, make the Req's params available
   */
  private object paramsForComet extends TransientRequestVar[Map[String, List[String]]](Map())

  /**
   * We can now collect JavaScript to append to the outgoing request, no matter the format of the
   * outgoing request
   */
  ...
  // callback.
  private[liftweb] object requestCometVersions
  ...
  private object p_notice
      extends TransientRequestVar(new ListBuffer[(NoticeType.Value, NodeSeq, Box[String])])

  /**
   * This method returns true if the S object has been initialized for our current scope. If the S
   * object has not been initialized then functionality on S will not work.
   */
  ...
  /**
   * An exception was thrown during the processing of this request. This is tested to see if the
  ...
  /**
   * @return
   *   a List of any Cookies that have been set for this Response. If you want a specific cookie,
   *   use findCookie.
   *
   * @see
  ...
  /**
   * @return
   *   a List of any Cookies that have been added to the response to be sent back to the user. If
   *   you want the cookies that were sent with the request, see receivedCookies.
   *
   * @see
   *   net.liftweb.http.provider.HTTPCookie
   * @see
   ...
   *
   * <pre name="code" class="scala" > import net.liftweb.http.provider.HTTPCookie
   *
   ...
   *
   * def setCookie() { val cookie = HTTPCookie(cookieName, cookieVal).setMaxAge(3600) // 3600
   * seconds, or one hour S.addCookie(cookie) }
   *
   * bind("cookie", xhtml, "value" -> SHtml.text(cookieVal, cookieVal = _), "add" ->
   * SHtml.submit("Add", setCookie) "remove" -> SHtml.link(S.uri, () => S.deleteCookie(cookieName),
   * "Delete Cookie") ) } } </pre>
   *
   * @see
   *   net.liftweb.http.provider.HTTPCookie
  ...
  /**
   * A boolean indicating whether or not the response should be rendered with special accomodations
   * for IE 6 / 7 / 8 compatibility.
   *
   * @return
   *   <code>true</code> if this response should be rendered in IE 6/7/8 compatibility mode.
   *
   * @see
  ...
  /**
   * Return a List of the LiftRules.DispatchPF functions that are set for this session. See
   * addHighLevelSessionDispatcher for an example of how these are used.
   *
   * @see
  ...
  /**
   * Adds a dispatch function for the current session, as opposed to a global dispatch through
   * LiftRules.dispatch. An example would be if we wanted a user to be able to download a document
   * only when logged in. First, we define a dispatch function to handle the download, specific to a
   * given user:
   *
   * <pre name="code" class="scala" > def getDocument(userId : Long)() : Box[LiftResponse] = { ... }
   * </pre>
   *
   * Then, in the login/logout handling snippets, we could install and remove the custom dispatch as
   * appropriate:
   *
   * <pre name="code" class="scala" > def login(xhtml : NodeSeq) : NodeSeq = { def doAuth () { ...
   * if (user.loggedIn_?) { S.addHighLevelSessionDispatcher("docDownload", { case
   * Req(List("download", "docs"), _, _) => getDocument(user.id) } ) } }
   *
   * def logout(xhtml : NodeSeq) : NodeSeq = { def doLogout () { ...
   * S.removeHighLevelSessionDispatcher("docDownload") // or, if more than one dispatch has been
   * installed, this is simpler S.clearHighLevelSessionDispatcher } } </pre>
   *
   * It's important to note that per-session dispatch takes precedence over LiftRules.dispatch, so
   * you can override things set there.
   *
   * @param name
  ...
  /**
   * Removes a custom dispatch function for the current session. See addHighLevelSessionDispatcher
  ...
  /**
   * Return the list of RewriteHolders set for this session. See addSessionRewriter for an example
   * of how to use per-session rewrites.
   *
   * @see
   *   RewriteHolder
   * @see
  ...
  /**
   * Adds a per-session rewrite function. This can be used if you only want a particular rewrite to
   * be valid within a given session. Per-session rewrites take priority over rewrites set in
   * LiftRules.rewrite, so you can use this mechanism to override global functionality. For example,
   * you could set up a global rule to make requests for the "account profile" page go back to the
   * home page by default:
   *
   * <pre name="code" class="scala" > package bootstrap.liftweb ... imports ... class Boot { def
   * boot { LiftRules.rewrite.append { case RewriteRequest(ParsePath(List("profile")), _, _, _) =>
   * RewriteResponse(List("index")) } } } </pre>
   *
   * Then, in your login snippet, you could set up a per-session rewrite to the correct template:
   *
   * <pre name="code" class="scala" > def loginSnippet (xhtml : NodeSeq) : NodeSeq = { ... def
   * doLogin () { ... S.addSessionRewriter("profile", { case
   * RewriteRequest(ParsePath(List("profile")), _, _, _) => RewriteResponse(List("viewProfile"),
   * Map("user" -> user.id)) } ... } ... } </pre>
   *
   * And in your logout snippet you can remove the rewrite:
   *
   * <pre name="code" class="scala" > def doLogout () { S.removeSessionRewriter("profile") // or
   * S.clearSessionRewriter } </pre>
   *
   * @param name
  ...
  /**
   * Put the given Elem in the head tag. The Elems will be de-dupped so no problems adding the same
   * style tag multiple times
   */
  ...
  /**
   * Add a comet to the list of comets that should be registered to receive updates on the page
   * currently being rendered or on the page that invoked the currently running callback.
   */
  ...
  /**
   * As with {findOrBuildComet[T]}, but specify the type as a `String`. If the comet doesn't already
   * exist, the comet type is first looked up via `LiftRules.cometCreationFactory`, and then as a
   * class name in the comet packages designated by `LiftRules.buildPackage("comet")`.
   *
   * If `receiveUpdates` is `true`, updates to this comet will be pushed to the page currently being
   * rendered or to the page that is currently invoking an AJAX callback. You can also separately
   * register a comet to receive updates like this using {S.addComet}.
   */
  ...
  /**
   * Find or build a comet actor of the given type `T` with the given configuration parameters. If a
   * comet of that type with that name already exists, it is returned; otherwise, a new one of that
   * type is created and set up, then returned.
   *
   * If `receiveUpdates` is `true`, updates to this comet will be pushed to the page currently being
   * rendered or to the page that is currently invoking an AJAX callback. You can also separately
   * register a comet to receive updates like this using {S.addComet}.
  ...
  /**
   * Sometimes it's helpful to accumulate JavaScript as part of servicing a request. For example,
   * you may want to accumulate the JavaScript as part of an Ajax response or a Comet Rendering or
   * as part of a regular HTML rendering. Call `S.appendJs(jsCmd)`. The accumulated Javascript will
   * be emitted as part of the response, wrapped in an `OnLoad` to ensure that it executes after the
   * entire dom is available. If for some reason you need to run javascript at the top-level scope,
   * use appendGlobalJs.
   */
  ...
  /**
   * Add javascript to the page rendering that will execute in the global scope. Usually you should
   * use `appendJs`, so that the javascript runs after the entire dom is available. If you need to
   * declare a global var or you want javascript to execute immediately with no guarantee that the
   * entire dom is available, you may use `appendGlobalJs`.
   */
  def appendGlobalJs(js: JsCmd*): Unit = _globalJsToAppend.is ++= js

  /**
   * Generates the JsCmd needed to initialize comets in `S.requestCometVersions` on the client.
   */
   ...
   *
   * @return
   *   <code>true</code> if the request is a POST request, <code>false</code> otherwise.
   */
  def post_? = request.map(_.post_?).openOr(false)

  /**
   * Localize the incoming string based on a resource bundle for the current locale. The localized
   * string is converted to an XML element if necessary via the
   * <code>LiftRules.localizeStringToXml</code> function (the default behavior is to wrap it in a
   * Text element). If the lookup fails for a given resource bundle (e.g. a null is returned), then
   * the <code>LiftRules.localizationLookupFailureNotice</code> function is called with the input
   * string and locale.
   *
   * @param str
  ...
  /**
   * Localize the incoming string based on a resource bundle for the current locale, with a default
   * value to to return if localization fails.
   *
   * @param str
   *   the string or ID to localize
   * @param dflt
   ...
   * Text element). If the lookup fails for a given resource bundle (e.g. a null is returned), then
   * the <code>LiftRules.localizationLookupFailureNotice</code> function is called with the input
   * string and locale. The function is applied to the result/
   *
   * @param str
   *   the string or ID to localize
   * @param xform
   ...
   * the LiftRules.resourceNames and LiftRules.resourceBundleFactories variables. If you do not
   * define an entry for a particular key, we fall back to using Lift's core entries.
   *
   * @see
   *   LiftRules.resourceNames
   * @see
  ...
  /**
   * Get a localized string or return the original string. We first try your own bundle resources,
   * if that fails, we try Lift's core bundle.
   *
   * @param str
  ...
  /**
   * Attempt to localize and then format the given string. This uses the String.format method to
   * format the localized string. We first try your own bundle resources, if that fails, we try
   * Lift's core bundle.
   *
   * @param str
   *   the string to localize
  ...
  private def ?!(str: String, resBundle: List[ResourceBundle]): String = resBundle.flatMap(r =>
    tryo(r.getObject(str) match {
  ...
  /**
   * Returns the logical page_id of the current request. All RequestVars for a current page share
   * this id.
   */
  def renderVersion: String = RenderVersion.get

  /**
   * The URI of the current request (not re-written). The URI is the portion of the request URL
   * after the context path. For example, with a context path of "myApp", Lift would return the
   * following URIs for the given requests:
   *
   ...
   * <td>http://foo.com/myApp/item.html?id=42<td><td>/item.html</td> </tr> </table>
   *
   * If you want the full URI, including the context path, you should retrieve it from the
   * underlying HTTPRequest. You could do something like:
   *
   * <pre name="code" class="scala" > val fullURI = S.request.map(_.request.getRequestURI) openOr
   * ("Undefined") </pre>
   *
   ...
   * approaches would be:
   *
   * <pre name="code" class="scala" > ... try { // your code here S.redirectTo(...) } catch { case
   * e: Exception if !e.isInstanceOf[LiftFlowOfControlException] => ... } </pre>
   *
   * or
   *
   * <pre name="code" class="scala" > ... try { // your code here S.redirectTo(...) } catch { case
   * rse: LiftFlowOfControlException => throw rse case e: Exception => ... } </pre>
   *
   * @param where
  ...
  /**
   * Redirects the browser to a given URL and registers a function that will be executed when the
   * browser accesses the new URL. Otherwise the function is exactly the same as
   * S.redirectTo(String), which has example documentation. Note that if the URL that you redirect
   * to must be part of your web application or the function won't be executed. This is because the
   ...
   * approaches would be:
   *
   * <pre name="code" class="scala" > ... try { // your code here S.seeOther(...) } catch { case e:
   * Exception if !e.instanceOf[LiftFlowOfControlException] => ... } </pre>
   *
   * or
   *
   * <pre name="code" class="scala" > ... try { // your code here S.seeOther(...) } catch { case
   * rse: LiftFlowOfControlException => throw rse case e: Exception => ... } </pre>
   *
   * @param where
   *   The new URL to redirect to.
  ...
  /**
   * Redirects the browser to a given URL and registers a function that will be executed when the
   * browser accesses the new URL. Otherwise the function is exactly the same as S.seeOther(String),
   * which has example documentation. Note that if the URL that you redirect to must be part of your
   * web application or the function won't be executed. This is because the function is only
   * registered locally.
   *
   * @param where
   *   The new URL to redirect to.
  ...
  private[http] object oldNotices
      extends TransientRequestVar[Seq[(NoticeType.Value, NodeSeq, Box[String])]](Nil)

  /**
   * Initialize the current request session. Generally this is handled by Lift during request
   * processing, but this method is available in case you want to use S outside the scope of a
  ...
  /**
   * Creates `LAFuture` instance aware of the current request and session. Each `LAFuture` returned
   * by chained transformation method (e.g. `map`, `flatMap`) will be also request/session-aware.
   * However, it's important to bear in mind that initial session or request are not propagated to
   * chained methods. It's required that current execution thread for chained method has request or
   * session available in scope if reading/writing some data to it as a part of chained method
   * execution.
   */
  ...
  /**
   * Log a query for the given request. The query log can be tested to see if queries for the
   * particular page rendering took too long. The query log starts empty for each new request.
   * net.liftweb.mapper.DB.queryCollector is a method that can be used as a log function for the
   * net.liftweb.mapper.DB.addLogFunc method to enable logging of Mapper queries. You would set it
   * up in your bootstrap like:
   *
   * <pre name="code" class="scala" > import net.liftweb.mapper.DB import net.liftweb.http.S class
   * Boot { def boot { ... DB.addLogFunc(DB.queryCollector) ... } } </pre>
   *
   * Note that the query log is simply stored as a List and is not sent to any output by default. To
   * retrieve the List of query log items, use S.queryLog. You can also provide your own analysis
   * function that will process the query log via S.addAnalyzer.
   *
   * @see
   *   # queryLog
   * @see
  ...
  /**
   * Register a stateful snippet for a given class name. Only registers if the name is not already
   * set.
   */
  ...
  /**
   * Add a query analyzer (passed queries for analysis or logging). The analyzer methods are
   * executed with the request, total time to process the request, and the List of query log entries
  ...
  /**
   * You can wrap the handling of an HTTP request with your own wrapper. The wrapper can execute
   * code before and after the request is processed (but still have S scope). This allows for query
   * analysis, etc. See S.addAround(LoanWrapper) for an example. This version of the method takes a
   * list of LoanWrappers that are applied in order. This method is *NOT* intended to change the
   * generated HTTP request or to respond to requests early. LoanWrappers are there to set up and
   * take down state *ONLY*. The LoanWrapper may be called outside the scope of an HTTP request
   * (e.g., as part of an Actor invocation).
   *
   * @see
   ...
   * You can wrap the handling of an HTTP request with your own wrapper. The wrapper can execute
   * code before and after the request is processed (but still have S scope). This allows for query
   * analysis, etc. Wrappers are chained, much like servlet filters, so you can layer processing on
   * the request. As an example, let's look at a wrapper that opens a resource and makes it
   * available via a RequestVar, then closes the resource when finished:
   *
   * <pre name="code" class="scala" > import net.liftweb.http.{ ResourceVar,S } import
   * net.liftweb.util.LoanWrapper
   *
   * // Where "ResourceType" is defined by you object myResource extends
   * ResourceVar[ResourceType](...)
   *
   * class Boot { def boot { ... S.addAround( new LoanWrapper { def apply[T](f: => T) : T = {
   * myResource(... code to open and return a resource instance ...) f() // This call propagates the
   * request further down the "chain" for template processing, etc. myResource.is.close() // Release
   * the resource } } ) ... } } </pre> This method is *NOT* intended to change the generated HTTP
   * request or to respond to requests early. LoanWrappers are there to set up and take down state
   * *ONLY*. The LoanWrapper may be called outside the scope of an HTTP request (e.g., as part of an
   * Actor invocation).
   *
   * @see
   *   # addAround ( LoanWrapper )
  ...
  /**
   * Sets a HTTP response header attribute. For example, you could set a "Warn" header in your
   * response:
   *
   * <pre name="code" class="scala" > ... S.setHeader("Warn", "The cheese is old and moldy") ...
   * </pre>
   *
   * @see
   *   # getHeaders
  ...
  /**
   * Returns the currently set HTTP response headers as a List[(String, String)]. To retrieve a
   * specific response header, use S.getResponseHeader. If you want to get request headers (those
  ...
  /**
   * Returns the current set value of the given HTTP response header as a Box. If you want a request
   * header, use Req.getHeader or S.getRequestHeader.
   *
   * @param name
  ...
  def getResponseHeader(name: String): Box[String] = {
    Box.legacyNullTest(_responseHeaders.value).map(rh => Box(rh.headers.get(name))).openOr(Empty)
  }

  /**
   * Returns the current value of the given HTTP request header as a Box. This is really just a thin
   * wrapper on Req.header(String). For response headers, see S.getHeaders, S.setHeader, or
   * S.getHeader.
   *
   * @param name
   *   The name of the HTTP header to retrieve
   ...
   *   DocType
   */
  def getDocType: (Boolean, Box[String]) = Box.legacyNullTest(_responseHeaders.value).map(rh =>
   ...
   * content. If you're sending XHTML and this is set to true, you need to include the DocType in
   * your template.
   *
   * @see
   ...
   *
   * @param skip
   *   Set to <code>true</code> to prevent Lift from emitting a DocType in its response
   *
   * @see
   *   # skipDocType
   */
  ...
  /**
   * Adds a cleanup function that will be executed at the end of the request pocessing. Exceptions
   * thrown from these functions will be swallowed, so make sure to handle any expected exceptions
   ...
   *
   * @return
   *   the value from executing LiftRules.loggedInTest, or <code>false</code> if a test function is
   *   not defined.
   */
  ...
  /**
   * Functions that are mapped to HTML elements are, by default, garbage collected if they are not
   * seen in the browser in the last 10 minutes (defined in LiftRules.unusedFunctionsLifeTime). In
   * some cases (e.g., JSON handlers), you may want to extend the lifespan of the functions to the
   * lifespan of the session.
   *
   * @param span
   *   If <code>true</code>, extend the mapped function lifetime to the life of the session
   * @param f
   ...
   *
   * @return
   *   <code>true</code> if mapped functions will currently last the life of the session.
   */
  def functionLifespan_? : Boolean = _lifeTime.box openOr false

  /**
   * Get a list of current attributes. Each attribute item is a pair of (key,value). The key is an
   * Either that depends on whether the attribute is prefixed or not. If the attribute is prefixed,
   * the key is a Right((prefix, name)). If the attribute is unprefixed then the key is a
   * Left(name). For example, the following table shows how various tag attributes would be
   * represented:
   *
   * <table> <tr> <th>Snippet Tag</th> <th>Parsed attrs</th> </tr> <tr> <td>&lt;lift:MySnippet
   * testname="test" /&gt;</td> <td>List((Left("testname"), "test"))</td> </tr> <tr>
   * <td>&lt;lift:MySnippet anchor:name="test" /&gt;</td> <td>List((Right(("anchor", "name")),
   * "test"))</td> </tr> </table>
   *
   * <p>The prefixedAttrsToMap method provides a convenient way to retrieve only attributes with a
   * given prefix. The prefixedAttrsToMetaData method can be used to add attributes onto an XML
   * node</p>
   *
   * @see
  ...
  /**
   * Returns the S attributes that are prefixed by 'prefix' parameter as a Map[String, String] that
   * will be 'merged' with the 'start' Map
   *
   * @param prefix
  ...
  /**
   * <p>Returns the S attributes that are prefixed by 'prefix' parameter as a MetaData. The start
   * Map will be 'merged' with the Map resulted after prefix matching and the result Map will be
   * converted to a MetaData. The MetaData can be used to add attributes back onto XML elements via
   * Scala's '%' method. For example, if we wanted to add attributes prefixed with "anchor" to any
   * &lt;a&gt; elements we create, we could do something like:</p>
   *
  ...
  /**
   * Similar with prefixedAttrsToMetaData(prefix: String, start: Map[String, String]) but there is
   * no 'start' Map
   */
  def prefixedAttrsToMetaData(prefix: String): MetaData = prefixedAttrsToMetaData(prefix, Map.empty)

  /**
   * Converts a Map[String, String] into a MetaData instance. This can be used to add attributes to
   * an XML element based on a map of attribute->value pairs. See
  ...
  /**
   * Converts the S.attrs to a Map[String, String]. The key of the map depends on whether the
   * attribute is prefixed or not. Prefixed attributes have keys of the form "prefix:name", while
   * unprefixed attributes have keys of the form "name". If you only want attributes for a specific
   * prefix, use prefixedAttrsToMap.
   *
   * @see
   *   # prefixedAttrsToMap ( String )
   * @see
  ...
  /**
   * Converts S.attrs attributes to a MetaData object that can be used to add attributes to one or
   * more XML elements. Similar to prefixedAttrsToMetaData, except that it handles both prefixed and
   * unprefixed attributes. This version of the method will use all of the currently set attributes
   * from S.attrs. If you want to filter it, use the attrsToMetaData(String => Boolean) version,
   * which allows you to specify a predicate function for filtering. For example, if you want all of
   * the current attributes to be added to a div tag, you could do:
   *
   * <pre name="code" class="scala" > val myDiv = (<div> {...} </div>) % S.attrsToMetaData </pre>
   *
   * @return
   ...
   *   <code>true</code>, then the attribute is included in the MetaData.
   *
   * @see
   *   # attrsToMetaData
   */
   ...
   * more XML elements. Similar to prefixedAttrsToMetaData, except that it handles both prefixed and
   * unprefixed attributes. This version of the method will use all of the currently set attributes
   * from S.attrs. If you want to filter it, use the currentAttrsToMetaData(String => Boolean)
   * version, which allows you to specify a predicate function for filtering. For example, if you
   * want all of the current attributes to be added to a div tag, you could do:
   *
   * <pre name="code" class="scala" > val myDiv = (<div> {...} </div>) % S.attrsToMetaData </pre>
   *
   * @return
  ...
  /**
   * Find and process a template. This can be used to load a template from within some other Lift
   * processing, such as a snippet or view. If you just want to retrieve the XML contents of a
   * template, use Templates.apply.
   *
   * @param path
   ...
   * @return
   *   a Full Box containing the processed template, or a Failure if the template could not be
   *   found.
   *
   * @see
  ...
  /**
   * Evaluate a template for snippets. This can be used to run a template from within some other
   * Lift processing, such as a snippet or view.
   *
   * @param template
   ...
   *
   * <pre name="code" class="scala"> // Get a Box for the attribute: val myAttr = S.attr("test")
   * openOr "Not found"
   *
   * // Get an attribute or return a default value: val myAttr = S.attr("name", "Fred")
   *
   * // Apply a transform function on the attribute value, or return an Empty: val pageSize =
   * S.attr("count", _.toInt) openOr 20
   *
   * // There are also prefixed versions: val prefixedAttr = S.attr("prefix", "name") openOr "Not
   * found" </pre>
   *
   * Note that this uses the data held in S.attrs, which means that it will find attributes through
   * the entire snippet nesting stack. For example, given the snippets:
   *
   * <pre name="code" class="xml"> &lt;lift:MyStuff.snippetA foo="bar">
   * &lt;lift.MyStuff.snippetB>...&lt;/lift.MyStuff.snippetB> &lt;/lift:MyStuff.snippetA> </pre>
   *
   * Calling <code>S.attr("foo")</code> from snippetB will return <code>Full("bar")</code>.
   */
  object attr extends AttrHelper[Box] {
    ...
    /**
     * Returns the unprefixed attribute value as an Option[NodeSeq] for easy addition to the
     * attributes
     */
    def ~(key: String): Option[NodeSeq] = apply(key).toOption.map(Text(_))

    /**
     * Returns the prefixed attribute value as an Option[NodeSeq] for easy addition to the
     * attributes
     */
  ...
  /**
   * Sometimes, in the course of eager evaluation, it becomes necessary to clear attribtues so they
   * do not polute the eagerly evaluated stuff. When you need to clear the attributes, wrap your
   * code block in clearAttrs and have fun.
   *
   * @param f
  ...
  /**
   * Initialize the current request session if it's not already initialized. Generally this is
   * handled by Lift during request processing, but this method is available in case you want to use
   * S outside the scope of a request (standard HTTP or Comet).
   *
   * @param session
   *   the LiftSession for this request
   * @param f
   ...
   *
   * If you want a particular attribute, the S.currentAttr helper object simplifies things
   * considerably.
   */
  ...
  /**
   * Used to get an attribute by its name from the current snippet element. There are several means
   * to getting attributes:
   *
   * <pre name="code" class="scala"> // Get a Box for the attribute: val myAttr =
   * S.currentAttr("test") openOr "Not found"
   *
   * // Get an attribute or return a default value: val myAttr = S.currentAttr("name", "Fred")
   *
   * // Apply a transform function on the attribute value, or return an Empty: val pageSize =
   * S.currentAttr("count", _.toInt) openOr 20
   *
   * // There are also prefixed versions: val prefixedAttr = S.currentAttr("prefix", "name") openOr
   * "Not found" </pre>
   *
   * Note that this uses the data held in S.currentAttrs, which means that it will only find
   * attributes from the current snippet element. For example, given the snippets:
   *
   * <pre name="code" class="xml"> &lt;lift:MyStuff.snippetA foo="bar">
   * &lt;lift.MyStuff.snippetB>...&lt;/lift.MyStuff.snippetB> &lt;/lift:MyStuff.snippetA> </pre>
   *
   * Calling <code>S.currentAttr("foo")</code> from snippetB will return <code>Empty</code>.
   */
  ...
  /**
   * Returns the 'type' S attribute. This corresponds to the current Snippet's name. For example,
   * the snippet:
   *
  ...
  /**
   * The hostname to which the request was sent. This is taken from the "Host" HTTP header, or if
   * that does not exist, the DNS name or IP address of the server.
   */
  def hostName: String = request.map(_.hostName) openOr Req.localHostName

  /**
   * The host and path of the request up to and including the context path. This does not include
   * the template path or query string.
   */
  ...
  /**
   * Set the ignore snippet error mode. In this mode, any snippet failures (usually snippets not
   * being invocable) will be ignored and the original NodeSeq will be returned.
   *
   * This is useful if you want to do an initial pass of a page with a white-list of snippets, but
   * not run every snippet on the page.
   *
   * @param ignore
  ...
  /**
   * Associates a name with a snippet function 'func'. This can be used to change a snippet mapping
   * on a per-request basis. For example, if we have a page that we want to change behavior on based
   * on query parameters, we could use mapSnippet to programmatically determine which snippet
   * function to use for a given snippet in the template. Our code would look like:
   *
   * <pre name="code" class="scala" > import scala.xml.{ NodeSeq,Text } class SnipMap { def
   * topSnippet (xhtml : NodeSeq) : NodeSeq = { if (S.param("showAll").isDefined) {
   * S.mapSnippet("listing", listing) } else { S.mapSnippet("listing", { ignore => Text("") } ) }
   *
   * ... }
   *
   ...
   *
   * <pre name="code" class="scala" > &lt;lift:surround with="default" at="content"&gt; ...
   * &lt;p&gt;&lt;lift:SnipMap.topSnippet /&gt;&lt;/p&gt; &lt;p&gt;&lt;lift:listing /&gt;&lt;/p&gt;
   * &lt;/lift:surround&gt; </pre>
   *
   * Snippets are processed in the order that they're defined in the template, so if you want to use
   * this approach make sure that the snippet that defines the mapping comes before the snippet that
   * is being mapped. Also note that these mappings are per-request, and are discarded after the
  ...
  /**
   * The are times when it's helpful to define snippets for a certain call stack... snippets that
   * are local purpose. Use doWithSnippets to temporarily define snippet mappings for the life of f.
   */
  ...
  def generatePredictableFuncName: String = {
    ...
    "f" + prefix + "_" + Helpers.hashHex((new Exception).getStackTrace.toList.filter(
      notLiftOrScala).take(2).map(_.toString).mkString(","))
  }
  ...
  /**
   * Attaches to this uri and parameter that has function f associated with. When this request is
   * submitted to server the function will be executed and then it is automatically cleaned up from
   * functions caches.
   */
  ...
  /**
   * Wrap an AFuncHolder with the current snippet and Loc context so that for Ajax calls, the
   * original snippets, RequestVars and Loc (location) are populated
   *
   * @param f
  ...
  def render(xhtml: NodeSeq, httpRequest: HTTPRequest): NodeSeq = {
    ...
    if (inS.value) doRender(session.openOrThrowException("legacy code"))
      ...
      CurrentReq.doWith(req) {
        val ses: LiftSession = SessionMaster.getSession(
          httpRequest,
          Empty) match {
          case Full(ret) =>
            ret.fixSessionTime()
  ...
  /**
   * Maps a function that will be called with a parsed JValue and should return a JsCmd to be sent
   * back to the browser. Note that if the passed JSON does not parse, the function will not be
   * invoked.
   */
  def jsonFmapFunc[T](in: JValue => JsCmd)(f: String => T)(implicit
    ...
    addFunctionMap(
      name,
      SFuncHolder((s: String) => JsonParser.parseOpt(s).map(in) getOrElse JsCmds.Noop))
    f(name)
  }
  ...
  def error(vi: List[FieldError]): Unit = {
    p_notice ++= vi.map { i => (NoticeType.Error, i.msg, i.field.uniqueFieldId) }
  }
   ...
   */
  def appendNotices(list: Seq[(NoticeType.Value, NodeSeq, Box[String])]): Unit = {
    p_notice.is ++= list
  }
  ...
  def errors: List[(NodeSeq, Box[String])] =
    List(oldNotices.is, p_notice.is).flatMap(_.filter(_._1 == NoticeType.Error).map(n =>
      (n._2, n._3)))
  ...
  def notices: List[(NodeSeq, Box[String])] =
    List(oldNotices.is, p_notice.is).flatMap(_.filter(_._1 == NoticeType.Notice).map(n =>
      (n._2, n._3)))
  ...
  def warnings: List[(NodeSeq, Box[String])] =
    List(oldNotices.is, p_notice.is).flatMap(_.filter(_._1 == NoticeType.Warning).map(n =>
      (n._2, n._3)))
  ...
  implicit def tuple2FieldError(t: (FieldIdentifier, NodeSeq)): net.liftweb.util.FieldError =
  ...
  /**
   * If you bind functions (i.e. using SHtml helpers) inside the closure passed to callOnce, after
   * your function is invoked, it will be automatically removed from functions cache so that it
  ...
  /**
   * All functions created inside the oneShot scope will only be called once and their results will
   * be cached and served again if the same function is invoked
   */
  def oneShot[T](f: => T): T =
