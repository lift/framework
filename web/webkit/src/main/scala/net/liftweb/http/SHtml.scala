...
/**
 * The SHtml object defines a suite of XHTML element generator methods to simplify the creation of
 * markup, particularly with forms and AJAX.
 */
...
trait SHtml extends Loggable {
  ...
  /**
   * An attribute that can be applied to an element. Typically, this will be a key-value pair, but
   * there is a class of HTML5 attributes that should be similated in JavaScript.
   */
  ...
  /**
   * Any old attribute. You should not explicitly construct one of these, but rather use "name" ->
   * "value" and let the implicit conversion take care of making a BasicElemAttr.
   */
  ...
  /**
   * Build a JavaScript function that will perform an AJAX call based on a value calculated in
   * JavaScript
   *
   * @param jsCalcValue
   *   the JavaScript that will be executed on the client to calculate the value to be sent to the
   *   server
   * @param func
  ...
  /**
   * A wrapper around parseOpt for use in the jsonCall variants that will notify the developer on
   * the console when parseOpt is unsuccessful at parsing the JSON.
   */
  ...
  /**
   * Build a JavaScript function that will perform a JSON call based on a value calculated in
   * JavaScript. This method uses the Lift-JSON package rather than the old, slow, not-typed
   * JSONParser. This is the preferred way to do client to server JSON calls.
   *
   * @param jsCalcValue
   ...
   * JavaScript.
   *
   * The JSON generated by func will be returned to the client and passed as argument to the
   * javascript function specified in jsonContext.success
   *
   * @param jsCalcValue
   *   the JavaScript to calculate the value to be sent to the server
   * @param jsonContext
   ...
   *   -- the JavaScript to calculate the value to be sent to the server
   * @param ajaxContext
   *   -- the context defining the javascript callback functions and the response type
   * @param func
   *   -- the function to call when the data is sent
   *
   * @return
  ...
  private def ajaxCall_*(jsCalcValue: JsExp, func: AFuncHolder): (String, JsExp) =
    fmapFunc((func))(name =>
      (
        name,
        makeAjaxCall(JsRaw("'" + name + "=' + encodeURIComponent(" + jsCalcValue.toJsCmd + ")"))))
  ...
  private def ajaxCall_*(
    ...
    fmapFunc((func))(name =>
      (
        name,
        makeAjaxCall(
          JsRaw("'" + name + "=' + encodeURIComponent(" + jsCalcValue.toJsCmd + ")"),
          ajaxContext)))
   ...
   * @param func
   *   -- the function to execute when the button is pushed. Return Noop if nothing changes on the
   *   browser.
   * @param attrs
  ...
  /**
   * Memoize the NodeSeq used in apply() and then call applyAgain() in an Ajax call and you don't
   * have to explicitly capture the template
   */
  ...
  def ajaxButton(text: NodeSeq, jsExp: JsExp, func: String => JsCmd, attrs: ElemAttr*): Elem = {
    attrs.foldLeft(fmapFunc((SFuncHolder(func)))(name =>
      <button onclick={
        makeAjaxCall(JsRaw(name.encJs + "+'='+encodeURIComponent(" + jsExp.toJsCmd + ")")).toJsCmd +
          "; return false;"
      }>{text}</button>))((e, f) => f(e))
  }
  ...
  def jsonButton(
    ...
    attrs.foldLeft(jsonFmapFunc(func)(name =>
      <button onclick={
        makeAjaxCall(
          JsRaw(name.encJs + "+'='+ encodeURIComponent(JSON.stringify(" + jsExp.toJsCmd + "))"),
          ajaxContext).toJsCmd +
          "; return false;"
      }>{text}</button>))(_ % _)
  }
   ...
   *   -- the name/text of the button
   * @param jsFunc
   *   -- the user function that will be executed. This function will receive as last parameter the
   *   function that will actually do the ajax call. Hence the user function can decide when to make
   *   the ajax request.
   * @param func
   *   -- the function to execute when the button is pushed. Return Noop if nothing changes on the
   *   browser.
  ...
  def ajaxButton(text: NodeSeq, jsFunc: Call, func: () => JsCmd, attrs: ElemAttr*): Elem = {
    attrs.foldLeft(fmapFunc((func))(name =>
      <button onclick={deferCall(Str(name + "=true"), jsFunc).toJsCmd + "; return false;"}>{
        text
      }</button>))(_ % _)
  }
   ...
   *
   * Normally, the displayContents will be shown, with an "Edit" button. If the "Edit" button is
   * clicked, the field will be replaced with the edit form, along with an "OK" and "Cancel" button.
   * If the OK button is pressed, the form fields are submitted and the onSubmit function is called,
   * and then the displayContents are re-run to get a new display. If cancel is pressed then the
   * original displayContents are re-shown.
   *
   * Note that the editForm NodeSeq is wrapped inside of an <code>ajaxForm</code>, so it can be
   * comprised of normal (non-AJAX) <code>SHtml</code> form elements. For example:
   *
   * <pre name="code" class="scala"> ajaxEditable(Test("Click me"), SHtml.text("Edit me", s =>
   * println("Edited with " +s)), () => { println("submitted"); Noop }) </pre>
   */
  def ajaxEditable(
    ...
    def setAndSwap(show: String, showContents: => NodeSeq, hide: String): JsCmd =
      (SHtml.ajaxCall(
        Str("ignore"),
        { ignore: String => SetHtml(show, showContents) })._2.cmd & swapJsCmd(show, hide))
  ...
  private def ajaxCheckbox_*(
      ...
      attrs: ElemAttr*): Elem = {
    val raw =
      (funcName: String, value: String) => JsRaw("'" + funcName + "=' + " + value + ".checked")
    val key = formFuncName
  ...
  def ajaxSelectElem[T](
      options: Seq[T],
      default: Box[T],
      attrs: ElemAttr*)(onSubmit: T => JsCmd)(implicit f: PairStringPromoter[T]): Elem = {
    ajaxSelectObj[T](
      options.map(v => (v -> f(v))),
      default,
      onSubmit,
  ...
  /**
   * Create a select box based on the list with a default value and the function to be executed on
   * form submission with an additional selection that transforms select into an ajaxText allowing
   * the user to add a new select option
   *
   * @param options
   *   -- a list of value and text pairs (value, text to display)
   * @param default
  ...
  def ajaxEditableSelect(
    ...
    val attributes =
      if (attrs.contains(BasicElemAttr("id", id))) attrs else BasicElemAttr("id", id) +: attrs
    ...
    lazy val func: (String) => JsCmd = (select: String) => {
      ...
      if (select == textOpt) Replace(id, ajaxText("", text(_), attributes: _*)) & Focus(id)
      else f(select)
    }
  ...
  private def ajaxSelect_*(
      ...
      attrs: ElemAttr*): Elem = {
    val raw = (funcName: String, value: String) =>
      JsRaw("'" + funcName + "=' + encodeURIComponent(" + value + ".options[" + value + ".selectedIndex].value)")
    val key = formFuncName
  ...
  /**
   * Build a swappable visual element. If the shown element is clicked on, it turns into the hidden
   * element and when the hidden element blurs, it swaps into the shown element.
   */
  ...
  def swappable(shown: Elem, hidden: String => Elem): Elem = {
    ...
    (<span>{
      rs % ("onclick" -> (ui.hide(sid).toJsCmd + ";" + ui.show(hid).toJsCmd + "; return false;"))
  ...
  /**
   * If you want to update the href of an &lt;a> tag, this method returns a function that mutates
   * the href by adding a function that will be executed when the link is clicked: <code> "#my_link"
   * #> SHtml.hrefFunc(() => println("howdy")) </code>
   */
  ...
  /**
   * Create something that's bindable to an event attribute and when the event happens, the command
   * will fire: <code> "input [onblur]" #> SHtml.onEvent(s => Alert("Thanks: "+s)) </code>
   */
  def onEvent(func: String => JsCmd): GUIDJsExp =
    ajaxCall(JsRaw("this.value"), func)

  /**
   * Create something that's bindable to an event attribute and guarded by a confirm dialog. The fn
   * passed in will only execute if the user clicks the "OK" button in the confirm dialog. Useful
   * for things like delete buttons that you want a confirmation on.
   *
   * <code> ".deleteBtn [onclick]" #> SHtml.onEventIf("Are you sure you want to do that?", s =>
   * deleteUser()) </code>
   */
   ...
   * Returns a NodeSeq => NodeSeq that will add the events to all the Elements <code> ":text" #>
   * SHtml.onEvents("onchange", "onblur")(s => Alert("yikes "+s)) </code>
   */
  def onEvents(event: String, events: String*)(func: String => JsCmd): NodeSeq => NodeSeq = {
  ...
  /**
   * Execute the function when the form is submitted. This method returns a function that can be
   * applied to form fields (input, button, textarea, select) and the function is executed when the
  ...
  /**
   * Execute the String function when the form is submitted. This method returns a function that can
   * be applied to form fields (input, button, textarea, select) and the function is executed when
   * the form containing the field is submitted.
   */
  ...
  /**
   * Execute the List[String] function when the form is submitted. This method returns a function
   * that can be applied to form fields (input, button, textarea, select) and the function is
   * executed when the form containing the field is submitted.
   */
  def onSubmitList(func: List[String] => Any): NodeSeq => NodeSeq =
    onSubmitImpl(func: AFuncHolder)

  /**
   * Execute the Boolean function when the form is submitted. This method returns a function that
   * can be applied to form fields (input, button, textarea, select) and the function is executed
      ...
      S._formGroup.doWith(fgSnap) {
        ...
        def runNodes(in: NodeSeq): NodeSeq =
          in.flatMap {
            ...
            // submit
            case e: Elem
                if e.label == "input" && e.attribute("type").map(_.text) == Some("submit") =>
              _formGroup.is match {
                case Empty =>
                  formGroup(1)(fmapFunc(func) { dupWithName(e, _) })
                case _ => fmapFunc(func) { dupWithName(e, _) }
  ...
  /**
   * Generate an input field with type email. At some point, there will be graceful fallback for
   * non-HTML5 browsers. FIXME
   */
  def email(value: String, func: String => Any, attrs: ElemAttr*): Elem =
    email_*(value, SFuncHolder(func), attrs: _*)

  /**
   * Generate an email input element for the Settable. At some point there will be graceful fallback
  ...
  /**
   * Generate an input field with type url. At some point, there will be graceful fallback for
   * non-HTML5 browsers. FIXME
   */
  ...
  /**
   * Generate a url input element for the Settable. At some point there will be graceful fallback
   * for non-HTML5 browsers. FIXME
   */
  def url(
      settable: Settable { type ValueType = String },
  ...
  /**
   * Generate an input field with type number. At some point, there will be graceful fallback for
   * non-HTML5 browsers. FIXME
   */
  def number(
      value: Int,
  ...
  /**
   * Generate a number input element for the Settable. At some point there will be graceful fallback
   * for non-HTML5 browsers. FIXME
   */
  def number(
  ...
  /**
   * Generate an input field with type number. It allows for Double if your step is for example: 0.1
   * At some point, there will be graceful fallback for non-HTML5 browsers. FIXME
   */
  def number(
      value: Double,
      func: Double => Any,
  ...
  /**
   * Generate a number input element for the Settable. It allows for Double if your step is for
   * example: 0.1 At some point there will be graceful fallback for non-HTML5 browsers. FIXME
   */
  def number(
  ...
  /**
   * Generate an input field with type range. At some point, there will be graceful fallback for
   * non-HTML5 browsers. FIXME
   */
  def range(
      value: Int,
  ...
  /**
   * Generate a range input element for the Settable. At some point there will be graceful fallback
   * for non-HTML5 browsers. FIXME
   */
  def range(
      settable: Settable { type ValueType = Int },
  ...
  /**
   * Add appropriate attributes to an input type="submit" or button element to make it submit an
   * ajaxForm correctly and return a JsCmd to the client. Note that the key difference between this
   * and ajaxSubmit is that ajaxSubmit returns a complete input type="submit" element, while
   * ajaxOnSubmit applies the right attributes to any input type="submit" *or* button element.
   *
   * Example:
   *
  ...
  /**
   * Takes a form and wraps it so that it will be submitted via AJAX. This also takes a parameter
   * for script code that will be executed after the form has been submitted.
   *
   * @param body
   *   The form body. This should not include the &lt;form&gt; tag.
   * @param postSubmit
  ...
  /**
   * Submits a form denominated by a formId and execute the func function after form fields
   ...
   *   -- the default value (or Empty if no default value)
   * @param attrs
   *   -- the attributes to append to the resulting Elem, these may be name-value pairs (static
   *   attributes) or special HTML5 ElemAtts
   * @param onSubmit
   *   -- the function to execute on form submission
   * @param f
   *   -- the function that converts a T to a Display String.
   */
  def selectElem[T](options: Seq[T], default: Box[T], attrs: ElemAttr*)(onSubmit: T => Any)(implicit
  ...
  /**
   * Create a select box based on the list with a default value and the function to be executed on
   * form submission. No check is made to see if the resulting value was in the original list. For
   * use with DHTML form updating.
   *
   * @param opts
   *   -- the options. A list of value and text pairs
   * @param deflt
  ...
  /**
   * Create a multiple select box based on the list with a default value and the function to be
   * executed on form submission. No check is made to see if the resulting value was in the original
   * list. For use with DHTML form updating.
   *
   * @param opts
   *   -- the options. A list of value and text pairs
   * @param deflt
  ...
  private def ajaxUntrustedSelect_*(
      ...
      attrs: (String, String)*): Elem = {
    val raw = (funcName: String, value: String) =>
      JsRaw("'" + funcName + "=' + this.options[" + value + ".selectedIndex].value")
    val key = formFuncName
   ...
   *   -- the function to execute on form submission
   */
  def multiSelectElem[T](
      options: Seq[T],
      default: Seq[T],
      attrs: ElemAttr*)(onSubmit: List[T] => Any)(implicit f: PairStringPromoter[T]): Elem = {
    multiSelectObj[T](
      options.map(v => SelectableOption(v, f(v))),
      default,
      onSubmit,
  ...
  def radioElem[T](
      opts: Seq[T],
      deflt: Box[T],
      attrs: ElemAttr*)(onSubmit: Box[T] => Any): ChoiceHolder[T] = {
  ...
  /**
   * Generate a CSS transform that will bind to a template containing radio buttons without
   * overwriting the radio buttons in the template with entirely new elements.
   *
  ...
  def radioCssSel[T](
      initialValue: Box[T],
      onSubmit: Box[T] => Any)(cssSelToValue: (String, T)*): CssSel = {
  ...
  /**
   * Defines a form element for a file upload that will call the specified function when the file is
   * uploaded if the file size is greater than zero. Note that in order to use the fileUpload
   * element you need to specify the multipart attribute on your snippet tag:
   *
   * <pre name="code" class="xml"> &lt;lift:Some.snippet form="POST" multipart="true"> ...
   * &lt;/lift:Some.snippet> </pre>
   */
  ...
  final case class ChoiceHolder[T](items: Seq[ChoiceItem[T]]) {
    ...
    /**
     * Retrieve the ChoiceItem that has the given key, throwing NoSuchElementException if there is
     * no matching ChoiceItem
     */
    ...
    /**
     * Generate a simple form by calling ChoiceItem.htmlize on each ChoiceItem and concatenating the
     * resulting HTML
     */
  ...
  object ChoiceHolder {
    ...
    var htmlize: ChoiceItem[_] => NodeSeq =
      c => (<span>{c.xhtml}&nbsp;{c.key.toString}<br/> </span>)
  }
  ...
  private def setId(in: Box[String]) =
    in match { case Full(id) => new UnprefixedAttribute("id", Text(id), Null); case _ => Null }

  /**
   * Generate a ChoiceHolder of possible checkbox type inputs that calls back to the given function
    ...
    fmapFunc {
      ...
      ChoiceHolder(possible.toList.zipWithIndex.map { possibleChoice =>
        ChoiceItem(
          possibleChoice._1,
          attrs.foldLeft(
            <input type="checkbox" name={name} value={possibleChoice._2.toString}/>)(_ % _) %
  ...
  /**
   * Defines a new checkbox set to {@code value} and running {@code func} when the checkbox is
   * submitted.
   */
  ...
  /**
   * Defines a new checkbox set to {@code value} and running {@code func} when the checkbox is
   * submitted. Has an id of {@code id}.
   */
  def checkbox_id(
      value: Boolean,
      func: Boolean => Any,
  ...
  def checkbox_*(
      ...
      attrs: ElemAttr*): NodeSeq = {
    fmapFunc(func)(name =>
      (attrs.foldLeft(<input type="checkbox" name={name} value="true"/>)(_ % _) % checked(
        value) % setId(id)) ++
...
class JsContext(override val success: Box[String], override val failure: Box[String])
    extends AjaxContext(success, failure, AjaxType.JavaScript)

class JsonContext(override val success: Box[String], override val failure: Box[String])
...
/**
 * Mix this trait into a snippet class so that you have a convenient value to redirect back to
 * (whence). When you're done with the snippet, <code>S.redirectTo(whence)</code>
 */
trait Whence {
...
/**
 * Memoize the CSS Selector Transform and the most recent NodeSeq sent to the NodeSeq => NodeSeq so
 * that when applyAgain() is called, the NodeSeq most recently used in apply() is used.
 */
...
/**
 * A mechanism to memoize a transformation and then re-use the most recent html and ID to redraw the
 * content or even use an Ajax call to update the content
 */
...
object NodeSeqFuncOrSeqNodeSeqFunc {
  ...
  implicit def promoteSeqNodeSeq(in: Seq[NodeSeq => NodeSeq]): NodeSeqFuncOrSeqNodeSeqFunc =
    SeqNodeSeqFunc(in)
}
...
/**
 * A long time ago, Lift was going to track every function/GUID combination vended to a web page
 * with extreme granularity. This meant that for every function/GUID vended, Lift would put that
 * GUID in an attribute associated with the element on the page. In order to capture the GUIDs, some
 * methods like SHtml.ajaxCall() returned a Tuple containing the GUID and the JsExp. This caused
 * confusion and ugly code. So, the GUIDJsExp came into being. Basically, it's backward compatible
 * with the Tuple (String, JsExp), but it functions like a JsExp (although you don't even have to
 * call .toJsCmd because the toString method returns the expresion itself). It should make the
 * ajaxCall()._2.toJsCmd thing into ajaxCall().
 */
