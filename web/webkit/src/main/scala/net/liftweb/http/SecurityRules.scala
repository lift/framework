...
final case class HttpsRules(
    /**
     * When set, the duration of the requirement that HTTPS be used for this site. It's usually a
     * good idea for this to be a high number. If unset, HTTPS is not required when interacting with
     * this site.
     */
  ...
  /**
   * Returns the headers implied by this set of HTTPS rules. If `enforce` is false, returns nothing.
   */
...
object HttpsRules {
  ...
  /**
   * Creates a restrictive set of HTTPS rules requiring HTTPS for 365 days and including subdomains
   * in the requirement.
   */
...
/**
 * Base trait for content source restrictions. These are different ways of restricting where
 * contents of various types are allowed to come from, in conjunction with `ContentSecurityPolicy`
 * categories.
 */
...
sealed trait GeneralSourceRestriction extends JavaScriptSourceRestriction
...
object ContentSourceRestriction {
  ...
  /**
   * Indicates content from the given host path is allowed. See the `Content-Security-Policy` spec's
   * [[https://www.w3.org/TR/CSP/#source-list-path-patching matching rules for `host-source`]] for
   * more about what this can look like.
   *
   * Example:
   * {{{
  ...
  /**
   * Indicates inline content on the page is allowed to be interpreted. It is highly recommended
   * that this not be used, as it exposes your application to cross-site scripting and other
   * vulnerabilities.
   *
   * If not specified for JavaScript, JavaScript `on*` event handler attributes, `<script>`
   * elements, and `javascript:` URIs will not be executed by a browser that supports content
   * security policies.
   *
   * If not specified for stylesheets, `<style>` elements and inline `style` attributes will not be
   * read by a browser that supports content security policies.
   */
  ...
  /**
   * Indicates `eval` and related functionality can be used. Some of Lift's functionality, including
   * `idMemoize` and comet handling, relies on eval, so not including this in your script sources
   * will mean you won't be able to use those.
   *
   * If not specified for JavaScript, invoking `eval`, the `Function` constructor, or
   * `setTimeout`/`setInterval` with a string parameter will all throw security exceptions in a
...
/**
 * Specifies a
 * `[[https://developer.mozilla.org/en-US/docs/Web/Security/CSP Content-Security-Policy]] ` for this
 * site. This will be sent to the client in a `Content-Security-Policy` header when responses are
 * returned from Lift.
 *
 * In development mode, content security policy violations are only reported if the browser supports
 * them, not enforced. In all other modes, content security policy violations are enforced if the
 * browser supports them.
 *
 * Note that the `X-Webkit-CSP` header is NOT specified, due to potentially-broken behavior in iOS 5
 * and 5.1. This means iOS 6/6.1 will not receive a content security policy that it can understand.
 * See the
 * [[http://caniuse.com/#feat=contentsecuritypolicy caniuse page on content security policy]] for
 * more.
 *
 * @param defaultSources
 *   A list of default source restrictions; if one of the other sources parameters is empty, the
 *   default sources will apply instead.
 ...
 *   `[[ContentSourceRestriction.UnsafeEval UnsafeEval]]` source restrictions, though these are
 *   strongly discouraged.
 * @param styleSources
 *   A list of source restrictions for loading styles. Also accepts the
 *   `[[ContentSourceRestriction.UnsafeInline UnsafeInline]]` source, though it is strongly
 *   discouraged.
 * @param reportUri
 *   The URI where any violation of the security policy will be reported. You can set the function
 *   that handles these violations in `[[LiftRules.contentSecurityPolicyViolationReport]]`. By
 *   default, reported to `[[ContentSecurityPolicy.defaultReportUri]]`.
 *
...
object ContentSecurityPolicy {
  ...
  /**
   * The default URI where security policy violations will be reported. This URI is under Lift's URI
   * namespace, at `[[LiftRules.liftPath]]`.
   */
  ...
  /**
   * Creates a restrictive content security policy that disallows images from all sources except the
   * page's origin.
   *
   * Note that the default content security policy restricts all other resources to the same origin,
   * but allows images from any source; the secure one only differs because it adds restrictions to
   * the image sources.
   */
...
object ContentSecurityPolicyViolation extends LazyLoggable {
  private[this] implicit val formats: net.liftweb.json.DefaultFormats.type = DefaultFormats
...
sealed trait FrameRestrictions {
   ...
   *
   * Because of how frame restrictions are handled, if enforcement is turned off, no headers are
   * generated.
   */
...
object FrameRestrictions {
  ...
  /**
   * Allows other pages from the same origin as the one being served to embed this page in a frame.
   */
 ...
 * `Content-Security-Policy` is restricted to the current domain for everything except images, which
 * are accepted from any domain. Additionally, served pages can only be embedded in other frames
 * from the current domain.
 *
 * You can use `[[SecurityRules.secure]]` to enable more restrictive, but also more secure,
 * defaults.
 *
 * @param enforceInDevMode
 *   If true, security policies and HTTPS rules are enforced in dev mode in addition to
 *   staging/pilot/production/etc.
 * @param logInDevMode
 *   If true, dev mode violations of security policies are logged by default. Note that if you
 *   override `[[LiftRules.contentSecurityPolicyViolationReport]]` or otherwise change the default
 *   Lift policy violation handling behavior, it will be up to you to handle this property as
 *   desired.
 */
...
object SecurityRules {
   ...
   *
   * To tweak any of these settings, use the `SecurityRules` constructor directly.
   */
  def secure = {
    apply(
