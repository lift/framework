...
object SessionMaster extends LiftActor with Loggable {
  ...
  /**
   * If you have a rule other than <pre>Box !! req.request.remoteAddress</pre> for calculating the
   * remote address, change this function
   */
  @volatile var getIpFromReq: Req => Box[String] = req => Box !! req.request.remoteAddress

  /**
   * A list of functions that are run every 10 seconds. The first param is map containing the
   * session ID and the sessions. The second param is a function to call to destroy the session.
   */
  ...
  /**
   * End comet long polling for all sessions. This allows a clean reload of Nginx because Nginx
   * children stick around for long polling.
   */
  ...
  def getSession(id: String, otherId: Box[String]): Box[LiftSession] = lockAndBump {
    val dead =
      killedSessions.containsKey(id) || (otherId.map(killedSessions.containsKey(_)) openOr false)
  ...
  /**
   * Put an Actor in this list and the Actor will receive a message every 10 seconds with the
   * current list of sessions: SessionWatcherInfo
   */
  ...
  def getSession(req: HTTPRequest, otherId: Box[String]): Box[LiftSession] =
    lockAndBump {
      otherId.flatMap(a => Box !! nsessions.get(a)) or req.sessionId.flatMap(id =>
    ...
    f.map {
      s =>
        nsessions.put(
          ...
          SessionInfo(s.session, s.userAgent, s.ipAddress, s.requestCnt + 1, millis))
  ...
  def addSession(
    ...
    lockAndBump {
      Full(SessionInfo(
        ...
        ipAddress,
        -1,
        0L)) // bumped twice during session creation.  Ticket #529 DPP
    }
