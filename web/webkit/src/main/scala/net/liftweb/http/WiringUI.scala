...
object WiringUI {
  ...
  /**
   * Given a NodeSeq, a Cell and a function that can generate a NodeSeq => NodeSeq from the cell's
   * value, register the postPageJavaScript that will update the element with a new value.
   *
   * @param in
  ...
  /**
   * Given a Cell and a function that can generate a NodeSeq => NodeSeq from the cell's value,
   * return a function that takes a NodeSeq and registers the postPageJavaScript that will update
   ...
   *
   * @return
   *   a function that mutates NodeSeq (an id attribute may be added if there's none already
   *   defined)
   */
   ...
   *
   * @param jsEffect
   *   a function that wraps the SetHtml JsCmd so you can, for example, fade out the old value, set
   *   the new value and fade it in. The first parameter is the id of the element, the second is a
   *   flag that's true if this is the first time the element is being rendered (you might want to
   *   skip effects for the inital page load), and the third parameter is the SetHtml JavaScript
   *   code.
   *
   * @param f
   *   the function that performs the drawing
  ...
  def apply[T](
      in: NodeSeq,
      cell: Cell[T],
      jsEffect: (String, Boolean, JsCmd) => JsCmd)(f: T => NodeSeq => NodeSeq): NodeSeq =
    toNode(in, cell, jsEffect)((t, ns) => f(t)(ns))

  /**
   * Given a Cell and a function that can generate a NodeSeq => NodeSeq from the cell's value,
   * return a function that with a NodeSeq will register the postPageJavaScript that will update the
  ...
  def apply[T](
      cell: Cell[T],
      jsEffect: (String, Boolean, JsCmd) => JsCmd)(f: T => NodeSeq => NodeSeq): NodeSeq => NodeSeq =
    in => toNode(in, cell, jsEffect)((t, ns) => f(t)(ns))

  /**
   * Given a NodeSeq, a Cell and a function that can generate a NodeSeq from the cell's value and
   * the template value, register the postPageJavaScript that will update the element with a new
   * value.
   *
   * @param in
   *   the NodeSeq that contains the view markup
  ...
  /**
   * Given a NodeSeq, a Cell register the postPageJavaScript that will update the element with a new
   * value.
   *
   * @param in
   *   the NodeSeq that contains the view markup
   ...
   *
   * @return
   *   a function that will mutate the NodeSeq (an id attribute may be added if there's none already
   *   defined)
   */
  ...
  /**
   * Given a Cell and a function that can generate a NodeSeq from the cell's value and the template
   * value, register the postPageJavaScript that will update the element with a new value.
   *
   * @param cell
   *   the cell to associate with
  ...
  /**
   * Associate a Cell and a function that converts from the cell's value to a JavaScript command to
   * be sent to the browser as part of the page's post-processing.
   *
   * @param cell
