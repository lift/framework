...
/**
 * All http authentication methods must implement these methods. The most important method to note
 * here is the verified_? partial function as this is what is used to then determine if the response
 * specified in the boot dispatcher is used or its a 401 response.
 */
...
/**
 * Methods that are specific to HTTP basic are defined here. The methods from the parent trait are
 * implemented to decode the Base64 encoded input from the http client.
 */
case class HttpBasicAuthentication(realmName: String)(func: PartialFunction[
  (String, String, Req),
  Boolean]) extends HttpAuthentication {
  def credentials(r: Req): Box[(String, String)] = {
    header(r).flatMap(auth => {
      val decoded =
        new String(Base64.decodeBase64(auth.substring(6, auth.length).getBytes)).split(":").toList
      decoded match {
...
case class HttpDigestAuthentication(realmName: String)(func: PartialFunction[
  (String, Req, (String) => Boolean),
  Boolean]) extends HttpAuthentication with Loggable {
   ...
   *
   * A useful usability would be to return something like "5 seconds" where seconds function is
   * defined in TimeHelpers. The default value returned is 30 seconds.
   */
  ...
  private def validate(clientAuth: DigestAuthentication)(password: String): Boolean = {
    val ha1 = hexEncode(
      md5((clientAuth.userName + ":" + clientAuth.realm + ":" + password).getBytes("UTF-8")))
