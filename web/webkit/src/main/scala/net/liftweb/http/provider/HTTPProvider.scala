...
/**
 * Implement this trait in order to integrate Lift with other underlaying web containers. Not
 * necessarily JEE containers.
 */
trait HTTPProvider {
  ...
  protected def bootLift(loader: Box[String]): Unit = {
    try {
      val b: Bootable = loader.map(b =>
        Class.forName(
          b).getDeclaredConstructor().newInstance().asInstanceOf[Bootable]) openOr DefaultBootstrap
      preBoot()
      b.boot()
    } catch {
  ...
  /**
   * Tests if a request should be handled by Lift or passed to the container to be executed by other
   * potential filters or servlets.
   */
