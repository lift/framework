...
trait RestHelper extends LiftRules.DispatchPF {
  ...
  /**
   * Will the request accept a JSON response? Yes if the Accept header contains "text/json",
   * "application/json" or the Accept header is missing or contains "star/star" and the suffix is
   * "json". Override this method to provide your own logic. If there's no suffix, no accepts (or
   * it's "star/star") then look to either the Content-Type header or the defaultGetAsJson flag
   */
   ...
   * By default, yes, override to change the behavior.
   */
  protected def defaultGetAsJson: Boolean = true

  /**
   * If the headers and the suffix say nothing about the response type, should we default to XML. By
   * default, no, override to change the behavior.
   */
  ...
  /**
   * If there are additional custom rules (e.g., looking at query parameters) you can override this
   * method which is consulted if the other rules in jsonResponse_? fail
   */
  protected def suplimentalJsonResponse_?(in: Req): Boolean = false

  /**
   * Will the request accept an XML response? Yes if the Accept header contains "application/xml" or
   * "text/xml" or the Accept header is missing or contains "star/star" and the suffix is "xml".
   * Override this method to provide your own logic. If there's no suffix, no accepts (or it's
   * "star/star") then look to either the Content-Type header or the defaultGetAsXml flag.
   */
  ...
  /**
   * If there are additional custom rules (e.g., looking at query parameters) you can override this
   * method which is consulted if the other rules in xmlResponse_? fail
   */
  protected def suplimentalXmlResponse_?(in: Req): Boolean = false

  /**
   * A trait that defines the TestReq extractor. Is the request something that expects JSON, XML in
   * the response. Subclass this trait to change the behavior
   */
  protected trait TestReq {

    /**
     * Test to see if the request is expecting JSON, XML in the response. The path and a Tuple2
     * representing RequestType and the Req instance are extracted.
     */
  ...
  /**
   * A trait that defines the TestGet extractor. Is the request a GET and something that expects
   * JSON or XML in the response. Subclass this trait to change the behavior
   */
  protected trait TestGet {

    /**
     * Test to see if the request is a GET and expecting JSON in the response. The path and the Req
  ...
  /**
   * A trait that defines the TestDelete extractor. Is the request a DELETE and something that
   * expects JSON or XML in the response. Subclass this trait to change the behavior
   */
  protected trait TestDelete {

    /**
     * Test to see if the request is a DELETE and expecting JSON or XML in the response. The path
     * and the Req instance are extracted.
     */
    def unapply(r: Req): Option[(List[String], Req)] =
  ...
  /**
   * A trait that defines the TestPost extractor. Is the request a POST, has JSON or XML data in the
   * post body and something that expects JSON or XML in the response. Subclass this trait to change
   * the behavior
   */
  protected trait TestPost[T] {

    /**
     * Test to see if the request is a POST, has JSON data in the body and expecting JSON in the
     * response. The path, JSON Data and the Req instance are extracted.
     */
  ...
  /**
   * A trait that defines the TestPatch extractor. Is the request a PATCH, has JSON or XML data in
   * the post body and something that expects JSON or XML in the response. Subclass this trait to
   * change the behavior
   */
  protected trait TestPatch[T] {

    /**
     * Test to see if the request is a PATCH, has JSON data in the body and expecting JSON in the
     * response. The path, JSON Data and the Req instance are extracted.
     */
    def unapply(r: Req): Option[(List[String], (T, Req))] =
  ...
  /**
   * a trait that extracts the JSON body from a request It is composed with a TestXXX to get the
   * correct thing for the extractor
   */
  ...
  /**
   * a trait that extracts the XML body from a request It is composed with a TestXXX to get the
   * correct thing for the extractor
   */
  ...
  /**
   * An extractor that tests the request to see if it's a GET and if it is, the path and the request
   * are extracted. It can be used as:<br/> <pre>case "api" :: id :: _ Get req => ...</pre><br/>
   * or<br/> <pre>case Get("api" :: id :: _, req) => ...</pre><br/> *
   */
  ...
  /**
   * An extractor that tests the request to see if it's a POST and if it is, the path and the
   * request are extracted. It can be used as:<br/> <pre>case "api" :: id :: _ Post req =>
   * ...</pre><br/> or<br/> <pre>case Post("api" :: id :: _, req) => ...</pre><br/>
   */
  ...
  /**
   * An extractor that tests the request to see if it's a PUT and if it is, the path and the request
   * are extracted. It can be used as:<br/> <pre>case "api" :: id :: _ Put req => ...</pre><br/>
   * or<br/> <pre>case Put("api" :: id :: _, req) => ...</pre><br/>
   */
  ...
  /**
   * An extractor that tests the request to see if it's a PATCH and if it is, the path and the
   * request are extracted. It can be used as:<br/> <pre>case "api" :: id :: _ Patch req =>
   * ...</pre><br/> or<br/> <pre>case Patch("api" :: id :: _, req) => ...</pre><br/>
   */
  ...
  /**
   * An extractor that tests the request to see if it's a DELETE and if it is, the path and the
   * request are extracted. It can be used as:<br/> <pre>case "api" :: id :: _ Delete req =>
   * ...</pre><br/> or<br/> <pre>case Delete("api" :: id :: _, req) => ...</pre><br/>
   */
  ...
  /**
   * An extractor that tests the request to see if it's an OPTIONS and if it is, the path and the
   * request are extracted. It can be used as:<br/> <pre>case "api" :: id :: _ Options req =>
   * ...</pre><br/> or<br/> <pre>case Options("api" :: id :: _, req) => ...</pre><br/> *
   */
  ...
  /**
   * Serve a given request by determining the request type, computing the response and then
   * converting the response to the given type (e.g., JSON or XML).<br/><br/>
   * @param selection
   *   -- a function that determines the response type based on the Req.
   * @param pf
   *   -- a PartialFunction that converts the request to a response type (e.g., a case class that
   *   contains the response).
   * @param cvt
   *   -- a function that converts from the response type to a the appropriate LiftResponse based on
   *   the selected response type.
   */
  protected def serveType[
      T,
      SelectType](selection: Req => BoxOrRaw[SelectType])(pf: PartialFunction[
    Req,
    BoxOrRaw[T]])(implicit cvt: PartialFunction[(SelectType, T, Req), LiftResponse]): Unit = {
    serve(
   ...
   * @param cvt
   *   -- convert the intermediate response to a LiftResponse based on the request being for XML or
   *   JSON. If T is JsonXmlAble, there are built-in converters. Further, you can return auto(thing)
   *   and that will invoke built-in converters as well. The built-in converters use Lift JSON's
   *   Extraction.decompose to convert the object into JSON and then Xml.toXml() to convert to XML.
   */
  protected def serveJx[T](pf: PartialFunction[Req, BoxOrRaw[T]])(implicit cvt: JxCvtPF[T]): Unit =
   ...
   *
   * @param pf
   *   -- a Partial Function that converts the request into Any (note that the response must be
   *   convertable into JSON vis Lift JSON Extraction.decompose
   */
  protected def serveJxa(pf: PartialFunction[Req, BoxOrRaw[Any]]): Unit =
    serveType(jxSel)(pf)(new PartialFunction[
      (
          JsonXmlSelect,
          Any,
          Req),
  ...
  /**
   * A trait that defines the TestPut extractor. Is the request a PUT, has JSON or XML data in the
   * put body and something that expects JSON or XML in the response. Subclass this trait to change
   * the behavior
   */
  protected trait TestPut[T] {

    /**
     * Test to see if the request is a PUT, has JSON or XML data in the body and expecting JSON or
     * XML in the response. The path, Data and the Req instance are extracted.
     */
    def unapply(r: Req): Option[(List[String], (T, Req))] =
  ...
  /**
   * Turn T into the return type expected by DispatchPF as long as we can convert T to a
   * LiftResponse.
   */
  ...
  /**
   * Turn a Box[T] into the return type expected by DispatchPF. Note that this method will return
   * messages from Failure() and return codes and messages from ParamFailure[Int[(msg, _, _, code)
   */
  ...
  /**
   * Turn a () => Box[T] into the return type expected by DispatchPF. Note that this method will
   * return messages from Failure() and return codes and messages from ParamFailure[Int[(msg, _, _,
   * code)
   */
  ...
  /**
   * Override this method to create an AppXmlResponse with the mime type application/xml rather then
   * text/xml
   */
  ...
  /**
   * Allows you to use >> after a path list to handle all the cases where you have a prefix for a
   * series of differ suffixes with the same path prefix. For example: <code> serve("foo" / "bar" >>
   * { case baz :: Nil Post _ => ... case Nil Get _ => ... }) </code>
   */
  protected implicit def listToServeMagic(in: List[String]): ListServeMagic = new ListServeMagic(in)

  /**
   * Take an original piece of JSON (most probably, JObject and replace all the JFields with those
   * in toMerge
   */
...
final class ListServeMagic(list: List[String]) {
  ...
  /**
   * Used in conjunction with RestHelper.serveJx to prefix the pattern matched path. For example:
   * <code> serveJx[Foo]("foo" / "bar" prefixJx { case FindBaz(baz) :: Nil Get _ => baz }) </code>
   */
  ...
  /**
   * Used in conjunction with RestHelper.serveJx to prefix the pattern matched path. For example:
   * <code> serve("foo" / "bar" prefix { case FindBaz(baz) :: Nil GetJson _ => baz: JValue case
   * FindBaz(baz) :: Nil GetXml _ => baz: Node }) </code>
   */
...
/**
 * A trait that can be mixed into an class (probably a case class) so that the class can be
 * converted automatically into JSON or XML
 */
trait JsonXmlAble
