...
package net.liftweb {
  package mockweb {
    ...
    /**
     * The MockWeb object contains various methods to simplify unit testing in Lift outside of the
     * full-blown testkit stack.
     *
     * There is partial support for configuration defined in the LiftRules object. This includes:
     *
     * <ul> <li>early</li> <li>statelessRewrite</li> <li>statelessTest</li> <li>statefulRewrite</li>
     * </ul>
     */
    object MockWeb {

      /**
       * Setting this var to <code>true</code> will force all tests to use LiftRules. See
      ...
      /**
       * Executes a given function against a new Req constructed from the given url/path String and
       * contextPath. See MockHttpServletRequest.processUrl for details on the url String format,
       * and see testReq(HttpServletRequest) for more details on how the Req is processed.
       */
       ...
       * If useLiftRules is set to true, then LiftRules.early, LiftRules.statelessRewrite, and
      ...
      /**
       * Sets up S based on the provided url, contextPath and session. You can provide your own
       * session if you want to simulate sharing a session across multiple requests. For example:
       *
       * <pre name="code" class="scala"> object testVar extends SessionVar[String]("Empty")
       *
       * val testSession = testS("http://foo.com/test") { testVar("Foo!") S.session // returns the
       * current session }
       *
       * // A second test testS("http://foo.com/test2", session = testSession) { testVar.is must_==
       * "Foo!" } </pre>
       *
       * @param url
       *   The url to use for this request. Can either be a full URL, or just the path and
       *   queryString. See MockHttpServletRequest.processUrl for more details
       *
       * @param session
       *   The LiftSession to use for this request. If you don't provide one a new one will be
       *   created for you
       *
       * @param contextPath
      ...
      /**
       * Sets up S based on the provided request and session. You can use this method if you want to
       * do special setup on the request outside of what is handled by the MockHttpServletRequest
       * constructor, or if you want to use a different mock impl.
       *
       * You can provide your own session if you want to simulate sharing a session across multiple
       * requests. See testS(String,String,Box[LiftSession]) for an example of this use.
       *
       ...
       * @param session
       *   The LiftSession to use for this request. Passing Empty will force creation of a new
       *   session
       * @param testFunc
       *   The function to be executed in the scope of a new S
       */
      ...
      /**
       * This is the common delegate for the testS methods to avoid code duplication.
       */
       ...
       *
       * @param name
       *   The snippet name to be tested. For example, &lt;lift:MyWidget.foo/> has a name of
       *   MyWidget.foo. You can retrieve this via <code>S.currentSnippet</code> or
       *   <code>S.invokedAs</code>
       *
       * @param attrs
       *   The list of snippet tag attributes. Defaults to Null. See <code>S.attrs</code> for more
       *   details
       *
       * @param f
