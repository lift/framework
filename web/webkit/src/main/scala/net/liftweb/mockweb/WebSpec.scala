...
/**
 * This trait provides Lift-specific extensions to the Specification base trait to simplify unit
 * testing of your code. In addition to the Scaladoc, Please see the source to WebSpecSpec.scala for
 * an example of how to use this. This class allows you to optionally configure LiftRules for your
 * spec. LiftRules rules are always used, so if you don't want that to happen you can just use
 * MockWeb directly in your own custom spec and simply omit any LiftRules setup.
 *
 * @param boot
 *   defines a method that is called prior to testing to set up the Lift environment. This is where
 *   you'll initialize LiftRules, Mapper, etc. The simplest approach is to just point this at your
 *   Boostrap.boot method.
 */
abstract class WebSpec(boot: () => Any = () => {}) extends Specification with XmlMatchers {

  /**
   * This is our private spec instance of Liftrules. Everything we run will use this instance
  ...
  /**
   * A class that bridges between the description string and classes that provide the Lift-specific
   * wrapping of the Expectation code. The method names here should be self-explanatory. For methods
   * that take a Box[LiftSession], an Empty Box will result in the creation of a new Session.
   */
  ...
  /**
   * Converts a String description into a WebSpecBridge that can then be used to set up either an S
   * or Req instance.
   */
  ...
  trait ModifiableRequest[T <: ModifiableRequest[T]] {
    ...
    /**
     * Modifies the request to POST the given request body text. Optionally, you can set the content
    ...
    /**
     * Modifies the request to PUT the given request body text. Optionally, you can set the content
     * type (defaults to "text/plain")
     */
    ...
    /**
     * Allows you to specify your own modification function for the servlet request prior to
     * initialization.
     */
    def withMods(f: MockHttpServletRequest => Unit): T = req match {
      case r: MockHttpServletRequest => f(r); this
      case _ =>
        throw new IllegalArgumentException("We can only mutate MockHttpServletRequest instances")
    }
  }
