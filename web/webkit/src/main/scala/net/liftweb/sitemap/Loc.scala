...
trait Loc[T] {
   ...
   */
  def calcHref(in: T): String = appendQueryParameters(
    link.createPath(in),
    Full(in))
  ...
  /**
   * A `PartialFunction` that maps a snippet name, and an optional `Loc` value, in a `Tuple2`, to a
   * snippet function (`NodeSeq` => `NodeSeq`).
   */
  type SnippetTest = PartialFunction[(String, Box[T]), NodeSeq => NodeSeq]

  /**
   * The snippets defined by the `Loc` class itself, as opposed to those provided by its
   * `LocParams`.
   */
   ...
   */
  protected def findStatelessCalc: (Box[Loc.CalcStateless], Box[Loc.CalcParamStateless[T]]) = (
    allParams.collect {
   ...
   */
  protected lazy val _foundStatelessCalc: (Box[Loc.CalcStateless], Box[Loc.CalcParamStateless[T]]) =
    findStatelessCalc
  ...
  protected def _testAccess: Either[Boolean, Box[() => LiftResponse]] = {
    def testParams(what: List[Loc.LocParam[T]]): Either[Boolean, Box[() => LiftResponse]] =
      what match {
        case Nil => Left(true)
  ...
  /**
   * The template assocaited with this Loc, if any. Any Loc.Template or Loc.ValueTemplate parameter
   * will take precedence over a value returned by the calcTemplate method.
   */
  ...
  /**
   * The title to be displayed for the value associated with this Loc. Any Loc.Title parameter will
   * take precedence over the value returned by the linkText method.
   */
  def title(in: T): NodeSeq = paramTitle.map(_.apply(in)) openOr linkText(in)

  /**
   * The title of the location given the current value associated with this Loc. If no current value
   * is available, this will use the name of this Loc as the title.
   */
...
object Loc {
   ...
   *
   * @param name
   *   -- the name of the location. This must be unique across your entire sitemap. It's used to
   *   look up a menu item in order to create a link to the menu on a page.
   * @param link
  ...
  def apply(
      ...
      link: Link[Unit],
      text: LinkText[Unit],
      params: LocParam[Unit]*): Loc[Unit] = UnitLoc(name, link, text, params.toList)
  def apply(
      name: String,
      link: Link[Unit],
      text: LinkText[Unit],
      params: List[LocParam[Unit]]): Loc[Unit] = UnitLoc(name, link, text, params)
  ...
  /**
   * Indicates that the path denominated by Loc requires HTTP authentication and only a user
   * assigned to this role or to a role that is child-of this role can access it.
   */
  ...
  /**
   * Allows you to generate an early response for the location rather than going through the whole
   * Lift XHTML rendering pipeline
   */
  case class EarlyResponse(func: () => Box[LiftResponse]) extends AnyLocParam

  /**
   * Tests to see if the request actually matches the requirements for access to the page. For
   * example, if a parameter is missing from the request, this is a good way to restrict access to
   * the page.
   */
  case class Test(test: Req => Boolean) extends AnyLocParam

  /**
   * If the test returns True, the page can be accessed, otherwise, the result of FailMsg will be
   * sent as a response to the browser. If the Loc cannot be accessed, it will not be displayed in
   * menus.
   *
   * @param test
  ...
  /**
   * MenuCssClass is used to add css to the Menu node. The css allows for replacing menu with an
   * icon and other super-fun and helpful things. cssClass is a StringFunc which can either be a
   * String constant or a Function that returns a String. Thus, you can compute the css based on the
   * current state or you can have a constant. Syntactically you can use either: <pre>
   * MenuCssClass("foobar") MenuCssClass(() => calculateCssForMyMenuItem()) </pre>
   */
  case class MenuCssClass(cssClass: StringFunc) extends AnyLocParam

  /**
   * Unless the test returns True, the page can be accessed, otherwise, the result of FailMsg will
   * be sent as a response to the browser. If the Loc cannot be accessed, it will not be displayed
   * in menus.
   *
   * @param test
   *   -- the function that tests access to the page
  ...
  /**
   * Allows extra access testing for a given menu location such that you can generically return a
   * response during access control testing
   */
  ...
  /**
   * Allows a user to specify a template based upon a function from the current value encapsulated
   * in the Loc
   */
  ...
  /**
   * Allows a user to specify a template based upon a function from the current value encapsulated
   * in the Loc. Allow the return of Box[NodeSeq] so that it's more friendly to Templates.
   */
  ...
  /**
   * This LocParam may be used to specify a function that calculates a title for the page based upon
   * the current value encapsulated by this Loc.
   */
  case class Title[T](title: T => NodeSeq) extends LocParam[T]

  /**
   * If the Loc is in a group (or groups) like "legal" "community" etc. the groups can be specified
   * and recalled at the top level
   */
  case class LocGroup(group: String*) extends AnyLocParam

  /**
   * Calculate the value for the Loc. This is useful for parameterized menus. It allows you to
   * calculate the value of the Loc.
   */
  ...
  /**
   * The common interface for `LocParam`s that provide snippet functions, which can be aware of the
   * `Loc` value.
   * @tparam A
  ...
  object ValueSnippets {
     ...
     *   to a snippet function (`NodeSeq` => `NodeSeq`).
     */
    def apply[A](pf: PartialFunction[(String, Box[A]), NodeSeq => NodeSeq]): ValueSnippets[A] =
      new ValueSnippets[A] {
        def snippets = pf
      }
  }
   ...
   *   the snippet name
   * @param _func
   *   the snippet function. Note that this is a call-by-name parameter; that is, the function
   *   expression passed in will be evaluated each time the function is invoked.
   */
  class Snippet(val name: String, _func: => NodeSeq => NodeSeq) extends ValueSnippets[Any]
      with AnyLocParam {
  ...
  object Snippet {
    ...
    /**
     * Build a Loc.Snippet instance out of a name and a DispatchSnippet (or StatefulSnippet,
     * LiftScreen or Wizard). The "render" method will be invoked on the Dispatch snippet
     */
    ...
    /**
     * Build a Loc.Snippet instance for a given name and a function. Note that the function is
     * call-by-name so that it will be created each time it's used. This is useful for CSS Selector
     * Transforms
     */
  ...
  /**
   * Allows you to create a handler for many snippets that are associated with a Loc, but agnostic
   * to the `Loc`'s value.
   * @see
   *   ValueSnippets
   * @see
   *   ValueSnippets.apply
   */
  trait LocSnippets extends PartialFunction[String, NodeSeq => NodeSeq] with ValueSnippets[Any]
      with AnyLocParam {
    def snippets = {
  ...
  /**
   * A subclass of LocSnippets with a built in dispatch method (no need to implement isDefinedAt or
   * apply... just def dispatch: PartialFunction[String, NodeSeq => NodeSeq].
   * @see
   *   ValueSnippets
   * @see
   *   ValueSnippets.apply
  ...
  /**
   * If this parameter is included, the item will not be visible in the menu, but will still be
   * accessable.
   */
  case object Hidden extends AnyLocParam

  /**
   * If this parameter is included, the Loc will continue to execute even if currentValue is not
   * defined.
   *
   * By default, Lift will determine that a Loc does not match a given request if its currentValue
   * comes up Empty, and as a result will return an HTTP 404. For situations where this is not the
   * desired, "Not Found" behavior, you can add the MatchWithoutCurrentValue LocParam to a Loc, then
   * use the IfValue LocParam to define what should happen when the currentValue is Empty.
   *
   * For example, given some class Thing, you could do the following to trigger a redirect when a
   * Thing with a particular ID isn't found.
   *
   * {{{
  ...
  /**
   * The Loc does not represent a menu itself, but is the parent menu for children (implies
   * HideIfNoKids)
   */
  ...
  /**
   * A function that calculates the statelessness of the Loc for the given request with the
   * parameterized type passed into the function
   */
  case class CalcParamStateless[-T](f: Box[T] => Boolean) extends LocParam[T]

  /**
   * A function that can be used to calculate the link text from the current value encapsulated by
   * the Loc.
   */
   ...
   *
   * @param matchHead_?
   *   -- false -- absolute match. true -- match anything that begins with the same path. Useful for
   *   opening a set of directories (for example, help pages)
   */
  class Link[-T](val uriList: List[String], val matchHead_? : Boolean)
    ...
    /**
     * Returns the value created by createPath wrapped in a boxed scala.xml.Text element. NOTE: This
     * does not create a clickable HTML link on its own!
     */
