...
/**
 * A common trait between Menu and something that can be converted to a Menu. This makes building
 * Lists of things that can be converted to Menu instance easier because there's a common trait.
 */
...
/**
 * A common trait that defines a portion of a Menu's Link URI path. This allows us to constrain how
 * people construct paths using the DSL by restricting it to Strings or to the <pre>**</pre> object.
 */
...
/**
 * This object may be appended to a Menu DSL path, with the syntax <pre>Menu("Foo") / "test" /
 * **</pre> to match anything starting with a given path. For more info, see Loc.Link.matchHead_?
 *
 * @see
 *   Loc.Link
 */
...
/**
 * Defines a single path element for a Menu's Link URI. Typically users will not utilize this case
 * class, but will use the WithSlash trait's "/" method that takes Strings.
 */
...
object Menu extends MenuSingleton {
  ...
  class PreParamMenu[T <: AnyRef](
    ...
    def /(pathElement: LocPath): ParamMenuable[T] with WithSlash =
      new ParamMenuable[T](name, linkText, parser, encoder, pathElement :: Nil, false, Nil, Nil)
        with WithSlash

    /**
     * The Java way of building menus. Put the path String here, for example "/foo/bar" or "/foo/ *
     * /bar"
     */
  ...
  class ParamMenuable[T](
    ...
    def buildOne(newPath: List[LocPath], newHead: Boolean): BuiltType =
      new ParamMenuable[T](name, linkText, parser, encoder, newPath, newHead, params, submenus)
    ...
    /**
     * Convert the ParamMenuable into a Loc so you can access the well typed currentValue
     */
  ...
  class ParamsMenuable[T](
    ...
    def buildOne(newPath: List[LocPath], newHead: Boolean): BuiltType =
      new ParamsMenuable[T](name, linkText, parser, encoder, newPath, newHead, params, submenus)
    def buildSlashOne(newPath: List[LocPath], newHead: Boolean): BuiltType with WithSlash =
      new ParamsMenuable[T](name, linkText, parser, encoder, newPath, newHead, params, submenus)
        with WithSlash
    ...
    /**
     * Convert the ParamsMenuable into a Loc so you can access the well typed currentValue
     */
  ...
  /**
   * This trait contains helper method that will extract parameters and convert path items based on
   * the locPath
   */
  trait ParamExtractor[ConvertFrom, ConvertTo] {
    ...
    /**
     * Convert the List[String] extracted from the parse params into whatever is necessary to
     * convert to a ConvertTo
     */
    ...
    override lazy val rewrite: LocRewrite =
      Full(NamedPF(locPath.toString) {
        case RewriteRequest(
              ParsePath(
                ExtractSan(path, param),
                _,
                _,
                _),
              _,
              _) if param.isDefined || params.contains(Loc.MatchWithoutCurrentValue) => {
    ...
    /**
     * Given an incoming request path, match the path and extract the parameters. If the path is
     * matched, return all the extracted parameters. If the path matches, the return Full box will
     * contain the rewritten path and the extracted path parameter
     */
  ...
  class Menuable(
    ...
    def buildSlashOne(newPath: List[LocPath], newHead: Boolean): BuiltType with WithSlash =
      new Menuable(name, linkText, newPath, newHead, params, submenus) with WithSlash
...
sealed trait MenuSingleton {
  ...
  /**
   * A Menu can be created with the syntax <pre>Menu("Home") / "index"</pre> The first parameter is
   * the LinkText which calculates how Links are presented. The parameter to Menu will be treated as
   * call-by-name such that it is re-evaluated each time the menu link is needed. That means you can
   * do <pre>Menu(S ? "Home") / "index"</pre> and the menu link will be localized for each display.
   */
  def apply(linkText: Loc.LinkText[Unit]): PreMenu = this.apply(Helpers.randomString(20), linkText)

  /**
   * A Menu can be created with the syntax <pre>Menu("home_page", "Home") / "index"</pre> The first
   * parameter is the name of the page and the second is the LinkText which calculates how Links are
   * presented. The LinkText parameter to Menu will be treated as call-by-name such that it is
   * re-evaluated each time the menu link is needed. That means you can do <pre>Menu("home_page", S
   * ? "Home") / "index"</pre> and the menu link will be localized for each display. You can look up
   * a Menu item by name as well as using the &lt;lift:menu.item name="home_page"&gt; snippet.
   */
   ...
   * <pre>Menu.i("Home") / "index"</pre> is short-hand for <pre>Menu("Home", S.loc("Home",
   * Text("Home")) / "index"</pre>
   */
...
case class Menu(loc: Loc[_], private val convertableKids: ConvertableToMenu*) extends HasKids
  ...
  /**
   * Rebuild the menu by mutating the child menu items. This mutation can be changing, adding or
   * removing
   */
  ...
  private[sitemap] def validate: Unit = {
    _parent.foreach(p =>
      if (p.isRoot_?)
        throw new SiteMapException("Menu items with root location (\"/\") cannot have children"))
  ...
  override private[sitemap] def testAccess: Either[Boolean, Box[() => LiftResponse]] =
    loc.testAccess
  ...
  override def buildUpperLines(
    ...
    val kids: List[MenuItem] =
      _parent.toList.flatMap(_.kids.toList.flatMap(m =>
        m.loc.buildItem(
          if (m == this)
            populate
          else
...
final class ParamLocLink[T](path: List[LocPath], headMatch: Boolean, backToList: T => List[String])
