...
/**
 * Decorates `Future` instance to allow access to session and request resources. Should be created
 * with `FutureWithSession.withCurrentSession` that takes the current Lift request and session and
 * make it available to all transformation methods and initial `Future` execution body. Each
 * transformation method returns `FutureWithSession`, thus, they can be all chained together.
 *
 * It's important to bear in mind that each chained method requires current thread's `LiftSession`
 * to be available. `FutureWithSession` does _not_ propagate initial session or request to all
 * chained methods.
 *
 * @see
 ...
 *
 * @param delegate
 *   original `Future` instance that will be enriched with session and request access
 */
private[http] class FutureWithSession[T](private[this] val delegate: Future[T]) extends Future[T] {
  ...
  override def recover[U >: T](pf: PartialFunction[Throwable, U])(implicit
      executor: ExecutionContext): FutureWithSession[U] = {
    val sessionFn = withCurrentSession(pf)
  ...
  override def recoverWith[U >: T](pf: PartialFunction[Throwable, Future[U]])(implicit
      executor: ExecutionContext): FutureWithSession[U] = {
    val sessionFn = withCurrentSession(pf)
...
object FutureWithSession {
  ...
  /**
   * Creates `Future` instance aware of current request and session. Each `Future` returned by
   * chained transformation method (e.g. `map`, `flatMap`) will be also request/session-aware.
   * However, it's important to bear in mind that initial request and session are not propagated to
   * chained methods. It's required that current execution thread for chained method has its own
   * request/session available if reading/writing some data to it as a part of chained method
   * execution.
   */
  ...
  private def apply[T](task: => T)(implicit
      executionContext: ExecutionContext): FutureWithSession[T] = {
    S.session match {
      case Full(_) =>
