...
/**
 * Given an instance method or variable `rules`, wraps the spec in a setup of that rules instance as
 * the one used by Lift for the duration of the spec.
 */
...
/**
 * Wraps a spec in a context where `rules` are the Lift rules in effect, `session` is the current
 * Lift session, and `req`, if specified, is the current request.
 */
